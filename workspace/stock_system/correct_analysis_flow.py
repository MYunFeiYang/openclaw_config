#!/usr/bin/env python3
"""
A股分析系统 - 正确的"先预测再总结"流程
预测阶段生成个股分析，总结阶段基于已有预测结果生成综合报告
"""
...
class StockPredictionSystem:
    ...
    def generate_predictions(self, analysis_type: str = "evening") -> List[Dict]:
        """生成个股预测（这是预测阶段，不是总结阶段）"""
        
        print(f"📊 开始生成个股预测 ({analysis_type})...")
        
        predictions = []
        
        # 使用基于时间和分析类型的确定性种子，确保一致性
        seed_str = f"{datetime.now().strftime('%Y%m%d')}{hash(analysis_type)}"
        seed = hash(seed_str) % (2**31)  # 转换为有效的随机种子
        import random
        random.seed(seed)
...
class StockSummarySystem:
    ...
    def generate_summary(self, predictions: List[Dict], analysis_type: str) -> Dict:
        """基于已有预测结果生成总结（这是总结阶段）"""
    ...
    def _analyze_market_overview(self, predictions: List[Dict], buy: List[Dict], sell: List[Dict], hold: List[Dict]) -> Dict:
        """分析市场概况"""
        ...
        # 行业分布
        buy_sectors = list(set(p['sector'] for p in buy))
        sell_sectors = list(set(p['sector'] for p in sell))
        hold_sectors = list(set(p['sector'] for p in hold))
    ...
    def _analyze_sectors(self, predictions: List[Dict]) -> Dict:
        ...
        for sector, scores in sector_scores.items():
            ...
            sector_analysis[sector] = {
                ...
                'top_stock': max([p for p in predictions if p['sector'] == sector], key=lambda x: x['final_score'])['name']
            }
    ...
    def _add_sector_analysis_summary(self, sector_analysis: Dict, report_lines: List[str]):
        ...
        if sector_analysis:
            ...
            for sector, data in sector_analysis.items():
                ...
                report_lines.append(f"       代表股票: {data['top_stock']}")
...
class StockAnalysisManager:
    ...
    def run_full_analysis(self, analysis_type: str = "evening") -> Dict:
        """运行完整的分析流程：先预测，再总结"""
        
        print(f"🚀 启动A股完整分析流程 ({analysis_type})")
        print("=" * 70)
        ...
        if not predictions:
            print("❌ 预测生成失败")
            return {'success': False, 'error': '预测生成失败'}
        ...
        # 第二步：总结阶段（基于预测结果）
        print("\n📋 第二步：基于预测结果生成总结...")
        summary = self.summary_system.generate_summary(predictions, analysis_type)
        ...
        print(f"   买入推荐: {len(summary['buy_recommendations'])}只")
        print(f"   卖出推荐: {len(summary['sell_recommendations'])}只")
        print(f"   持有推荐: {len(summary['hold_recommendations'])}只")
        print(f"   市场情绪: {summary['market_overview']['market_sentiment']}")
    ...
    def generate_summary_only(self, analysis_type: str = "evening") -> Dict:
        """仅生成总结（基于最新的预测结果）"""
        
        print(f"📋 生成总结报告 ({analysis_type}) - 基于已有预测结果...")
        
        # 查找最新的预测结果
        latest_predictions = self._find_latest_predictions(analysis_type)
        
        if not latest_predictions:
            print("❌ 未找到预测结果，无法生成总结")
            return {'success': False, 'error': '未找到预测结果'}
        
        print(f"📊 找到预测结果，共{len(latest_predictions)}只股票")
        
        # 基于已有预测结果生成总结
        summary = self.summary_system.generate_summary(latest_predictions, analysis_type)
        saved_files = self.summary_system.save_summary(summary, analysis_type)
        ...
        return {
            'success': True,
            'analysis_type': analysis_type,
            'based_on_predictions': latest_predictions,
            'summary': summary,
            'summary_files': saved_files,
            'report_text': saved_files['report_text']
        }
    
    def _find_latest_predictions(self, analysis_type: str) -> Optional[List[Dict]]:
        """查找最新的预测结果"""
        
        # 查找今天生成的预测文件
        today = datetime.now().strftime("%Y%m%d")
        pattern = f"predictions_{analysis_type}_{today}_*.json"
...
def main():
    ...
    if result['success']:
        ...
        print(f"\n💾 文件保存位置:")
        print(f"   预测数据: {result['prediction_file']}")
        print(f"   总结数据: {result['summary_files']['summary_file']}")
        print(f"   文本报告: {result['summary_files']['report_file']}")
        
        print(f"\n✅ 分析完成！")
        print(f"🎯 系统特点: 先预测个股，再基于预测结果生成综合总结")
        print(f"📊 分析基础: 基于{len(result['predictions'])}只股票的预测结果")
