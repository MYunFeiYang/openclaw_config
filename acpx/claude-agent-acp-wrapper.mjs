...
const installedBinPath = "/Users/thinkway/.openclaw/npm/node_modules/@agentclientprotocol/claude-agent-acp/dist/index.js";
let defaultCommand;
let defaultArgs;
if (installedBinPath) {
  ...
  defaultCommand = process.execPath;
  defaultArgs = [npmCliPath, "exec", "--yes", "--package", "@agentclientprotocol/claude-agent-acp@0.33.1", "--", "claude-agent-acp"];
} else {
  defaultCommand = process.platform === "win32" ? "npx.cmd" : "npx";
  defaultArgs = ["--yes", "--package", "@agentclientprotocol/claude-agent-acp@0.33.1", "--", "claude-agent-acp"];
}
const command =
