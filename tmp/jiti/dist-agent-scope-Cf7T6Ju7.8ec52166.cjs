...
function resolveSessionAgentIds(params) {
  const defaultAgentId = (0, _agentScopeConfigDUwbsKjv.c)(params.config ?? {});
...
function resolveAgentExecutionContract(cfg, agentId) {
  ...
  return (0, _agentScopeConfigDUwbsKjv.r)(cfg, agentId)?.embeddedPi?.executionContract ?? defaultContract;
}
...
function resolveAgentExplicitModelPrimary(cfg, agentId) {
  const raw = (0, _agentScopeConfigDUwbsKjv.r)(cfg, agentId)?.model;
...
function resolveAgentModelFallbacksOverride(cfg, agentId) {
  return resolveSelectedModelFallbacksOverride((0, _agentScopeConfigDUwbsKjv.r)(cfg, agentId)?.model);
}
...
function resolveSubagentModelConfigSelectionResult(params) {
  const agentConfig = params.agentConfigOverride ?? (params.agentId ? (0, _agentScopeConfigDUwbsKjv.r)(params.cfg, params.agentId) : void 0);
  return [
...
function resolveSubagentModelFallbacksOverride(cfg, agentId) {
  const agentConfig = (0, _agentScopeConfigDUwbsKjv.r)(cfg, agentId);
...
function resolveAgentIdsByWorkspacePath(cfg, workspacePath) {
  ...
  const ids = (0, _agentScopeConfigDUwbsKjv.n)(cfg);
  const matches = [];
  for (let index = 0; index < ids.length; index += 1) {
    const id = ids[index];
    const workspaceDir = normalizePathForComparison((0, _agentScopeConfigDUwbsKjv.o)(cfg, id));
