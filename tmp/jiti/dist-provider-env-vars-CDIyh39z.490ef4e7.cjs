...
function isWorkspacePluginTrustedForProviderEnvVars(plugin, config) {
  return (0, _providerAuthAliasesCljpontN.i)({
    config,
    isImplicitlyAllowed: (pluginId) => (0, _slotsMFUVcT3n.r)(plugin.kind, "context-engine") && (0, _providerAuthAliasesCljpontN.a)(config?.plugins?.slots?.contextEngine) === pluginId,
    plugin
  });
}
...
function resolveProviderMetadataSnapshot(params) {
  ...
  if ((0, _configStateCOhlckC.s)(config.plugins).loadPaths.length === 0) {
    const unscopedCurrent = (0, _pluginRegistryIimloLsx.v)({
      env,
      ...(params?.workspaceDir !== void 0 ? { workspaceDir: params.workspaceDir } : {}),
      allowWorkspaceScopedSnapshot: true,
...
function resolveManifestProviderAuthEnvVarCandidates(params) {
  ...
  const aliases = (0, _providerAuthAliasesCljpontN.n)(params);
...
function resolveManifestProviderAuthEvidence(params) {
  ...
  for (const plugin of snapshot.plugins) {
    if (snapshot.index.plugins.length > 0 && !(0, _installedPluginIndexStoreIOgCdyHd.d)(snapshot.index, plugin.id, params?.config)) continue;
