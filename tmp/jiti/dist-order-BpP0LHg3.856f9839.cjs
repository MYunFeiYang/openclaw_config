...
function isOpenAIApiKeyCompatibleWithCodexAuth(params) {
  ...
  const providerKey = (0, _providerAuthAliasesCljpontN.r)(params.profileProvider ?? "", { config: params.cfg });
...
function isCredentialProviderCompatibleWithAuthProvider(params) {
  return (0, _providerAuthAliasesCljpontN.r)(params.credential.provider, { config: params.cfg }) === params.providerAuthKey || isOpenAIApiKeyCompatibleWithCodexAuth({
    cfg: params.cfg,
...
function isStoredCredentialCompatibleWithAuthProvider(params) {
  return isCredentialProviderCompatibleWithAuthProvider({
    cfg: params.cfg,
    providerAuthKey: (0, _providerAuthAliasesCljpontN.r)(params.provider, { config: params.cfg }),
    credential: params.credential
  });
}
function isConfiguredProfileCompatibleWithAuthProvider(params) {
  return (0, _providerAuthAliasesCljpontN.r)(params.provider, { config: params.cfg }) === params.providerAuthKey || isOpenAIApiKeyCompatibleWithCodexAuth({
    cfg: params.cfg,
    providerAuthKey: params.providerAuthKey,
    credential: params.credential,
...
function listProfilesCompatibleWithAuthProvider(params) {
  if (params.providerAuthKey !== OPENAI_CODEX_PROVIDER_ID) return (0, _profileListCdhQb6vz.n)(params.store, params.provider);
...
function isConfiguredAwsSdkAuthProfileForProvider(params) {
  ...
  const providerAuthKey = (0, _providerAuthAliasesCljpontN.r)(params.provider, { config: params.cfg });
  if ((0, _providerAuthAliasesCljpontN.r)(profileConfig.provider, { config: params.cfg }) !== providerAuthKey) return false;
...
function resolveAuthProfileEligibility(params) {
  ...
  const credentialEligibility = (0, _storeBYnnXRZ.N)({
    credential: cred,
    now: params.now
  });
  return {
...
function resolveAuthProfileOrder(params) {
  ...
  const providerAuthKey = (0, _providerAuthAliasesCljpontN.r)(provider, { config: cfg });
  const now = Date.now();
  ...
  const deduped = (0, _profileListCdhQb6vz.t)(filtered);
...
function isNativeCredentialProviderCompatibleWithAuthProvider(params) {
  ...
  return (0, _providerAuthAliasesCljpontN.r)(params.credential.provider, { config: params.cfg }) === params.providerAuthKey;
}
function mergeAliasOrderWithNativeProfiles(params) {
  const nativeIds = new Set(params.nativeProfiles);
  return (0, _profileListCdhQb6vz.t)(params.aliasOrder.some((profileId) => nativeIds.has(profileId)) ? [...params.aliasOrder, ...params.nativeProfiles] : [...params.nativeProfiles, ...params.aliasOrder]);
}
