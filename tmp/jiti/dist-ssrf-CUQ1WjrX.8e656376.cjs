...
var _hostnameLWxbUOHs = require("./hostname-LWxbUOHs.js");
...
function normalizeSsrFPolicyHostnames(values) {
  ...
  return Array.from(new Set(values.map((value) => (0, _hostnameLWxbUOHs.t)(value)).filter(Boolean))).toSorted();
}
...
function normalizeHostnameSet(values) {
  ...
  return new Set(values.map((value) => (0, _hostnameLWxbUOHs.t)(value)).filter(Boolean));
}
function normalizeHostnameAllowlist(values) {
  if (!values || values.length === 0) return [];
  return Array.from(new Set(values.map((value) => (0, _hostnameLWxbUOHs.t)(value)).filter((value) => value !== "*" && value !== "*." && value.length > 0)));
}
...
function isPrivateIpAddress(address, policy) {
  const normalized = (0, _hostnameLWxbUOHs.t)(address);
...
function isBlockedHostname(hostname) {
  const normalized = (0, _hostnameLWxbUOHs.t)(hostname);
  if (!normalized) return false;
...
function createPinnedLookup(params) {
  const normalizedHost = (0, _hostnameLWxbUOHs.t)(params.hostname);
  ...
  return (host, options, callback) => {
    ...
    if (!cb) return;
    const normalized = (0, _hostnameLWxbUOHs.t)(host);
...
function resolvePinnedDispatcherLookup(pinned, override, policy) {
  ...
  const normalizedOverrideHost = (0, _hostnameLWxbUOHs.t)(override.hostname);
...
function createPinnedDispatcher(pinned, policy, ssrfPolicy, timeoutMs) {
  ...
  if (!policy || policy.mode === "direct") return (0, _undiciRuntimeDDjv6AiC.t)({ connect: withPinnedLookup(lookup, policy?.connect) }, timeoutMs);
  if (policy.mode === "env-proxy") return (0, _undiciRuntimeDDjv6AiC.n)({
    connect: withPinnedLookup(lookup, policy.connect),
    ...(policy.proxyTls ? { proxyTls: { ...policy.proxyTls } } : {})
  }, timeoutMs);
  const proxyUrl = policy.proxyUrl.trim();
  const requestTls = withPinnedLookup(lookup, policy.proxyTls);
  if (!requestTls) return (0, _undiciRuntimeDDjv6AiC.r)({ uri: proxyUrl }, timeoutMs);
  return (0, _undiciRuntimeDDjv6AiC.r)({
    uri: proxyUrl,
    requestTls
  }, timeoutMs);
}
