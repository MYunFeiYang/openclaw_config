...
function assertBaseHashMatches(snapshot, expectedHash) {
  const currentHash = (0, _ioDnGGugrj.y)(snapshot) ?? null;
...
async function readConfigSnapshotForMutation(params) {
  ...
  return await (0, _ioDnGGugrj.d)({ skipPluginValidation: params.writeOptions?.skipPluginValidation });
}
...
async function writeJsonFileAtomic(filePath, value) {
  await (0, _replaceFileDfT5iHv.n)({
    ...
    beforeRename: async () => {
      await _promises.default.access(filePath).then(async () => await (0, _ioDnGGugrj.q)(filePath, _promises.default), () => void 0);
    }
  });
}
async function tryWriteSingleTopLevelIncludeMutation(params) {
  const nextConfig = (0, _ioDnGGugrj.z)(params.nextConfig, (0, _ioDnGGugrj.B)(params.writeOptions?.unsetPaths));
  ...
  const validated = (0, _ioDnGGugrj.T)(nextConfig, params.writeOptions?.skipPluginValidation ? { pluginValidation: "skip" } : void 0);
  if (!validated.ok) throw (0, _ioDnGGugrj.W)(params.snapshot.path, (0, _ioDnGGugrj.G)(validated.issues));
  ...
  if (params.writeOptions?.skipRuntimeSnapshotRefresh && !hadRuntimeSnapshot && !(0, _runtimeSnapshotBCdniHcd.o)()) return {
    persistedHash: null,
    persistedConfig: nextConfig
  };
  const refreshedSnapshot = (await (params.io?.readConfigFileSnapshotForWrite ?? _ioDnGGugrj.d)(params.writeOptions?.skipPluginValidation ? { skipPluginValidation: true } : void 0)).snapshot;
  const persistedHash = (0, _ioDnGGugrj.y)(refreshedSnapshot);
  if (!refreshedSnapshot.valid) throw (0, _ioDnGGugrj.W)(params.snapshot.path, (0, _ioDnGGugrj.G)(refreshedSnapshot.issues));
...
async function replaceConfigFileUnlocked(params) {
  ...
  const afterWrite = (0, _runtimeSnapshotBCdniHcd.p)(params.afterWrite ?? params.writeOptions?.afterWrite);
  ...
  if (!writeResult) writeResult = resolveConfigWriteResult(await (params.io?.writeConfigFile ?? _ioDnGGugrj.b)(params.nextConfig, {
  ...
  return {
    ...
    afterWrite,
    followUp: (0, _runtimeSnapshotBCdniHcd.m)(afterWrite)
  };
}
...
async function transformConfigFileAttempt(params, attempt) {
  ...
  return {
    ...
    followUp: (0, _runtimeSnapshotBCdniHcd.m)(committedAfterWrite)
  };
}
