...
function resolveGatewayCredentialsFromValues(params) {
  ...
  const envToken = (0, _credentialPlannerCmIPlWf.a)(env.OPENCLAW_GATEWAY_TOKEN);
  const envPassword = (0, _credentialPlannerCmIPlWf.a)(env.OPENCLAW_GATEWAY_PASSWORD);
  const configToken = (0, _credentialPlannerCmIPlWf.i)(params.configToken);
  const configPassword = (0, _credentialPlannerCmIPlWf.i)(params.configPassword);
...
function resolveGatewayCredentialsFromConfig(params) {
  const env = params.env ?? process.env;
  const explicitToken = (0, _credentialPlannerCmIPlWf.a)(params.explicitAuth?.token);
  const explicitPassword = (0, _credentialPlannerCmIPlWf.a)(params.explicitAuth?.password);
  if (explicitToken || explicitPassword) return {
    token: explicitToken,
    password: explicitPassword
  };
  if ((0, _credentialPlannerCmIPlWf.a)(params.urlOverride) && params.urlOverrideSource !== "env") return {};
  if ((0, _credentialPlannerCmIPlWf.a)(params.urlOverride) && params.urlOverrideSource === "env") return resolveGatewayCredentialsFromValues({
    configToken: void 0,
  ...
  const plan = (0, _credentialPlannerCmIPlWf.t)({
    config: params.cfg,
    env
  });
