...
var _channelCatalogRegistryDB8mFwRi = require("./channel-catalog-registry-DB8mFwRi.js");
...
function loadChannelPackageStateModule(params) {
  try {
    return (0, _bundledDowL9sGd.h)(params);
  } catch (error) {
    ...
    return (0, _pluginModuleLoaderCacheAuBzBauz.n)({
      ...
      importerUrl: "file:///Users/thinkway/.nvm/versions/node/v24.14.0/lib/node_modules/openclaw/dist/package-state-probes-BcFADgAF.js",
      tryNative: true,
...
function listBuiltBundledPackageStateModules(params) {
  ...
  for (const rootDir of [_nodePath.default.join(sourceRoot.packageRoot, "dist", "extensions", sourceRoot.dirName), _nodePath.default.join(sourceRoot.packageRoot, "dist-runtime", "extensions", sourceRoot.dirName)]) {
    const modulePath = (0, _bundledDowL9sGd.g)(rootDir, params.specifier);
...
function resolveChannelPackageStateModuleLocation(params) {
  return {
    modulePath: (0, _bundledDowL9sGd.g)(params.entry.rootDir, params.specifier),
...
function listChannelPackageStateCatalog(metadataKey) {
  return (0, _channelCatalogRegistryDB8mFwRi.t)({ origin: "bundled" }).filter((entry) => Boolean(resolveChannelPackageStateMetadata(entry, metadataKey)));
}
