...
var _identityBW3c3Rh = require("./identity-BW3c3Rh1.js");
...
const resolveAuthAgentDir = (agentDir, config) => agentDir ?? (0, _agentScopeConfigDUwbsKjv.s)(config ?? {});
...
function resolveProviderDefaultEnvSecretRef(provider, config) {
  const envVar = (0, _providerEnvVarsCDIyh39z.t)(provider, {
    ...(config ? { config } : {}),
...
function upsertApiKeyProfile(params) {
  const profileId = params.profileId ?? (0, _identityBW3c3Rh.t)({ providerId: params.provider });
  (0, _profilesDdWlNiEB.o)({
    profileId,
...
async function upsertAuthProfileWithLockOrThrow(params) {
  if (!(await (0, _profilesDdWlNiEB.s)(params))) throw new Error("Failed to update auth profile store; the auth store lock may be busy. Wait a moment and retry.");
}
function applyAuthProfileConfig(cfg, params) {
  const normalizedProvider = (0, _providerAuthAliasesCljpontN.r)(params.provider, { config: cfg });
  const profiles = {
    ...cfg.auth?.profiles,
    [params.profileId]: {
  ...
  const configuredProviderProfiles = Object.entries(cfg.auth?.profiles ?? {}).filter(([, profile]) => (0, _providerAuthAliasesCljpontN.r)(profile.provider, { config: cfg }) === normalizedProvider).map(([profileId, profile]) => ({
    profileId,
    mode: profile.mode
  }));
  const matchingProviderOrderEntries = Object.entries(cfg.auth?.order ?? {}).filter(([providerId]) => (0, _providerAuthAliasesCljpontN.r)(providerId, { config: cfg }) === normalizedProvider);
  ...
  const baseOrder = matchingProviderOrderEntries.length > 0 ? Object.fromEntries(Object.entries(cfg.auth?.order ?? {}).filter(([providerId]) => (0, _providerAuthAliasesCljpontN.r)(providerId, { config: cfg }) !== normalizedProvider)) : cfg.auth?.order;
...
async function writeOAuthCredentials(provider, creds, agentDir, options) {
  ...
  const profileId = (0, _identityBW3c3Rh.t)({
    providerId: provider,
