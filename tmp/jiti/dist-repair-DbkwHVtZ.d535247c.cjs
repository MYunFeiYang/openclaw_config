...
function suggestOAuthProfileIdForLegacyDefault(params) {
  ...
  const oauthProfiles = (0, _profileListCdhQb6vz.n)(params.store, providerKey).filter((id) => params.store.profiles[id]?.type === "oauth");
  if (oauthProfiles.length === 0) return null;
  const configuredEmail = legacyCfg?.email?.trim();
  if (configuredEmail) {
    const byEmail = oauthProfiles.find((id) => {
      return (0, _identityBW3c3Rh.n)({
        cfg: params.cfg,
        store: params.store,
        profileId: id
...
function repairOAuthProfileIdMismatch(params) {
  ...
  const { email: toEmail, displayName: toDisplayName } = (0, _identityBW3c3Rh.n)({
    store: params.store,
  ...
  const nextOrder = (() => {
    ...
    const deduped = (0, _profileListCdhQb6vz.t)(existing.map((id) => id === legacyProfileId ? toProfileId : id).filter((id) => typeof id === "string" && id.trim().length > 0));
    return {
      ...order,
      [resolvedKey]: deduped
    };
