"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.wecomPlugin = void 0;var _openclawCompat = require("./openclaw-compat.js");

var _runtime = require("./runtime.js");
var _monitor = require("./monitor.js");
var _stateManager = require("./state-manager.js");
var _onboarding = require("./onboarding.js");
var _accounts = require("./accounts.js");
var _const = require("./const.js");
var _mediaUploader = require("./media-uploader.js");
var _webhook = require("./agent/webhook.js");
var _target = require("./target.js");
var _apiClient = require("./agent/api-client.js");
var _index = require("./webhook/index.js");
/**
...
async function sendWeComMessage({ to, content, accountId, cfg }) {
  const resolvedAccountId = accountId ?? _openclawCompat.DEFAULT_ACCOUNT_ID;
  // 从 to 中提取目标（格式是 "${CHANNEL_ID}:xxx" 或直接是目标字符串）
  const channelPrefix = new RegExp(`^${_const.CHANNEL_ID}:`, "i");
  const chatId = to.replace(channelPrefix, "");
  // ── 尝试 Bot WebSocket ──
  const wsClient = (0, _stateManager.getWeComWebSocket)(resolvedAccountId);
  if (wsClient?.isConnected) {
    ...
    return { channel: _const.CHANNEL_ID, messageId, chatId };
  }
  // ── 回退到 Agent HTTP API ──
  if (!cfg) {
    throw new Error(`WSClient not connected for account ${resolvedAccountId} and no config available for Agent fallback`);
  }
  const account = (0, _accounts.resolveWeComAccountMulti)({ cfg, accountId: resolvedAccountId });
  ...
  const target = (0, _target.resolveWecomTarget)(chatId);
  if (!target) {
...
// 企业微信频道元数据
const meta = {
  id: _const.CHANNEL_ID,
  label: "企业微信",
  selectionLabel: "企业微信 (WeCom)",
  detailLabel: "企业微信智能机器人",
  docsPath: `/channels/${_const.CHANNEL_ID}`,
  docsLabel: _const.CHANNEL_ID,
  blurb: "企业微信智能机器人接入插件",
  systemImage: "message.fill"
};
const wecomPlugin = exports.wecomPlugin = {
  id: _const.CHANNEL_ID,
  meta: {
    ...meta,
  ...
  pairing: {
    idLabel: "wecomUserId",
    normalizeAllowEntry: (entry) => entry.replace(new RegExp(`^(${_const.CHANNEL_ID}|user):`, "i"), "").trim(),
    ...
    } }, setupWizard: _onboarding.wecomSetupWizard, setup: _onboarding.wecomSetupAdapter, capabilities: { chatTypes: ["direct", "group"], reactions: false,
    threads: false,
    media: true,
    nativeCommands: false,
    blockStreaming: true
  },
  reload: { configPrefixes: [`channels.${_const.CHANNEL_ID}`] },
  config: {
    // 多账号：列出所有账户 ID
    listAccountIds: (cfg) => (0, _accounts.listWeComAccountIds)(cfg),
    // 多账号：按 accountId 解析账户配置
    resolveAccount: (cfg, accountId) => (0, _accounts.resolveWeComAccountMulti)({ cfg, accountId }),
    // 多账号：获取默认账户 ID
    defaultAccountId: (cfg) => (0, _accounts.resolveDefaultWeComAccountId)(cfg),
    // 多账号：设置账户启用状态
    setAccountEnabled: ({ cfg, accountId, enabled }) => {
      if (!(0, _accounts.hasMultiAccounts)(cfg)) {
        // 单账号模式：设置顶层 enabled
        const wecomConfig = cfg.channels?.[_const.CHANNEL_ID] ?? {};
        return {
          ...cfg,
          channels: {
            ...cfg.channels,
    ...
    deleteAccount: ({ cfg, accountId }) => {
      if (!(0, _accounts.hasMultiAccounts)(cfg)) {
        ...
        delete nextChannels[_const.CHANNEL_ID];
    ...
    resolveAllowFrom: ({ cfg, accountId }) => {
      const account = (0, _accounts.resolveWeComAccountMulti)({ cfg, accountId });
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      const result = (0, _openclawCompat.buildAccountScopedDmSecurityPolicy)({
        cfg,
        channelKey: _const.CHANNEL_ID,
        accountId,
        ...
        approveHint: (0, _openclawCompat.formatPairingApproveHint)(_const.CHANNEL_ID),
        normalizeEntry: (raw) => raw.replace(new RegExp(`^${_const.CHANNEL_ID}:`, "i"), "").trim()
      });
      return result;
    },
    collectWarnings: ({ cfg, accountId }) => {
      ...
      const warnings = [];
      // 动态构造配置路径（区分单账号 / 多账号）
      const isMulti = (0, _accounts.hasMultiAccounts)(cfg);
      const basePath = isMulti && accountId ?
      `channels.${_const.CHANNEL_ID}.accounts.${accountId}.` :
      `channels.${_const.CHANNEL_ID}.`;
      // DM 策略警告
  ...
  outbound: {
    ...
    chunker: (text, limit) => (0, _runtime.getWeComRuntime)().channel.text.chunkMarkdownText(text, limit),
    textChunkLimit: _const.TEXT_CHUNK_LIMIT,
    ...
    sendMedia: async ({ to, text, mediaUrl, mediaLocalRoots, accountId, cfg }) => {
      ...
      try {
        ...
        if (mediaResponse.ok) {
          ...
          const mediaId = await (0, _apiClient.uploadMedia)({
            agent,
            type: 'file',
            buffer,
            filename
          ...
          if (text) {
            await (0, _apiClient.sendText)({ agent, toUser: target.touser, toParty: target.toparty, toTag: target.totag, chatId: target.chatid, text });
          }
          return { channel: _const.CHANNEL_ID, messageId: `agent-media-${Date.now()}`, chatId };
        }
      }
      catch (err) {
      ...
      await (0, _apiClient.sendText)({ agent, toUser: target.touser, toParty: target.toparty, toTag: target.totag, chatId: target.chatid, text: fallbackContent });
      return { channel: _const.CHANNEL_ID, messageId: `agent-${Date.now()}`, chatId };
    }
  },
  status: {
    defaultRuntime: {
      accountId: _openclawCompat.DEFAULT_ACCOUNT_ID,
      running: false,
      lastStartAt: null,
      lastStopAt: null,
      lastError: null
    ...
    collectStatusIssues: (accounts) => accounts.flatMap((entry) => {
      const accountId = String(entry.accountId ?? _openclawCompat.DEFAULT_ACCOUNT_ID);
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      const account = (0, _accounts.resolveWeComAccountMulti)({ cfg: ctx.cfg, accountId: ctx.accountId });
      // 读取连接模式（默认 websocket）
      ...
      const agent = account.agent;
      if (agent?.configured) {
        const isMulti = (0, _accounts.hasMultiAccounts)(ctx.cfg);
        const defaultId = (0, _accounts.resolveDefaultWeComAccountId)(ctx.cfg);
        const isDefault = ctx.accountId === defaultId;
        const paths = isMulti ?
        [
        `${_const.WEBHOOK_PATHS.AGENT_PLUGIN}/${ctx.accountId}`,
        `${_const.WEBHOOK_PATHS.AGENT}/${ctx.accountId}`,
        // 默认账号额外注册 /default 别名路径
        ...(isDefault && ctx.accountId !== _openclawCompat.DEFAULT_ACCOUNT_ID ?
        [
        `${_const.WEBHOOK_PATHS.AGENT_PLUGIN}/${_openclawCompat.DEFAULT_ACCOUNT_ID}`,
        `${_const.WEBHOOK_PATHS.AGENT}/${_openclawCompat.DEFAULT_ACCOUNT_ID}`] :

        []),
        _const.WEBHOOK_PATHS.AGENT_PLUGIN,
        _const.WEBHOOK_PATHS.AGENT] :
        ...
        for (const p of paths) {
          (0, _webhook.registerAgentWebhookTarget)({
            agent,
            config: ctx.cfg,
            runtime: {
        ...
        ctx.abortSignal.addEventListener("abort", () => {
          (0, _webhook.deregisterAgentWebhookTarget)(agent.accountId);
        }, { once: true });
      }
      ...
      if (hasBotCredentials) {
        return (0, _monitor.monitorWeComProvider)({
          account,
          config: ctx.cfg,
          runtime: ctx.runtime,
      ...
      if (connectionMode === "webhook") {
        ...
        (0, _index.startWebhookGateway)(gatewayCtx);
        // 等待 abortSignal 停止后清理
        await new Promise((resolve) => {
          if (ctx.abortSignal.aborted) {
            (0, _index.stopWebhookGateway)(gatewayCtx);
            resolve();
            return;
          }
          ctx.abortSignal.addEventListener("abort", () => {
    ...
    logoutAccount: async ({ cfg, accountId }) => {
      ...
      if (!isMulti) {
        ...
        if (changed) {
          if (Object.keys(nextWecom).length > 0) {
            nextCfg.channels = { ...nextCfg.channels, [_const.CHANNEL_ID]: nextWecom };
          } else
          {
      ...
      if (changed) {
        await (0, _runtime.getWeComRuntime)().config.writeConfigFile(nextCfg);
      }
      const resolved = (0, _accounts.resolveWeComAccountMulti)({ cfg: changed ? nextCfg : cfg, accountId: resolvedAccountId });
