...
var _providerRuntimeBzn5r2hY = require("./provider-runtime-Bzn5r2hY.js");
require("./source-check-BilAfCfE.js");
...
const log = (0, _subsystemLmgZvqF.t)("agents/auth-profiles");
...
function overlayRuntimeExternalOAuthProfiles(store, profiles) {
  ...
  const next = (0, _runtimeSnapshotsCbKCG9ZW.g)(store);
...
const EXTERNAL_CLI_SYNC_PROVIDERS = [
{
  ...
  readCredentials: (options) => readCodexCliCredentialsCached({
    ttlMs: _storeCacheCiDn8a.s,
    allowKeychainPrompt: options?.allowKeychainPrompt
  }),
  bootstrapOnly: true
},
{
  profileId: _storeCacheCiDn8a.a,
  provider: "claude-cli",
...
{
  profileId: _storeCacheCiDn8a.c,
  provider: "minimax-portal",
  aliases: ["minimax", "minimax-cli"],
  readCredentials: () => readMiniMaxCliCredentialsCached({ ttlMs: _storeCacheCiDn8a.s })
}];
...
function resolveExternalCliAuthProfiles(store, options) {
  ...
  for (const providerConfig of EXTERNAL_CLI_SYNC_PROVIDERS) {
    ...
    if (existing && !existingOAuth) {
      _storeCacheCiDn8a.f.debug("kept explicit local auth over external cli bootstrap", {
    ...
    if (providerConfig.bootstrapOnly && existingOAuth && hasInlineOAuthTokenMaterial$1(existingOAuth)) {
      _storeCacheCiDn8a.f.debug("kept local oauth over external cli bootstrap-only provider", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider
      });
      continue;
    ...
    if (existingOAuth && !isSafeToUseExternalCliCredential(existingOAuth, creds)) {
      _storeCacheCiDn8a.f.warn("refused external cli oauth bootstrap: identity mismatch", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider
      });
      continue;
    ...
    if (existingOAuth && !isSafeToAdoptBootstrapOAuthIdentity(existingOAuth, creds) && !areOAuthCredentialsEquivalent(existingOAuth, creds)) {
      _storeCacheCiDn8a.f.warn("refused external cli oauth bootstrap: identity mismatch or missing binding", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider
      });
      continue;
    ...
    if (!shouldBootstrapFromExternalCliCredential({
      ...
      if (existingOAuth) _storeCacheCiDn8a.f.debug("kept usable local oauth over external cli bootstrap", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider,
    ...
    _storeCacheCiDn8a.f.debug("used external cli oauth bootstrap because local oauth was missing or unusable", {
      profileId: providerConfig.profileId,
      provider: providerConfig.provider,
      localExpires: existingOAuth?.expires,
      externalExpires: creds.expires
...
function resolveExternalAuthProfileMap(params) {
  ...
  const profiles = (resolveExternalAuthProfilesForRuntime ?? _providerRuntimeBzn5r2hY.C)({
    env,
...
function hasPersistableExternalCliSyncCandidate(store, params) {
  ...
  for (const profileId of [_storeCacheCiDn8a.a, _storeCacheCiDn8a.c]) if (store.profiles[profileId]?.type === "oauth") return true;
  return false;
}
...
function syncPersistedExternalCliAuthProfiles(store, params) {
  ...
  for (const profile of persistedProfiles) {
    ...
    next ??= (0, _runtimeSnapshotsCbKCG9ZW.g)(store);
...
function loadPersistedAuthProfileState(agentDir) {
  return coerceAuthProfileState((0, _jsonFileDIl8If_.t)((0, _runtimeSnapshotsCbKCG9ZW.s)(agentDir)));
}
...
function savePersistedAuthProfileState(store, agentDir) {
  ...
  const statePath = (0, _runtimeSnapshotsCbKCG9ZW.s)(agentDir);
  if (!payload) {
    try {
...
function warnRejectedCredentialEntries(source, rejected) {
  ...
  _storeCacheCiDn8a.f.warn("ignored invalid auth profile entries during store load", {
    source,
...
function loadLegacyAuthProfileStore(agentDir) {
  return coerceLegacyAuthStore((0, _jsonFileDIl8If_.t)((0, _runtimeSnapshotsCbKCG9ZW.d)(agentDir)));
}
//#endregion
...
function isInheritedMainOAuthCredential(params) {
  ...
  if ((0, _runtimeSnapshotsCbKCG9ZW.l)(params.agentDir) === (0, _runtimeSnapshotsCbKCG9ZW.l)()) return false;
...
function resolveRuntimeAuthProfileStore(agentDir, options) {
  const mainKey = (0, _runtimeSnapshotsCbKCG9ZW.l)(void 0);
  const requestedKey = (0, _runtimeSnapshotsCbKCG9ZW.l)(agentDir);
  const mainStore = (0, _runtimeSnapshotsCbKCG9ZW.n)(void 0);
  const requestedStore = (0, _runtimeSnapshotsCbKCG9ZW.n)(agentDir);
...
function maybeSyncPersistedExternalCliAuthProfiles(params) {
  ...
  const authPath = (0, _runtimeSnapshotsCbKCG9ZW.l)(params.agentDir);
  const release = acquireAuthStoreLockSync(authPath);
  if (!release) {
    _storeCacheCiDn8a.f.warn("skipped persisted external cli auth sync because auth store is locked", { authPath });
    return {
      store: params.store,
      cacheable: false
    };
  ...
  try {
    ...
    for (const [profileId, credential] of changedProfiles) {
      ...
      if (!(0, _nodeUtil.isDeepStrictEqual)(latest, previous)) {
        _storeCacheCiDn8a.f.debug("skipped persisted external cli auth sync for concurrently changed profile", { profileId });
        continue;
      }
...
function buildLocalAuthProfileStoreForSave(params) {
  const localStore = (0, _runtimeSnapshotsCbKCG9ZW.g)(params.store);
...
async function updateAuthProfileStoreWithLock(params) {
  ...
  try {
    return await (0, _fileLockCCOJxG.o)(authPath, _storeCacheCiDn8a.i, async () => {
...
function loadAuthProfileStoreForAgent(agentDir, options) {
  ...
  if (!readOnly) {
    const cached = (0, _storeCacheCiDn8a.n)({
      authPath,
      authMtimeMs,
      stateMtimeMs
    });
  ...
  if (asStore) {
    ...
    if (!readOnly && synced.cacheable) (0, _storeCacheCiDn8a.r)({
      authPath,
  ...
  if (shouldWrite && legacy !== null) {
    const legacyPath = (0, _runtimeSnapshotsCbKCG9ZW.d)(agentDir);
    try {
      _nodeFs.default.unlinkSync(legacyPath);
    } catch (err) {
      if (err?.code !== "ENOENT") _storeCacheCiDn8a.f.warn("failed to delete legacy auth.json after migration", {
        err,
        legacyPath
      });
    }
...
function findPersistedAuthProfileCredential(params) {
  ...
  if ((0, _runtimeSnapshotsCbKCG9ZW.l)(params.agentDir) === (0, _runtimeSnapshotsCbKCG9ZW.l)()) return requestedProfile;
...
function resolvePersistedAuthProfileOwnerAgentDir(params) {
  ...
  if ((0, _runtimeSnapshotsCbKCG9ZW.l)(params.agentDir) === (0, _runtimeSnapshotsCbKCG9ZW.l)()) return;
...
function replaceRuntimeAuthProfileStoreSnapshots(entries) {
  (0, _runtimeSnapshotsCbKCG9ZW.a)(entries);
}
function clearRuntimeAuthProfileStoreSnapshots() {
  (0, _runtimeSnapshotsCbKCG9ZW.t)();
  (0, _storeCacheCiDn8a.t)();
}
function saveAuthProfileStore(store, agentDir, options) {
  ...
  (0, _storeCacheCiDn8a.r)({
    authPath,
    authMtimeMs: readAuthStoreMtimeMs(authPath),
    stateMtimeMs: readAuthStoreMtimeMs(statePath),
    store: localStore
  });
  if ((0, _runtimeSnapshotsCbKCG9ZW.i)(agentDir)) (0, _runtimeSnapshotsCbKCG9ZW.o)(localStore, agentDir);
}
//#endregion /* v9-5f13873693e82a0b */
