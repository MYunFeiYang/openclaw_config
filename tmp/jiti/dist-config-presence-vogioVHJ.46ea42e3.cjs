...
var _channelTarget9E0rxBf = require("./channel-target-9E-0rxBf.js");
...
function listBundledChannelIdsWithPersistedAuthState() {
  return (0, _packageStateProbesBcFADgAF.n)("persistedAuthState");
}
...
function listPotentialConfiguredChannelPresenceSignals(cfg, env = process.env, options = {}) {
  ...
  const channelEnvPrefixes = listChannelEnvPrefixes(options.channelIds ?? (0, _bootstrapRegistryC094XSLH.r)(env));
  const channels = (0, _utilsCKsuXgDI.c)(cfg.channels) ? cfg.channels : null;
  ...
  for (const [key, value] of Object.entries(env)) {
    if (!(0, _channelTarget9E0rxBf.i)(value)) continue;
