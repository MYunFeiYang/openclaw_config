...
var _activationContextDmNUF47b = require("./activation-context-DmNUF47b.js");
var _activationPlannerDUmDTqNr = require("./activation-planner-DUmDTqNr.js");
var _loaderCxUWY2_ = require("./loader-CxUWY2_6.js");
var _runtimeNSi6iKFR = require("./runtime-nSi6iKFR.js");
var _activeRuntimeRegistryDQEvnLxe = require("./active-runtime-registry-DQEvnLxe.js");
var _loadContextBF5zRngD = require("./load-context-BF5zRngD.js");
...
function resolveExplicitProviderOwnerPluginIds(params) {
  return dedupeSortedPluginIds(params.providerRefs.flatMap((provider) => {
    const plannedPluginIds = (0, _activationPlannerDUmDTqNr.t)({
      trigger: {
        kind: "provider",
        provider
      },
    ...
    if (apiOwnerHint) {
      const apiOwnerPluginIds = (0, _activationPlannerDUmDTqNr.t)({
        trigger: {
          kind: "provider",
      ...
      const legacyApiOwnerPluginIds = (0, _pluginAutoEnableQ8Kl1Dsn.h)({
        provider: apiOwnerHint,
        config: params.config,
        workspaceDir: params.workspaceDir,
        env: params.env,
    ...
    return (0, _pluginAutoEnableQ8Kl1Dsn.h)({
      provider,
      config: params.config,
      workspaceDir: params.workspaceDir,
      env: params.env,
...
function resolvePluginProviderLoadBase(params) {
  ...
  const workspaceDir = params.workspaceDir ?? (0, _runtimeNSi6iKFR.c)();
  ...
  const modelOwnedPluginIds = params.modelRefs?.length ? (0, _pluginAutoEnableQ8Kl1Dsn.m)({
...
function resolveSetupProviderPluginLoadState(params, base) {
  const setupPluginIds = mergeExplicitOwnerPluginIds((0, _pluginAutoEnableQ8Kl1Dsn.u)({
    config: params.config,
  ...
  const setupConfig = (0, _activationContextDmNUF47b.r)({
    config: base.rawConfig,
    pluginIds: setupPluginIds
  });
  return { loadOptions: (0, _loadContextBF5zRngD.n)({
      config: setupConfig,
      activationSourceConfig: setupConfig,
      autoEnabledReasons: {},
      workspaceDir: base.workspaceDir,
      env: base.env,
      logger: (0, _loadContextBF5zRngD.r)()
    }, {
...
function resolveRuntimeProviderPluginLoadState(params, base) {
  const explicitOwnerPluginIds = (0, _pluginAutoEnableQ8Kl1Dsn.o)({
    pluginIds: base.explicitOwnerPluginIds,
    config: base.rawConfig,
    workspaceDir: base.workspaceDir,
    env: base.env,
  ...
  const activation = (0, _activationContextDmNUF47b.t)({
    rawConfig: (0, _activationContextDmNUF47b.r)({
      config: base.rawConfig,
    ...
    resolveCompatPluginIds: (compatParams) => (0, _pluginAutoEnableQ8Kl1Dsn.s)({
      ...compatParams,
      manifestRegistry: params.pluginMetadataSnapshot?.manifestRegistry
    })
  });
  const config = params.bundledProviderVitestCompat ? (0, _pluginAutoEnableQ8Kl1Dsn.g)({
    config: activation.config,
    pluginIds: activation.compatPluginIds,
    env: base.env
  }) : activation.config;
  const providerPluginIds = mergeExplicitOwnerPluginIds((0, _pluginAutoEnableQ8Kl1Dsn.d)({
    config,
    workspaceDir: base.workspaceDir,
    env: base.env,
    onlyPluginIds: runtimeRequestedPluginIds,
...
function isPluginProvidersLoadInFlight(params) {
  ...
  return (0, _loaderCxUWY2_.s)(loadState.loadOptions);
}
function resolvePluginProviders(params) {
  ...
  if (params.mode === "setup") {
    ...
    if (!loadState) return [];
    return (0, _loaderCxUWY2_.l)(loadState.loadOptions).providers.map((entry) => Object.assign({}, entry.provider, { pluginId: entry.pluginId }));
  }
  const loadState = resolveRuntimeProviderPluginLoadState(params, base);
  const registry = loadState.loadOptions.onlyPluginIds?.length === 0 ? void 0 : (0, _activeRuntimeRegistryDQEvnLxe.n)({
    env: base.env,
    loadOptions: loadState.loadOptions,
    workspaceDir: base.workspaceDir,
    requiredPluginIds: loadState.loadOptions.onlyPluginIds
  }) ?? (0, _loaderCxUWY2_.o)(loadState.loadOptions);
  if (!registry) return [];
  return registry.providers.map((entry) => Object.assign({}, entry.provider, { pluginId: entry.pluginId }));
}
//#endregion /* v9-131660bd3da3f692 */
