"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveSecretRefValue;exports.i = resolveSecretRefString;exports.n = void 0;exports.o = resolveSecretRefValues;exports.r = isProviderScopedSecretResolutionError;exports.t = void 0;var _errorsG8SYDTCe = require("./errors-g8SYDTCe.js");
...
var _auditFsBIdj7WRc = require("./audit-fs-BIdj7WRc.js");
var _scanPathsCdBrZLbm = require("./scan-paths-CdBrZLbm.js");
var _runWithConcurrencyB6E2CnAh = require("./run-with-concurrency-B6E2CnAh.js");
var _jsonPointerCUDVrnJq = require("./json-pointer-CUDVrnJq.js");
...
function throwUnknownProviderResolutionError(params) {
  ...
  throw providerResolutionError({
    ...
    message: (0, _errorsG8SYDTCe.i)(params.err),
    cause: params.err
  });
}
async function readFileStatOrThrow(pathname, label) {
  const stat = await (0, _auditFsBIdj7WRc.u)(pathname);
...
function resolveResolutionLimits(config) {
  ...
  return {
    maxProviderConcurrency: (0, _sharedC9ga15VD.r)(resolution?.maxProviderConcurrency, DEFAULT_PROVIDER_CONCURRENCY),
    maxRefsPerProvider: (0, _sharedC9ga15VD.r)(resolution?.maxRefsPerProvider, DEFAULT_MAX_REFS_PER_PROVIDER),
    maxBatchBytes: (0, _sharedC9ga15VD.r)(resolution?.maxBatchBytes, DEFAULT_MAX_BATCH_BYTES)
  };
}
...
function resolveConfiguredProvider(ref, config) {
  ...
  if (!providerConfig) {
    if (ref.source === "env" && ref.provider === (0, _refContractDtvCiPbj.l)(config, "env")) return { source: "env" };
    throw providerResolutionError({
      source: ref.source,
...
async function assertSecurePath(params) {
  ...
  if (params.trustedDirs && params.trustedDirs.length > 0) {
    if (!params.trustedDirs.map((entry) => (0, _utilsDvkbxKCZ.p)(entry)).some((dir) => (0, _scanPathsCdBrZLbm.n)(dir, effectivePath))) throw new Error(`${params.label} is outside trustedDirs: ${effectivePath}`);
  }
  if (params.allowInsecurePath) return effectivePath;
  const perms = await (0, _auditFsBIdj7WRc.i)(effectivePath);
...
async function readFileProviderPayload(params) {
  ...
  const filePath = (0, _utilsDvkbxKCZ.p)(params.providerConfig.path);
  const readPromise = (async () => {
    ...
    const timeoutMs = (0, _sharedC9ga15VD.r)(params.providerConfig.timeoutMs, DEFAULT_FILE_TIMEOUT_MS);
    const maxBytes = (0, _sharedC9ga15VD.r)(params.providerConfig.maxBytes, DEFAULT_FILE_MAX_BYTES);
    ...
    try {
      const payload = await Promise.race([_promises.default.readFile(secureFilePath, { signal: abortController.signal }), timeoutPromise]);
      ...
      if (!(0, _utilsDvkbxKCZ.c)(parsed)) throw new Error(`File provider "${params.providerName}" payload is not a JSON object.`);
      return parsed;
    } catch (error) {
...
async function resolveEnvRefs(params) {
  ...
  for (const ref of params.refs) {
    ...
    if (!(0, _sharedC9ga15VD.n)(envValue)) throw refResolutionError({
      source: "env",
      provider: params.providerName,
      refId: ref.id,
...
async function resolveFileRefs(params) {
  ...
  if (mode === "singleValue") {
    for (const ref of params.refs) {
      if (ref.id !== "value") throw refResolutionError({
        ...
        provider: params.providerName,
        refId: ref.id,
        message: `singleValue file provider "${params.providerName}" expects ref id "${_refContractDtvCiPbj.i}".`
      });
  ...
  for (const ref of params.refs) try {
    resolved.set(ref.id, (0, _jsonPointerCUDVrnJq.n)(payload, ref.id, { onMissing: "throw" }));
  } catch (err) {
    throw refResolutionError({
      source: "file",
      provider: params.providerName,
      refId: ref.id,
      message: (0, _errorsG8SYDTCe.i)(err),
      cause: err
    });
  }
  return resolved;
...
function parseExecValues(params) {
  ...
  if (!(0, _utilsDvkbxKCZ.c)(responseValues)) throw providerResolutionError({
    source: "exec",
    provider: params.providerName,
    message: `Exec provider "${params.providerName}" response missing "values".`
  });
  const responseErrors = (0, _utilsDvkbxKCZ.c)(parsed.errors) ? parsed.errors : null;
  const out = {};
  for (const id of params.ids) {
    if (responseErrors && id in responseErrors) {
      const entry = responseErrors[id];
      if ((0, _utilsDvkbxKCZ.c)(entry) && typeof entry.message === "string" && entry.message.trim()) throw refResolutionError({
        source: "exec",
        provider: params.providerName,
        refId: id,
...
async function resolveExecRefs(params) {
  ...
  const commandPath = (0, _utilsDvkbxKCZ.p)(params.providerConfig.command);
  ...
  const timeoutMs = (0, _sharedC9ga15VD.r)(params.providerConfig.timeoutMs, DEFAULT_EXEC_TIMEOUT_MS);
  const noOutputTimeoutMs = (0, _sharedC9ga15VD.r)(params.providerConfig.noOutputTimeoutMs, timeoutMs);
  const maxOutputBytes = (0, _sharedC9ga15VD.r)(params.providerConfig.maxOutputBytes, DEFAULT_EXEC_MAX_OUTPUT_BYTES);
...
async function resolveSecretRefValues(refs, options) {
  ...
  for (const ref of refs) {
    ...
    if (ref.source === "exec" && !(0, _refContractDtvCiPbj.o)(id)) throw new Error(`${(0, _refContractDtvCiPbj.a)()} (ref: ${ref.source}:${ref.provider}:${id}).`);
    uniqueRefs.set((0, _refContractDtvCiPbj.u)(ref), {
      ...ref,
      id
    });
  }
  ...
  const taskResults = await (0, _runWithConcurrencyB6E2CnAh.t)({
  ...
  for (const result of taskResults.results) for (const ref of result.group.refs) {
    ...
    resolved.set((0, _refContractDtvCiPbj.u)(ref), result.values.get(ref.id));
  }
  return resolved;
}
async function resolveSecretRefValue(ref, options) {
  const cache = options.cache;
  const key = (0, _refContractDtvCiPbj.u)(ref);
...
async function resolveSecretRefString(ref, options) {
  ...
  if (!(0, _sharedC9ga15VD.n)(resolved)) throw new Error(`Secret reference "${ref.source}:${ref.provider}:${ref.id}" resolved to a non-string or empty value.`);
  return resolved;
}
//#endregion /* v9-3cc3d360d3bb239b */
