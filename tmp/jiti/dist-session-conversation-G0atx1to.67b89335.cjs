...
var _facadeRuntimeDfDvVKPW = require("./facade-runtime-DfDvVKPW.js");
...
function normalizeResolvedChannel(channel) {
  return (0, _registryOCFvwkK.a)(channel) ?? (0, _registryOM2UOqu.o)(channel) ?? (0, _stringCoerceLndEvhRk.s)(channel) ?? "";
}
function getMessagingAdapter(channel) {
  const normalizedChannel = normalizeResolvedChannel(channel);
  try {
    return (0, _registryOCFvwkK.n)(normalizedChannel)?.messaging;
  } catch {
    return;
  }
}
...
function resolveBundledSessionConversationFallback(params) {
  ...
  let loaded = null;
  try {
    loaded = (0, _facadeRuntimeDfDvVKPW.a)({
      dirName,
...
function isBundledSessionConversationFallbackDisabled(channel) {
  const snapshot = (0, _runtimeSnapshotBCdniHcd.i)();
