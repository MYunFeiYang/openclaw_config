"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveEffectiveSessionToolsVisibility;exports.c = void 0;exports.i = listSpawnedSessionKeys;exports.n = createSessionVisibilityChecker;exports.o = resolveSandboxSessionToolsVisibility;exports.r = createSessionVisibilityGuard;exports.s = resolveSessionToolsVisibility;exports.t = createAgentToAgentPolicy;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
let callGatewayForListSpawned = _callXuTtjcDL.r;
/** Test hook: must stay aligned with `sessions-resolution` `__testing.setDepsForTest`. */
const sessionVisibilityGatewayTesting = exports.c = { setCallGatewayForListSpawned(overrides) {
    callGatewayForListSpawned = overrides ?? _callXuTtjcDL.r;
  } };
async function listSpawnedSessionKeys(params) {
  ...
  try {
    ...
    const keys = (Array.isArray(list?.sessions) ? list.sessions : []).map((entry) => (0, _stringCoerceBje8XVt.c)(entry?.key) ?? "").filter(Boolean);
    return new Set(keys);
  } catch {
...
function resolveSessionToolsVisibility(cfg) {
  ...
  const value = (0, _stringCoerceBje8XVt.a)(raw);
...
function createAgentToAgentPolicy(cfg) {
  ...
  const matchesAllow = (agentId) => {
    ...
    return allowPatterns.some((pattern) => {
      const raw = (0, _stringCoerceBje8XVt.c)(typeof pattern === "string" ? pattern : String(pattern ?? "")) ?? "";
      if (!raw) return false;
      if (raw === "*") return true;
...
function createSessionVisibilityChecker(params) {
  const requesterAgentId = (0, _sessionKeyC6FNnIG.u)(params.requesterSessionKey);
  const spawnedKeys = params.spawnedKeys;
  const check = (targetSessionKey) => {
    const targetAgentId = (0, _sessionKeyC6FNnIG.u)(targetSessionKey);
