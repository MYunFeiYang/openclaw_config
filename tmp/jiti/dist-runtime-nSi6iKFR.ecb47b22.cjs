...
const log$1 = (0, _subsystemLmgZvqF.t)("plugins/host-hooks");
...
const log = (0, _subsystemLmgZvqF.t)("plugins/runtime");
...
async function cleanupPreviousPluginHostRegistry(params) {
  const [{ getRuntimeConfig }, { cleanupReplacedPluginHostRegistry }] = await Promise.all([Promise.resolve().then(() => jitiImport("./config/config.js").then((m) => _interopRequireWildcard(m))), Promise.resolve().then(() => jitiImport("./host-hook-cleanup-DDaHCQqW.js").then((m) => _interopRequireWildcard(m)))]);
