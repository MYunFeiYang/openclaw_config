...
var _consoleDqjTIgfM = require("./console-DqjTIgfM.js");
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _loggerApTu_tEr._)(level) >= (0, _loggerApTu_tEr._)(settings.level);
}
const inspectValue = (() => {
...
function formatConsoleLine(opts) {
  ...
  if (opts.style === "json") return (0, _redactCxqQQvmK.s)(JSON.stringify({
    time: (0, _consoleDqjTIgfM.n)("json"),
    level: opts.level,
  ...
  return `${[(() => {
    if (opts.style === "pretty") return color.gray((0, _consoleDqjTIgfM.n)("pretty"));
    if (_stateCiZRzmxP.t.consoleTimestampPrefix) return color.gray((0, _consoleDqjTIgfM.n)(opts.style));
    return "";
...
function shouldSuppressProbeConsoleLine(params) {
  if ((0, _loggerApTu_tEr.b)()) return false;
...
function createSubsystemLogger(subsystem) {
  ...
  const emitLog = (level, message, meta) => {
    const consoleSettings = (0, _consoleDqjTIgfM.r)();
    const consoleEnabled = shouldLogToConsole(level, { level: consoleSettings.level }) && (0, _consoleDqjTIgfM.l)(resolvedSubsystem);
    const fileEnabled = (0, _loggerApTu_tEr.s)(level);
    ...
    if (fileEnabled) logToFile((0, _loggerApTu_tEr.i)({ subsystem: resolvedSubsystem }), level, message, fileMeta);
  ...
  return {
    ...
    isEnabled(level, target = "any") {
      const isConsoleEnabled = shouldLogToConsole(level, { level: (0, _consoleDqjTIgfM.r)().level }) && (0, _consoleDqjTIgfM.l)(resolvedSubsystem);
      const isFileEnabled = (0, _loggerApTu_tEr.s)(level);
    ...
    raw(message) {
      if ((0, _loggerApTu_tEr.s)("info")) logToFile((0, _loggerApTu_tEr.i)({ subsystem: resolvedSubsystem }), "info", message, { raw: true });
      if (shouldLogToConsole("info", { level: (0, _consoleDqjTIgfM.r)().level }) && (0, _consoleDqjTIgfM.l)(resolvedSubsystem)) {
        if (shouldSuppressProbeConsoleLine({
          level: "info",
          subsystem: resolvedSubsystem,
          message
