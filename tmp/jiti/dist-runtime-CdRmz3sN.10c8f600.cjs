"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = isDebugProxyGlobalFetchPatchInstalled;exports.c = closeDebugProxyCaptureStore;exports.i = initializeDebugProxyCapture;exports.l = getDebugProxyCaptureStore;exports.n = captureWsEvent;exports.o = void 0;exports.r = finalizeDebugProxyCapture;exports.s = acquireDebugProxyCaptureStore;exports.t = captureHttpExchange;var _sqliteWalDMug4B7X = require("./sqlite-wal-DMug4B7X.js");
var _envCDFM4b5F = require("./env-CDFM4b5F.js");
var _fetchHeadersCGp03wKO = require("./fetch-headers-CGp03wKO.js");
var _nodeUrl = require("node:url");
var _nodeFs = _interopRequireDefault(require("node:fs"));
var _nodePath = _interopRequireDefault(require("node:path"));
var _nodeCrypto = require("node:crypto");
var _nodeZlib = require("node:zlib");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
function writeCaptureBlob(params) {
  ...
  const sha256 = (0, _nodeCrypto.createHash)("sha256").update(params.data).digest("hex");
  const blobId = sha256.slice(0, 24);
  const outputPath = _nodePath.default.join(params.blobDir, `${blobId}.bin.gz`);
  if (!_nodeFs.default.existsSync(outputPath)) _nodeFs.default.writeFileSync(outputPath, (0, _nodeZlib.gzipSync)(params.data));
  return {
    blobId,
    path: outputPath,
    encoding: "gzip",
...
function readCaptureBlobText(blobPath) {
  return (0, _nodeZlib.gunzipSync)(_nodeFs.default.readFileSync(blobPath)).toString("utf8");
}
//#endregion
...
function openDatabase(dbPath) {
  ...
  const { DatabaseSync } = (0, _sqliteWalDMug4B7X.n)();
  const db = new DatabaseSync(dbPath);
  const walMaintenance = (0, _sqliteWalDMug4B7X.t)(db);
...
var DebugProxyCaptureStore = class {
  ...
  readBlob(blobId) {
    ...
    const blobPath = _nodePath.default.join(this.blobDir, `${row.blobId}.bin.gz`);
    return _nodeFs.default.existsSync(blobPath) ? readCaptureBlobText(blobPath) : null;
  }
  ...
  purgeAll() {
    ...
    let blobs = 0;
    if (_nodeFs.default.existsSync(this.blobDir)) for (const entry of _nodeFs.default.readdirSync(this.blobDir)) {
      _nodeFs.default.rmSync(_nodePath.default.join(this.blobDir, entry), { force: true });
      blobs += 1;
    }
    return {
  ...
  deleteSessions(sessionIds) {
    ...
    for (const row of blobRows) {
      ...
      const blobPath = _nodePath.default.join(this.blobDir, `${blobId}.bin.gz`);
      if (_nodeFs.default.existsSync(blobPath)) {
        _nodeFs.default.rmSync(blobPath, { force: true });
        blobs += 1;
      }
    }
    return {
...
};exports.o = DebugProxyCaptureStore;
...
function protocolFromUrl(rawUrl) {
  try {
    switch (new _nodeUrl.URL(rawUrl).protocol) {
...
function resolveUrlString(input) {
  if (input instanceof _nodeUrl.URL) return input.toString();
...
function installDebugProxyGlobalFetchPatch(settings, deps = {}) {
  ...
  fetchTarget.fetch = async (input, init) => {
    ...
    const normalizedInit = (0, _fetchHeadersCGp03wKO.n)(init);
    try {
      ...
      if (url && /^https?:/i.test(url)) {
        ...
        const parsed = new _nodeUrl.URL(url);
        store.recordEvent({
...
function initializeDebugProxyCapture(mode, resolved, deps = {}) {
  const settings = resolved ?? (0, _envCDFM4b5F.r)();
...
function captureHttpExchange(params, resolved, deps = {}) {
  ...
  const runtime = resolveRuntimeDeps(deps);
  const store = runtime.getStore(settings.dbPath, settings.blobDir);
  const flowId = params.flowId ?? (0, _nodeCrypto.randomUUID)();
  const url = new _nodeUrl.URL(params.url);
...
function captureWsEvent(params) {
  const settings = (0, _envCDFM4b5F.r)();
  if (!settings.enabled) return;
