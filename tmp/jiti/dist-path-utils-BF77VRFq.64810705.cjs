"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = setPathExistingStrict;exports.n = getPath;exports.r = setPathCreateStrict;exports.t = deletePathStrict;var _utilsDvkbxKCZ = require("./utils-DvkbxKCZ.js");
...
function traverseToLeafParent(params) {
  ...
  for (let index = 0; index < params.segments.length - 1; index += 1) {
    ...
    if (!(0, _utilsDvkbxKCZ.c)(cursor)) throw new Error(`Invalid path shape at ${params.segments.slice(0, index).join(".") || "<root>"}.`);
...
function getPath(root, segments) {
  ...
  for (const segment of segments) {
    ...
    if (!(0, _utilsDvkbxKCZ.c)(cursor)) return;
    cursor = cursor[segment];
  }
  return cursor;
}
function setPathCreateStrict(root, segments, value) {
  ...
  for (let index = 0; index < segments.length - 1; index += 1) {
    ...
    if (Array.isArray(cursor)) {
      ...
      if (existing === void 0 || existing === null) {
        ...
        changed = true;
      } else if (needs === "array" ? !Array.isArray(existing) : !(0, _utilsDvkbxKCZ.c)(existing)) throw new Error(`Invalid path shape at ${segments.slice(0, index + 1).join(".")}.`);
      cursor = cursor[arrayIndex];
      continue;
    }
    if (!(0, _utilsDvkbxKCZ.c)(cursor)) throw new Error(`Invalid path shape at ${segments.slice(0, index).join(".") || "<root>"}.`);
  ...
  if (arrayTarget) {
    if (!(0, _nodeUtil.isDeepStrictEqual)(arrayTarget.array[arrayTarget.index], value)) {
      arrayTarget.array[arrayTarget.index] = value;
      changed = true;
    }
    return changed;
  }
  if (!(0, _utilsDvkbxKCZ.c)(cursor)) throw new Error(`Invalid path shape at ${segments.slice(0, -1).join(".") || "<root>"}.`);
  if (!(0, _nodeUtil.isDeepStrictEqual)(cursor[leaf], value)) {
    cursor[leaf] = value;
    changed = true;
  }
  return changed;
