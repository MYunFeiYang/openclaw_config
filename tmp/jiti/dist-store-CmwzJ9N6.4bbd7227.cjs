...
function deriveGroupSessionPatch(params) {
  ...
  const normalizedChannel = subjectLooksChannel && resolution.chatType !== "channel" ? (0, _registryOCFvwkK.a)(channel) : null;
  const isChannelProvider = Boolean(normalizedChannel && (0, _registryOCFvwkK.n)(normalizedChannel)?.capabilities.chatTypes.includes("channel"));
...
const log = (0, _subsystemLmgZvqF.t)("sessions/store");
...
function loadTrajectoryCleanupRuntime() {
  trajectoryCleanupRuntimePromise ??= Promise.resolve().then(() => jitiImport("./cleanup-DnX0TMkW.js").then((m) => _interopRequireWildcard(m)));
...
function readSessionUpdatedAt(params) {
  try {
    return (0, _storeLoadDjaY7SAt.A)({
      store: (0, _storeLoadDjaY7SAt.t)(params.storePath),
...
function updateSessionStoreWriteCaches(params) {
  const fileStat = (0, _storeLoadDjaY7SAt.E)(params.storePath);
  (0, _storeLoadDjaY7SAt.f)(params.storePath, params.serialized);
  if (!(0, _storeLoadDjaY7SAt.d)()) {
    (0, _storeLoadDjaY7SAt.l)(params.storePath);
    return;
  }
  (0, _storeLoadDjaY7SAt.m)({
...
function loadMutableSessionStoreForWriter(storePath) {
  if ((0, _storeLoadDjaY7SAt.d)()) {
    const currentFileStat = (0, _storeLoadDjaY7SAt.E)(storePath);
    const cached = (0, _storeLoadDjaY7SAt.p)({
      storePath,
  ...
  return (0, _storeLoadDjaY7SAt.t)(storePath, {
    skipCache: true,
    clone: false
  });
}
function resolveMutableSessionStoreKey(store, sessionKey) {
  ...
  const normalized = (0, _storeLoadDjaY7SAt.k)(trimmed);
  if (Object.prototype.hasOwnProperty.call(store, normalized)) return normalized;
  return Object.keys(store).find((key) => (0, _storeLoadDjaY7SAt.k)(key) === normalized);
}
...
function preserveExistingAcpMetadata(params) {
  const allowDrop = new Set((params.allowDropSessionKeys ?? []).map((key) => (0, _storeLoadDjaY7SAt.k)(key)));
  for (const [previousKey, previousAcp] of params.previousAcpByKey.entries()) {
    const normalizedKey = (0, _storeLoadDjaY7SAt.k)(previousKey);
...
async function saveSessionStoreUnlocked(storePath, store, opts) {
  ...
  if (!opts?.skipMaintenance) {
    ...
    if (shouldWarnOnly) {
      ...
      if (activeSessionKey && shouldRunEntryMaintenance) {
        const warning = (0, _storeLoadDjaY7SAt.g)({
          store,
          activeSessionKey,
      ...
      const preserveSessionKeys = (0, _storeLoadDjaY7SAt.i)([opts?.activeSessionKey]);
      const removedSessionFiles = /* @__PURE__ */new Map();
      const pruned = (0, _storeLoadDjaY7SAt.v)(store, maintenance.pruneAfterMs, {
        onPruned: ({ entry }) => {
      ...
      const capped = forceMaintenance || (0, _storeLoadDjaY7SAt.x)({
        entryCount: countAfterPrune,
        maxEntries: maintenance.maxEntries
      }) ? (0, _storeLoadDjaY7SAt.h)(store, maintenance.maxEntries, {
        onCapped: ({ entry }) => {
          rememberRemovedSessionFile(removedSessionFiles, entry);
        },
        preserveKeys: preserveSessionKeys
  ...
  if ((0, _storeLoadDjaY7SAt.u)(storePath) === json) {
...
async function runQuotaSuspensionMaintenance(params) {
  ...
  return await updateSessionStore(params.storePath, (store) => (0, _storeLoadDjaY7SAt._)({
    store,
...
async function updateSessionStoreEntry(params) {
  ...
  return await runExclusiveSessionStoreWrite(storePath, async () => {
    const store = loadMutableSessionStoreForWriter(storePath);
    const resolved = (0, _storeLoadDjaY7SAt.A)({
      store,
      sessionKey
    });
...
async function recordSessionMetaFromInbound(params) {
  ...
  return await updateSessionStore(storePath, (store) => {
    ...
    for (const legacyKey of resolved.legacyKeys) delete store[legacyKey];
    return next;
  }, { activeSessionKey: (0, _storeLoadDjaY7SAt.k)(sessionKey) });
}
async function updateLastRoute(params) {
  ...
  return await runExclusiveSessionStoreWrite(storePath, async () => {
    ...
    const existing = resolved.existing;
    if (!existing && !createIfMissing) return null;
    const explicitContext = (0, _deliveryContextSharedBuB7h14N.i)(params.deliveryContext);
    const inlineContext = (0, _deliveryContextSharedBuB7h14N.i)({
      channel,
      to,
      accountId,
      threadId
    });
    const mergedInput = (0, _deliveryContextSharedBuB7h14N.r)(explicitContext, inlineContext);
    ...
    const merged = (0, _deliveryContextSharedBuB7h14N.r)(mergedInput, Boolean(explicitContext?.channel || explicitContext?.to || inlineContext?.channel || inlineContext?.to) && explicitThreadValue == null ? removeThreadFromDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(existing)) : (0, _deliveryContextSharedBuB7h14N.t)(existing));
    const normalized = (0, _deliveryContextSharedBuB7h14N.a)({ deliveryContext: {
