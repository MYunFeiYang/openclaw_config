"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = listBuiltinAutoSelectMemoryEmbeddingProviderDoctorMetadata;exports.i = getBuiltinMemoryEmbeddingProviderDoctorMetadata;exports.n = closeAllMemoryIndexManagers;exports.o = registerBuiltInMemoryEmbeddingProviders;exports.r = createEmbeddingProvider;exports.t = void 0;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
var _providerEnvVarsDOy0Czuc = require("./provider-env-vars-DOy0Czuc.js");
var _pathsBg_QeV2r = require("./paths-Bg_QeV2r.js");
var _transcriptD_7jiGc = require("./transcript-D-_7jiGc.js");
var _queryExpansionCic3aAyO = require("./query-expansion-Cic3aAyO.js");
...
var _memoryEmbeddingProviderRuntime_vVOf3Nn = require("./memory-embedding-provider-runtime-_vVOf3Nn.js");
var _memoryCoreHostEngineEmbeddingsBDFBUqKJ = require("./memory-core-host-engine-embeddings-BDFBUqKJ.js");
...
var _engineStorageCO3qFXo = require("./engine-storage-CO3qFXo7.js");
require("./memory-core-host-engine-storage-CvdPsuqZ.js");
require("./provider-env-vars-C_HQxyPg.js");
...
function listRemoteEmbeddingSetupHints() {
  try {
    return (0, _memoryEmbeddingProviderRuntime_vVOf3Nn.n)().filter((adapter) => adapter.transport === "remote" && typeof adapter.autoSelectPriority === "number").toSorted((a, b) => (a.autoSelectPriority ?? 0) - (b.autoSelectPriority ?? 0)).map((adapter) => `Or set agents.defaults.memorySearch.provider = "${adapter.id}" (remote).`);
  } catch {
    return [];
  }
}
function formatLocalSetupError(err) {
  const detail = (0, _errorsG8SYDTCe.i)(err);
...
function canAutoSelectLocal(modelPath) {
  ...
  const resolved = (0, _utilsDvkbxKCZ.p)(trimmed);
  try {
    return _nodeFs.default.statSync(resolved).isFile();
  } catch {
    return false;
  }
}
const builtinMemoryEmbeddingProviderAdapters = [{
  id: "local",
  defaultModel: _memoryCoreHostEngineEmbeddingsBDFBUqKJ.O,
  transport: "local",
  autoSelectPriority: 10,
  formatSetupError: formatLocalSetupError,
  shouldContinueAutoSelection: () => true,
  create: async (options) => {
    const provider = await (0, _memoryCoreHostEngineEmbeddingsBDFBUqKJ.E)({
      ...options,
      provider: "local",
      fallback: "none"
    });
...
function getBuiltinMemoryEmbeddingProviderAdapter(id) {
  return (0, _memoryEmbeddingProviderRuntime_vVOf3Nn.n)().find((adapter) => adapter.id === id);
}
function registerBuiltInMemoryEmbeddingProviders(register) {
  for (const adapter of filterUnregisteredMemoryEmbeddingProviderAdapters({
    ...
    registeredAdapters: (0, _memoryEmbeddingProviderRuntime_vVOf3Nn.r)()
...
function getBuiltinMemoryEmbeddingProviderDoctorMetadata(providerId) {
  ...
  return {
    ...
    authProviderId,
    envVars: (0, _providerEnvVarsDOy0Czuc.n)(authProviderId),
...
function listBuiltinAutoSelectMemoryEmbeddingProviderDoctorMetadata() {
  return (0, _memoryEmbeddingProviderRuntime_vVOf3Nn.n)().filter((adapter) => typeof adapter.autoSelectPriority === "number").toSorted((a, b) => (a.autoSelectPriority ?? 0) - (b.autoSelectPriority ?? 0)).map((adapter) => {
    const authProviderId = adapter.authProviderId ?? adapter.id;
    return {
      providerId: adapter.id,
      authProviderId,
...
function formatProviderError(adapter, err) {
  return adapter.formatSetupError?.(err) ?? (0, _errorsG8SYDTCe.i)(err);
}
...
function resolveEmbeddingProviderFallbackModel(providerId, fallbackSourceModel, config) {
  return (0, _memoryEmbeddingProviderRuntime_vVOf3Nn.t)(providerId, config)?.defaultModel ?? fallbackSourceModel;
}
...
function tokenize(text) {
  const lower = (0, _stringCoerceBje8XVt.a)(text);
...
async function extractTimestamp(params) {
  ...
  const absolutePath = _nodePath.default.isAbsolute(params.filePath) ? params.filePath : _nodePath.default.resolve(params.workspaceDir, params.filePath);
  try {
    const stat = await _promises.default.stat(absolutePath);
...
function resolveSingletonManagedCache(cacheKey) {
  const resolved = (0, _globalSingletonCOlWgaGc.n)(cacheKey, () => ({
...
function openMemoryDatabaseAtPath(dbPath, allowExtension) {
  (0, _internalBhQAXls.a)(_nodePath.default.dirname(dbPath));
  const { DatabaseSync } = (0, _engineStorageCO3qFXo.r)();
  const db = new DatabaseSync(dbPath, { allowExtension });
  (0, _engineStorageCO3qFXo.n)(db);
  db.exec("PRAGMA busy_timeout = 5000");
  return db;
}
function closeMemoryDatabase(db) {
  (0, _engineStorageCO3qFXo.t)(db);
  db.close();
}
//#endregion
...
function loadMemoryEmbeddingCache(params) {
  ...
  for (let start = 0; start < unique.length; start += batchSize) {
    ...
    for (const row of rows) out.set(row.hash, (0, _internalBhQAXls.l)(row.embedding));
  }
  return out;
}
...
function collectMemoryCachedEmbeddings(params) {
  ...
  for (let index = 0; index < params.chunks.length; index += 1) {
    ...
    if (hit && hit.length > 0) embeddings[index] = hit;else
    if (chunk) missing.push({
      index,
      chunk
    });
  }
...
function estimateStructuredEmbeddingInputBytes(input) {
  ...
  let total = 0;
  for (const part of input.parts) if (part.type === "text") total += estimateUtf8Bytes(part.text);else
  {
...
async function moveMemoryIndexFiles(sourceBase, targetBase) {
  ...
  "-shm"])
  {
    ...
    try {
      await _promises.default.rename(source, target);
    } catch (err) {
...
async function removeMemoryIndexFiles(basePath) {
  ...
  "-wal",
  "-shm"].
  map((suffix) => _promises.default.rm(`${basePath}${suffix}`, { force: true })));
}
async function swapMemoryIndexFiles(targetPath, tempPath) {
  const backupPath = `${targetPath}.backup-${(0, _nodeCrypto.randomUUID)()}`;
...
function resolveConfiguredScopeHash(params) {
  const extraPaths = (0, _internalBhQAXls.c)(params.workspaceDir, params.extraPaths).map((value) => value.replace(/\\/g, "/")).toSorted();
  return (0, _hashB7y7CIO.t)(JSON.stringify({
    extraPaths,
    multimodal: {
...
function clearMemorySyncedSessionFiles(params) {
  if (!params.targetSessionFiles) params.sessionsDirtyFiles.clear();else
  for (const targetSessionFile of params.targetSessionFiles) params.sessionsDirtyFiles.delete(targetSessionFile);
...
async function runMemoryTargetedSessionSync(params) {
  ...
  try {
    ...
    const reason = (0, _errorsG8SYDTCe.i)(err);
    ...
    if (params.useUnsafeReindex) await params.runUnsafeReindex(reindexParams);else
...
const log$3 = (0, _subsystemIzrDYZz.t)("memory");
...
function resolveMemoryWatchFactory() {
  ...
  return _chokidar.default.watch.bind(_chokidar.default);
}
function shouldIgnoreMemoryWatchPath(watchPath, stats, multimodalSettings) {
  const normalized = _nodePath.default.normalize(watchPath);
  if (normalized.split(_nodePath.default.sep).map((segment) => (0, _stringCoerceBje8XVt.a)(segment)).some((segment) => IGNORED_MEMORY_WATCH_DIR_NAMES.has(segment))) return true;
  if (stats?.isDirectory?.()) return false;
  if (!stats) return false;
  const extension = (0, _stringCoerceBje8XVt.a)(_nodePath.default.extname(normalized));
  if (extension.length === 0 || extension === ".md") return false;
  if (!multimodalSettings) return true;
  return (0, _multimodalBR2nwsNX.r)(normalized, multimodalSettings) === null;
}
...
var MemoryManagerSyncOps = class {
  ...
  async loadVectorExtension() {
    ...
    try {
      const resolvedPath = this.vector.extensionPath?.trim() ? (0, _utilsDvkbxKCZ.p)(this.vector.extensionPath) : void 0;
      const loaded = await (0, _engineStorageCO3qFXo.i)({
        db: this.db,
  ...
  openDatabase() {
    return openMemoryDatabaseAtPath((0, _utilsDvkbxKCZ.p)(this.settings.store.path), this.settings.store.vector.enabled);
  }
  ...
  ensureSchema() {
    const result = (0, _engineStorageCO3qFXo.a)({
      db: this.db,
  ...
  ensureWatcher() {
    ...
    const watchPaths = new Set([_nodePath.default.join(this.workspaceDir, "MEMORY.md"), _nodePath.default.join(this.workspaceDir, "memory")]);
    const additionalPaths = (0, _internalBhQAXls.c)(this.workspaceDir, this.settings.extraPaths);
    for (const entry of additionalPaths) try {
      const stat = _nodeFs.default.lstatSync(entry);
      if (stat.isSymbolicLink()) continue;
      if (stat.isDirectory()) {
        watchPaths.add(entry);
        continue;
      }
      if (stat.isFile() && ((0, _stringCoerceBje8XVt.a)(entry).endsWith(".md") || (0, _multimodalBR2nwsNX.r)(entry, this.settings.multimodal) !== null)) watchPaths.add(entry);
    } catch {}
  ...
  ensureSessionListener() {
    ...
    this.sessionUnsubscribe = (0, _transcriptD_7jiGc.u)((update) => {
  ...
  async updateSessionDelta(sessionFile) {
    ...
    let stat;
    try {
      stat = await _promises.default.stat(sessionFile);
    } catch {
      return null;
    }
  ...
  async countNewlines(absPath, start, end) {
    ...
    let handle;
    try {
      handle = await _promises.default.open(absPath, "r");
    } catch (err) {
      if ((0, _internalBhQAXls.f)(err)) return 0;
      throw err;
    }
    try {
      let offset = start;
  ...
  isSessionFileForAgent(sessionFile) {
    ...
    const sessionsDir = (0, _pathsBg_QeV2r.l)(this.agentId);
    const resolvedFile = _nodePath.default.resolve(sessionFile);
    const resolvedDir = _nodePath.default.resolve(sessionsDir);
    return resolvedFile.startsWith(`${resolvedDir}${_nodePath.default.sep}`);
  }
  ...
  async syncMemoryFiles(params) {
    ...
    const fileEntries = (await (0, _internalBhQAXls.d)((await (0, _internalBhQAXls.s)(this.workspaceDir, this.settings.extraPaths, this.settings.multimodal)).map((file) => async () => await (0, _internalBhQAXls.t)(file, this.workspaceDir, this.settings.multimodal)), this.getIndexConcurrency())).filter((entry) => entry !== null);
    ...
    await (0, _internalBhQAXls.d)(fileEntries.map((entry) => async () => {
  ...
  async syncSessionFiles(params) {
    ...
    const files = targetSessionFiles ? Array.from(targetSessionFiles) : await (0, _engineQmdBLLUNP_T.l)(this.agentId);
    const { activePaths, existingRows, existingHashes, indexAll } = resolveMemorySessionSyncPlan({
      ...
      sessionPathForFile: _engineQmdBLLUNP_T.p
    });
    ...
    await (0, _internalBhQAXls.d)(files.map((absPath) => async () => {
      ...
      const entry = await (0, _engineQmdBLLUNP_T.c)(absPath);
      if (!entry) {
        if (params.progress) {
          params.progress.completed += 1;
          params.progress.report({
  ...
  async runSync(params) {
    ...
    try {
      ...
      } else if (this.sessionsDirtyFiles.size > 0) this.sessionsDirty = true;else
      this.sessionsDirty = false;
    } catch (err) {
      const reason = (0, _errorsG8SYDTCe.i)(err);
  ...
  async activateFallbackProvider(reason) {
    ...
    const fallbackResult = await createEmbeddingProvider({
      config: this.cfg,
      agentDir: (0, _agentScopeRNt6KatQ.b)(this.cfg, this.agentId),
      ...fallbackRequest
    });
  ...
  async runSafeReindex(params) {
    const dbPath = (0, _utilsDvkbxKCZ.p)(this.settings.store.path);
    const tempDbPath = `${dbPath}.tmp-${(0, _nodeCrypto.randomUUID)()}`;
    ...
    const restoreOriginalState = () => {
      if (originalDbClosed) this.db = openMemoryDatabaseAtPath(dbPath, this.settings.store.vector.enabled);else
      this.db = originalDb;
...
const log$2 = (0, _subsystemIzrDYZz.t)("memory");
...
var MemoryManagerEmbeddingOps = class extends MemoryManagerSyncOps {
  ...
  async embedChunksInBatches(chunks) {
    ...
    for (const batch of batches) {
      ...
      const hasStructuredInputs = inputs.some((input) => (0, _internalBhQAXls.m)(input));
  ...
  computeProviderKey() {
    if (!this.provider) return (0, _hashB7y7CIO.t)(JSON.stringify({
      provider: "none",
      model: "fts-only"
    }));
    if (this.providerRuntime?.cacheKeyData) return (0, _hashB7y7CIO.t)(JSON.stringify(this.providerRuntime.cacheKeyData));
    return (0, _hashB7y7CIO.t)(JSON.stringify({
  ...
  */
  writeChunks(entry, source, model, chunks, embeddings, vectorReady) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const id = (0, _hashB7y7CIO.t)(`${source}:${entry.path}:${chunk.startLine}:${chunk.endLine}:${chunk.hash}:${model}`);
  ...
  async indexFile(entry, options) {
    if (!this.provider) {
      ...
      const chunks = filterNonEmptyMemoryChunks((0, _internalBhQAXls.r)(options.content ?? (await _promises.default.readFile(entry.absPath, "utf-8")), this.settings.chunking));
      if (options.source === "sessions" && "lineMap" in entry) (0, _internalBhQAXls.u)(chunks, entry.lineMap);
    ...
    if ("kind" in entry && entry.kind === "multimodal") {
      ...
      const multimodalChunk = await (0, _internalBhQAXls.n)(entry);
      if (!multimodalChunk) {
        this.clearIndexedFileData(entry.path, options.source);
      ...
      const baseChunks = filterNonEmptyMemoryChunks((0, _internalBhQAXls.r)(options.content ?? (await _promises.default.readFile(entry.absPath, "utf-8")), this.settings.chunking));
      chunks = this.provider ? (0, _memoryCoreHostEngineEmbeddingsBDFBUqKJ.u)(this.provider, baseChunks, EMBEDDING_BATCH_MAX_TOKENS) : baseChunks;
      if (options.source === "sessions" && "lineMap" in entry) (0, _internalBhQAXls.u)(chunks, entry.lineMap);
    }
    if (!this.provider) {
      this.writeChunks(entry, options.source, "fts-only", chunks, [], false);
...
async function searchVector(params) {
  ...
  FROM ${params.vectorTable} v\n  JOIN chunks c ON c.id = v.id\n WHERE v.embedding MATCH ? AND k = ? AND c.model = ?${params.sourceFilterVec.sql}\n ORDER BY dist ASC\n LIMIT ?`).all(qBlob, qBlob, candidateLimit, params.providerModel, ...params.sourceFilterVec.params, params.limit);
    ...
    return rows.map((row) => ({
      ...
      score: 1 - row.dist,
      snippet: (0, _utilsDvkbxKCZ.y)(row.text, params.snippetMaxChars),
      source: row.source
    }));
  }
...
function searchChunksByEmbedding(params) {
 ...
 WHERE model = ?${params.sourceFilter.sql}`).iterate(params.providerModel, ...params.sourceFilter.params);
  const topResults = [];
  for (const row of rows) {
    const score = (0, _internalBhQAXls.i)(params.queryVec, (0, _internalBhQAXls.l)(row.embedding));
...
const log$1 = (0, _subsystemIzrDYZz.t)("memory");
function isMemoryReadonlyDbError(err) {
  ...
  pushValue((0, _errorsG8SYDTCe.i)(err));
...
var MemoryIndexManager = exports.t = class MemoryIndexManager extends MemoryManagerEmbeddingOps {
  static async loadProviderResult(params) {
    return await createEmbeddingProvider({
      config: params.cfg,
      agentDir: (0, _agentScopeRNt6KatQ.b)(params.cfg, params.agentId),
      ...resolveMemoryPrimaryProviderRequest({ settings: params.settings })
    });
  }
  static async get(params) {
    const { cfg, agentId } = params;
    const settings = (0, _memorySearchCwVTNndi.t)(cfg, agentId);
    if (!settings) return null;
    const workspaceDir = (0, _agentScopeRNt6KatQ.x)(cfg, agentId);
  ...
  async search(query, opts) {
    ...
    if (!this.provider) {
      ...
      const resultSets = fullQueryResults.length > 0 ? [fullQueryResults] : await Promise.all((() => {
        const keywords = (0, _queryExpansionCic3aAyO.t)(cleaned, { ftsTokenizer: this.settings.store.fts.tokenizer });
  ...
  async readFile(params) {
    return await (0, _backendConfigDGhc9Ni_.r)({
      workspaceDir: this.workspaceDir,
      extraPaths: this.settings.extraPaths,
