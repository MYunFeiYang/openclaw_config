"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.createWeComMcpTool = createWeComMcpTool;
...
var _index = require("./interceptors/index.js"); /**
...
// 响应构造辅助
// ============================================================================
/** 构造统一的文本响应结构 */const textResult = (data) => ({ content: [{ type: "text", text: JSON.stringify(data, null, 2) }], details: data });const normalizeOptionalString = (value) => {const trimmed = value?.trim();return trimmed ? trimmed : undefined;}; /** 构造错误响应 */const errorResult = (err) => {// 适配企业微信 API 返回的 { errcode, errmsg } 结构
  if (err && typeof err === "object" && "errcode" in err) {const { errcode, errmsg } = err;
...
const handleList = async (ctx) => {
  const result = await (0, _transport.sendJsonRpc)(ctx.category, "tools/list", undefined, {
  ...
  return {
    ...
    tools: tools.map((t) => ({
      ...
      inputSchema: t.inputSchema ? (0, _schema.cleanSchemaForGemini)(t.inputSchema) : undefined
    }))
  };
};
// ============================================================================
...
const handleCall = async (ctx) => {
  ...
  const { options, args: resolvedArgs } = await (0, _index.resolveBeforeCall)(ctx);
  ...
  // 2. 执行 MCP 调用
  const result = await (0, _transport.sendJsonRpc)(category, "tools/call", {
    name: method,
    arguments: finalArgs
  }, requestOptions);
  ...
  const finalResult = await (0, _index.runAfterCall)(ctx, result);
