"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = writeBackfillDiaryEntries;exports.i = runDetachedDreamNarrative;exports.n = generateAndAppendDreamNarrative;exports.r = removeBackfillDiaryEntries;exports.t = dedupeDreamDiaryEntries;var _pathsB2cMKWd = require("./paths-B2cMK-wd.js");
var _errorsG8SYDTCe = require("./errors-g8SYDTCe.js");
var _globalSingletonCOlWgaGc = require("./global-singleton-COlWgaGc.js");
var _jsonFilesD8bfb2BK = require("./json-files-D8bfb2BK.js");
var _ioB4W7YRox = require("./io-B4W7YRox.js");
var _pathsBg_QeV2r = require("./paths-Bg_QeV2r.js");
var _storeLoadCtb7CJ9w = require("./store-load-Ctb7CJ9w.js");
var _storeBNAn6FU = require("./store-BNAn-6FU.js");
var _errorRuntimeBSkJRsKu = require("./error-runtime-BSkJRsKu.js");
require("./runtime-config-snapshot-DLxQrq4U.js");
require("./session-store-runtime-zf6-yW4s.js");
...
const dreamsFileLocks = (0, _globalSingletonCOlWgaGc.t)(Symbol.for("openclaw.memoryCore.dreamingNarrative.fileLocks"));
function isRequestScopedSubagentRuntimeError(err) {
  return err instanceof _errorRuntimeBSkJRsKu.t || err instanceof Error && err.name === "RequestScopedSubagentRuntimeError" && (0, _errorsG8SYDTCe.r)(err) === "OPENCLAW_SUBAGENT_RUNTIME_REQUEST_SCOPE";
}
function formatFallbackWriteFailure(err) {
  const code = (0, _errorsG8SYDTCe.r)(err);
  const name = (0, _errorsG8SYDTCe.c)(err);
...
function buildNarrativeSessionKey(params) {
  const workspaceHash = (0, _nodeCrypto.createHash)("sha1").update(params.workspaceDir).digest("hex").slice(0, 12);
...
async function resolveDreamsPath(workspaceDir) {
  for (const name of DREAMS_FILENAMES) {
    const target = _nodePath.default.join(workspaceDir, name);
    try {
      await _promises.default.access(target);
      return target;
    } catch (err) {
      if (err?.code !== "ENOENT") throw err;
    }
  }
  return _nodePath.default.join(workspaceDir, DREAMS_FILENAMES[0]);
}
async function readDreamsFile(dreamsPath) {
  try {
    return await _promises.default.readFile(dreamsPath, "utf-8");
  } catch (err) {
...
async function assertSafeDreamsPath(dreamsPath) {
  const stat = await _promises.default.lstat(dreamsPath).catch((err) => {
...
async function writeDreamsFileAtomic(dreamsPath, content) {
  ...
  const mode = (await _promises.default.stat(dreamsPath).catch((err) => {
    if (err.code === "ENOENT") return null;
    throw err;
  }))?.mode ?? 384;
  const tempPath = `${dreamsPath}.${process.pid}.${Date.now()}.tmp`;
  await _promises.default.writeFile(tempPath, content, {
    encoding: "utf-8",
    flag: "wx",
    mode
  });
  await _promises.default.chmod(tempPath, mode).catch(() => void 0);
  try {
    await _promises.default.rename(tempPath, dreamsPath);
    await _promises.default.chmod(dreamsPath, mode).catch(() => void 0);
  } catch (err) {
    const cleanupError = await _promises.default.rm(tempPath, { force: true }).catch((rmErr) => rmErr);
    if (cleanupError) throw new Error(`Atomic DREAMS.md write failed (${(0, _errorsG8SYDTCe.i)(err)}); cleanup also failed (${(0, _errorsG8SYDTCe.i)(cleanupError)})`, { cause: err });
    throw err;
  }
}
async function updateDreamsFile(params) {
  const dreamsPath = await resolveDreamsPath(params.workspaceDir);
  await _promises.default.mkdir(_nodePath.default.dirname(dreamsPath), { recursive: true });
  let lockEntry = dreamsFileLocks.get(dreamsPath);
  if (!lockEntry) {
    lockEntry = {
      withLock: (0, _jsonFilesD8bfb2BK.n)(),
      refs: 0
    };
...
async function safePathExists(pathname) {
  try {
    await _promises.default.stat(pathname);
    return true;
  } catch {
    return false;
  }
...
async function normalizeSessionFileForComparison(params) {
  ...
  const resolved = _nodePath.default.isAbsolute(trimmed) ? trimmed : _nodePath.default.resolve(params.sessionsDir, trimmed);
  try {
    return normalizeComparablePath(await _promises.default.realpath(resolved));
  } catch {
    return normalizeComparablePath(_nodePath.default.resolve(resolved));
  }
}
...
async function scrubDreamingNarrativeArtifacts(logger) {
  const cfg = (0, _ioB4W7YRox.i)();
  const agentsDir = _nodePath.default.join((0, _pathsB2cMKWd._)(), "agents");
  let agentEntries = [];
  try {
    agentEntries = await _promises.default.readdir(agentsDir, { withFileTypes: true });
  } catch {
    return;
  }
  let prunedEntries = 0;
  ...
  for (const agentEntry of agentEntries) {
    ...
    const storePath = (0, _pathsBg_QeV2r.u)(cfg.session?.store, { agentId: agentEntry.name });
    const sessionsDir = _nodePath.default.dirname(storePath);
    let store;
    try {
      store = (0, _storeLoadCtb7CJ9w.t)(storePath);
    } catch {
      continue;
    }
    ...
    if (needsStoreUpdate) {
      ...
      prunedEntries += await (0, _storeBNAn6FU.o)(storePath, async (lockedStore) => {
    ...
    let sessionFiles = [];
    try {
      sessionFiles = await _promises.default.readdir(sessionsDir, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const fileEntry of sessionFiles) {
      if (!fileEntry.isFile() || !fileEntry.name.endsWith(".jsonl")) continue;
      const transcriptPath = _nodePath.default.join(sessionsDir, fileEntry.name);
      ...
      let stat;
      try {
        stat = await _promises.default.stat(transcriptPath);
      } catch {
        continue;
      }
      ...
      let content = "";
      try {
        content = await _promises.default.readFile(transcriptPath, "utf-8");
      } catch {
        continue;
      }
      ...
      try {
        await _promises.default.rename(transcriptPath, archivedPath);
        archivedOrphans += 1;
      } catch {}
    }
  }
  ...
  try {
    ...
    for (const [attemptIndex, attemptModel] of attemptModels.entries()) {
      ...
      try {
        ...
        if (attemptModel && isConfiguredModelUnavailableNarrativeError((0, _errorsG8SYDTCe.i)(err))) {
          params.logger.warn(`memory-core: narrative generation could not start with configured model "${attemptModel}" for ${params.data.phase} phase; retrying with the session default (${(0, _errorsG8SYDTCe.i)(err)}).`);
          continue;
        }
        throw err;
      }
    ...
    params.logger.warn(`memory-core: narrative generation failed for ${params.data.phase} phase: ${(0, _errorsG8SYDTCe.i)(err)}`);
  } finally {
    ...
    for (const attempt of attempts) {
      ...
      try {
        ...
        params.logger.warn(`memory-core: narrative session cleanup failed for ${params.data.phase} phase: ${(0, _errorsG8SYDTCe.i)(cleanupErr)}`);
      }
    }
    await scrubDreamingNarrativeArtifacts(params.logger).catch((scrubErr) => {
      params.logger.warn(`memory-core: dreaming cleanup scrub failed for ${params.data.phase} phase: ${(0, _errorsG8SYDTCe.i)(scrubErr)}`);
    });
  }
}
