...
var _manifestOwnerPolicyBCQw37oP = require("./manifest-owner-policy-BCQw37oP.js");
var _pluginScopeCEsy4RJw = require("./plugin-scope-CEsy4RJw.js");
var _harnessRuntimesDbZLl4OL = require("./harness-runtimes-DbZLl4OL.js");
var _configPresenceVogioVHJ = require("./config-presence-vogioVHJ.js");
var _channelConfiguredDTowqAKL = require("./channel-configured-DTowqAKL.js");
var _setupRegistryCHoxAZCG = require("./setup-registry-CHoxAZCG.js");
...
function loadProviderRegistrySnapshot(params) {
  ...
  return (0, _pluginRegistryIimloLsx.p)({
...
function resolveProviderOwnerPluginIds(params) {
  ...
  const normalizedConfig = (0, _pluginRegistryIimloLsx.r)(params.config?.plugins, registry, { manifestRegistry: params.manifestRegistry });
...
function resolveEffectiveRegistryPluginActivation(params) {
  return (0, _configStateCOhlckC.l)({
    ...
    enabledByDefault: (0, _installedPluginIndexStoreIOgCdyHd.g)(params.plugin)
  });
}
function toManifestOwnerRecord(plugin) {
  return {
    ...
    enabledByDefault: (0, _installedPluginIndexStoreIOgCdyHd.g)(plugin)
  };
}
function withBundledProviderVitestCompat(params) {
  return (0, _bundledCompatC5xrsnA.r)(params);
}
...
function resolveOwningPluginIdsForProvider(params) {
  ...
  const deduped = dedupeSortedPluginIds([...(0, _pluginRegistryIimloLsx.c)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env,
    providerId: normalizedProvider,
    includeDisabled: true
  }), ...(0, _pluginRegistryIimloLsx.s)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env,
...
function resolveCatalogHookProviderPluginIds(params) {
  ...
  const normalizedConfig = (0, _pluginRegistryIimloLsx.r)(params.config?.plugins, registry);
...
function hasConfiguredEmbeddedHarnessRuntime(cfg, env) {
  return (0, _harnessRuntimesDbZLl4OL.t)(cfg, env, { includeEnvRuntime: false }).length > 0;
}
...
function collectConfiguredChannelIds(cfg, env) {
  const configuredStateChannelIds = new Set((0, _channelConfiguredDTowqAKL.i)());
  return (0, _configPresenceVogioVHJ.a)(cfg, env, { includePersistedAuthState: false }).map((signal) => ({
...
function isAutoEnableConfiguredChannelSignal(params) {
  if (params.source === "env" && params.configuredStateChannelIds.has(params.channelId) && !(0, _channelConfiguredDTowqAKL.r)({
    channelId: params.channelId,
    cfg: params.cfg,
    env: params.env
  })) return false;
  return (0, _channelConfiguredDTowqAKL.t)(params.cfg, params.channelId, params.env);
}
...
function resolvePluginAutoEnableReadiness(cfg, env) {
  ...
  return {
    mayNeedAutoEnable: (0, _setupRegistryCHoxAZCG.n)({
      config: cfg,
      env,
...
function resolveConfiguredPluginAutoEnableCandidates(params) {
  ...
  for (const runtime of (0, _harnessRuntimesDbZLl4OL.t)(params.config, params.env, { includeEnvRuntime: false })) {
  ...
  if (hasSetupAutoEnableRelevantConfig(params.config)) {
    ...
    for (const entry of (0, _setupRegistryCHoxAZCG.n)({
      config: params.config,
      env: params.env,
...
function resolvePluginAutoEnableManifestRegistry(params) {
  ...
  return ((0, _pluginRegistryIimloLsx.v)({
    config: params.config,
    env: params.env,
    allowWorkspaceScopedSnapshot: true
  }) ?? (() => {
    if (!canReuseUnscopedCurrentPluginMetadataSnapshot(params.config)) return;
    const snapshot = (0, _pluginRegistryIimloLsx.v)({
      env: params.env,
      allowWorkspaceScopedSnapshot: true,
      requireDefaultDiscoveryContext: true
    });
    return snapshot?.policyHash === (0, _installedPluginIndexStoreIOgCdyHd._)(params.config) ? snapshot : void 0;
  })())?.manifestRegistry ?? (0, _pluginMetadataSnapshotBxWkpxUl.i)({
    config: params.config,
    env: params.env
  }).manifestRegistry;
}
