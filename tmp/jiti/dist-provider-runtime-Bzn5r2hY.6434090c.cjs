...
var _providersRuntimeKg4pVITC = require("./providers.runtime-kg4pVITC.js");
...
function resolveProviderRuntimePluginCacheKey(params) {
  return JSON.stringify({
    ...
    pluginControlPlane: (0, _pluginRegistryIimloLsx.C)({
...
function findProviderRuntimePluginInLoadedRegistries(params) {
  const activeRegistry = (0, _activeRuntimeRegistryDQEvnLxe.n)({
  ...
  for (const surface of PREPARED_PROVIDER_RUNTIME_SURFACES) {
    const registry = (0, _activeRuntimeRegistryDQEvnLxe.n)({
      env: params.lookup.env,
      workspaceDir: params.lookup.workspaceDir,
      surface
    });
...
function resolveProviderPluginsForHooks(params) {
  ...
  if ((0, _providersRuntimeKg4pVITC.t)({
    ...params,
    workspaceDir,
    env,
    activate: false,
  ...
  return (0, _providersRuntimeKg4pVITC.n)({
    ...params,
    workspaceDir,
    env,
    activate: false,
...
function resolveProviderRuntimePlugin(params) {
  const apiOwnerHint = (0, _providersRuntimeKg4pVITC.r)({
...
function resolveLoadedProviderRuntimePlugin(params) {
  return findProviderRuntimePluginInLoadedRegistries({
    lookup: params,
    apiOwnerHint: (0, _providersRuntimeKg4pVITC.r)({
      provider: params.provider,
      config: params.config
    })
  });
...
function resolveRuntimeTextTransforms() {
  const registry = (0, _activeRuntimeRegistryDQEvnLxe.t)();
...
const log = (0, _subsystemLmgZvqF.t)("plugins/provider-runtime");
...
function hasExplicitProviderRuntimePluginActivation(params) {
  ...
  const ownerPluginIds = (0, _pluginAutoEnableQ8Kl1Dsn.h)({
    provider: params.provider,
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env
...
function resolveProviderPluginsForCatalogHooks(params) {
  const workspaceDir = params.workspaceDir ?? (0, _runtimeStateD14rXNRL.n)();
  const env = params.env ?? process.env;
  const onlyPluginIds = (0, _pluginAutoEnableQ8Kl1Dsn.c)({
    config: params.config,
    workspaceDir,
    env
  });
...
function normalizeProviderModelIdWithPlugin(params) {
  return (0, _stringCoerceLndEvhRk.c)(resolveProviderHookPlugin(params)?.normalizeModelId?.(params.context)) ?? (0, _modelRefSharedDTXcmZXB.o)(params);
}
...
function normalizeProviderConfigWithPlugin(params) {
  ...
  const bundledSurface = (0, _thinkingCvQif4jm.v)(params.provider);
...
function resolveProviderSyntheticAuthWithPlugin(params) {
  ...
  const discoveryPluginIds = [...new Set(providerRefs.flatMap((provider) => (0, _pluginAutoEnableQ8Kl1Dsn.h)({
    provider,
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env
...
function resolveExternalAuthProfilesWithPlugins(params) {
  const workspaceDir = params.workspaceDir ?? (0, _runtimeStateD14rXNRL.n)();
  const env = params.env ?? process.env;
  const externalAuthPluginIds = (0, _pluginAutoEnableQ8Kl1Dsn.p)({
    config: params.config,
    workspaceDir,
    env
  });
  const declaredPluginIds = new Set(externalAuthPluginIds);
  const fallbackPluginIds = (0, _pluginAutoEnableQ8Kl1Dsn.f)({
    config: params.config,
    workspaceDir,
    env,
    declaredPluginIds
