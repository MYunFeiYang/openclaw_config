"use strict";Object.defineProperty(exports, "__esModule", { value: true });Object.defineProperty(exports, "WeComCommand", { enumerable: true, get: function () {return _const.WeComCommand;} });Object.defineProperty(exports, "deleteReqIdForChat", { enumerable: true, get: function () {return _stateManager.deleteReqIdForChat;} });Object.defineProperty(exports, "flushReqIdStore", { enumerable: true, get: function () {return _stateManager.flushReqIdStore;} });Object.defineProperty(exports, "getReqIdForChat", { enumerable: true, get: function () {return _stateManager.getReqIdForChat;} });Object.defineProperty(exports, "getReqIdForChatAsync", { enumerable: true, get: function () {return _stateManager.getReqIdForChatAsync;} });Object.defineProperty(exports, "getWeComWebSocket", { enumerable: true, get: function () {return _stateManager.getWeComWebSocket;} });exports.monitorWeComProvider = monitorWeComProvider;Object.defineProperty(exports, "sendWeComReply", { enumerable: true, get: function () {return _messageSender.sendWeComReply;} });Object.defineProperty(exports, "setReqIdForChat", { enumerable: true, get: function () {return _stateManager.setReqIdForChat;} });Object.defineProperty(exports, "warmupReqIdStore", { enumerable: true, get: function () {return _stateManager.warmupReqIdStore;} });
...
var _aibotNodeSdk = require("@wecom/aibot-node-sdk");
var _const = require("./const.js");
var _dmPolicy = require("./dm-policy.js");
var _dynamicRouting = require("./dynamic-routing.js");
var _groupPolicy = require("./group-policy.js");
var _mediaHandler = require("./media-handler.js");
var _mediaUploader = require("./media-uploader.js");
var _messageParser = require("./message-parser.js");
var _messageSender = require("./message-sender.js");
var _openclawCompat = require("./openclaw-compat.js");
var _runtime = require("./runtime.js");
var _stateManager = require("./state-manager.js");
var _templateCardManager = require("./template-card-manager.js");
var _templateCardParser = require("./template-card-parser.js");
var _version = require("./version.js");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);} /**
 ...
 */function buildMediaOversizeHintText(err) {const maxMb = err.maxBytes / 1024 / 1024;return `当前OpenClaw限制文件不超过${maxMb}MB，请修改OpenClaw配置。`;} // ============================================================================
// 媒体本地路径白名单扩展
// ============================================================================
/**
 ...
 *   }
 * }
 */async function getExtendedMediaLocalRoots(config) {// 从兼容层获取默认白名单（内部已处理低版本 SDK 的 fallback）
  const defaults = await (0, _openclawCompat.getDefaultMediaLocalRoots)();const roots = [...defaults];const stateDir = path.resolve((0, _openclawCompat.resolveStateDir)());if (!roots.includes(stateDir)) {roots.push(stateDir);} // 合并用户在 WeComConfig 中配置的自定义路径
  if (config?.mediaLocalRoots) {for (const r of config.mediaLocalRoots) {const resolved = path.resolve(r.replace(/^~(?=\/|$)/, os.homedir()));
...
function buildMessageContext(frame, account, config, text, mediaList, quoteContent, runtime) {
  const core = (0, _runtime.getWeComRuntime)();
  ...
  // ===== 动态 Agent 路由注入 =====
  const routingResult = (0, _dynamicRouting.processDynamicRouting)({
    route,
    config,
    core,
    accountId: account.accountId,
  ...
  mediaList.length > 0 ?
  hasImages ?
  _const.MEDIA_IMAGE_PLACEHOLDER :
  _const.MEDIA_DOCUMENT_PLACEHOLDER :
  "");
  // 构建多媒体数组
  ...
  const ctxPayload = core.channel.reply.finalizeInboundContext({
    ...
    From: chatType === "group" ? `${_const.CHANNEL_ID}:group:${chatId}` : `${_const.CHANNEL_ID}:${body.from.userid}`,
    To: `${_const.CHANNEL_ID}:${chatId}`,
    SenderId: body.from.userid,
    ...
    Timestamp: Date.now(),
    Provider: _const.CHANNEL_ID,
    Surface: _const.CHANNEL_ID,
    OriginatingChannel: _const.CHANNEL_ID,
    OriginatingTo: `${_const.CHANNEL_ID}:${chatId}`,
...
async function sendThinkingReply(params) {
  ...
  try {
    await (0, _messageSender.sendWeComReply)({
      wsClient,
      frame,
    ...
    if (err instanceof _messageSender.StreamExpiredError && state) {
...
async function sendMediaBatch(ctx, mediaUrls) {
  ...
  for (const mediaUrl of mediaUrls) {
    const result = await (0, _mediaUploader.uploadAndSendMedia)({
      wsClient,
      mediaUrl,
      chatId,
      mediaLocalRoots,
  ...
  if (finishText) {
    ...
    if (!expired) {
      try {
        ...
        if (err instanceof _messageSender.StreamExpiredError) {
          expired = true;
        } else
        {
          throw err;
  ...
  try {
    ...
    await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
      ...
      replyOptions: {
        ...
        // },
      }, dispatcherOptions: { onReplyStart: async () => {if (!isShowThink && state.streamId && !state.accumulatedText) {try {await sendThinkingReply({
                wsClient,
                frame,
                streamId: state.streamId,
                runtime,
        ...
        deliver: async (payload, info) => {
          ...
          if (state.accumulatedText && !state.streamExpired) {
            try {
              const displayText = (0, _templateCardParser.maskTemplateCardBlocks)(state.accumulatedText, (...args) => runtime.log?.(...args));
    ...
    const cardResult = await (0, _templateCardManager.processTemplateCardsIfNeeded)({
      wsClient,
      frame,
      state,
      account,
...
async function prepareWeComMessage(params) {
  ...
  // Step 1: 解析消息内容
  const { textParts, imageUrls, imageAesKeys, fileUrls, fileAesKeys, quoteContent } = (0, _messageParser.parseMessageContent)(body);
  ...
  // Step 2: 群组策略检查（仅群聊）
  if (chatType === "group") {
    const groupPolicyResult = (0, _groupPolicy.checkGroupPolicy)({
      chatId,
      senderId: body.from.userid,
      account,
      config,
  ...
  const dmPolicyResult = await (0, _dmPolicy.checkDmPolicy)({
    senderId: body.from.userid,
  ...
  try {
    ...
    (0, _mediaHandler.downloadAndSaveImages)({
      imageUrls,
      imageAesKeys,
      account,
      config,
    ...
    (0, _mediaHandler.downloadAndSaveFiles)({
      fileUrls,
      fileAesKeys,
      account,
      config,
    ...
    if (err instanceof _mediaHandler.MediaOversizeError) {
      ...
      try {
        await (0, _messageSender.sendWeComReply)({ wsClient, frame, text: hintText, runtime, finish: true });
      }
      catch (replyErr) {
...
async function processWeComMessageNow(entry) {
  ...
  // Step 5: 初始化消息状态
  (0, _stateManager.setReqIdForChat)(chatId, reqId, account.accountId);
  const streamId = (0, _aibotNodeSdk.generateReqId)("stream");
  const state = { accumulatedText: "", streamId };
  (0, _stateManager.setMessageState)(messageId, state);
  const cleanupState = () => {
    (0, _stateManager.deleteMessageState)(messageId);
  };
  // // Step 6: 发送"思考中"消息
  ...
  (0, _stateManager.setSessionChatInfo)(route.sessionKey, {
...
async function monitorWeComProvider(options) {
  ...
  runtime.log?.(`[${account.accountId}] [${_version.PLUGIN_VERSION}] Initializing WSClient with SDK...`);
  // 启动消息状态定期清理
  (0, _stateManager.startMessageStateCleanup)();
  return new Promise((resolve, reject) => {
    const logger = createSdkLogger(runtime, account.accountId);
    const wsClient = new _aibotNodeSdk.WSClient({
      botId: account.botId,
      ...
      scene: _const.SCENE_WECOM_OPENCLAW,
      plug_version: _version.PLUGIN_VERSION
    });
    ...
    const cleanup = async () => {
      ...
      return;
      cleanedUp = true;
      (0, _stateManager.stopMessageStateCleanup)();
      await (0, _stateManager.cleanupAccount)(account.accountId);
    };
    ...
    wsClient.on("authenticated", () => {
      ...
      (0, _stateManager.setWeComWebSocket)(account.accountId, wsClient);
    });
    // 监听断开事件
    ...
    wsClient.on("error", async (error) => {
      ...
      if (error instanceof _aibotNodeSdk.WSAuthFailureError) {
      ...
      if (error instanceof _aibotNodeSdk.WSReconnectExhaustedError) {
    ...
    wsClient.on(_const.EVENT_ENTER_CHECK_UPDATE, async (frame) => {
      try {
        ...
        await wsClient.reply(frame, { version: _version.PLUGIN_VERSION }, _const.CMD_ENTER_EVENT_REPLY);
      }
      catch (err) {
    ...
    wsClient.on("event", async (frame) => {
      try {
        ...
        if (eventType === "template_card_event") {
          ...
          try {
            await (0, _templateCardManager.updateTemplateCardOnEvent)({
              frame,
              accountId: account.accountId,
              runtime,
              wsClient
    ...
    (0, _stateManager.warmupReqIdStore)(account.accountId, (...args) => runtime.log?.(...args)).
    then((count) => {
