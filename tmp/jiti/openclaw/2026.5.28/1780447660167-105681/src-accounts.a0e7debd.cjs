"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.hasMultiAccounts = hasMultiAccounts;exports.listEnabledWeComAccounts = listEnabledWeComAccounts;exports.listWeComAccountIds = listWeComAccountIds;exports.resolveDefaultWeComAccountId = resolveDefaultWeComAccountId;exports.resolveWeComAccountMulti = resolveWeComAccountMulti;exports.setWeComAccountMulti = setWeComAccountMulti;var _accountId = require("openclaw/plugin-sdk/account-id");
...
function listConfiguredAccountIds(cfg) {
  const accounts = cfg.channels?.[_const.CHANNEL_ID]?.accounts;
  if (!accounts || typeof accounts !== "object") {
    return [];
  }
  return Object.keys(accounts).filter(Boolean).map(_accountId.normalizeAccountId);
}
/**
...
function resolveDefaultWeComAccountId(cfg) {
  ...
  if (preferred) {
    return (0, _accountId.normalizeAccountId)(preferred);
  }
  const ids = listWeComAccountIds(cfg);
  if (ids.includes(_accountId.DEFAULT_ACCOUNT_ID)) {
    return _accountId.DEFAULT_ACCOUNT_ID;
  }
  return ids[0] ?? _accountId.DEFAULT_ACCOUNT_ID;
}
// ============================================================================
// 配置合并
// ============================================================================
...
function findAccountConfig(accounts, accountId) {
  ...
  for (const [key, value] of Object.entries(accounts)) {
    if ((0, _accountId.normalizeAccountId)(key) === normalized)
    return value;
  }
  return {};
}
...
function resolveWeComAccountMulti(params) {
  ...
  (0, _accountId.normalizeAccountId)(params.accountId) :
  resolveDefaultWeComAccountId(params.cfg);
  const wecomConfig = params.cfg.channels?.[_const.CHANNEL_ID];
  // 顶层 enabled 状态
  ...
  return {
    ...
    enabled: baseEnabled && accountEnabled,
    websocketUrl: merged.websocketUrl || _utils.DefaultWsUrl,
    botId: merged.botId ?? "",
...
function setWeComAccountMulti(cfg, updates, accountId) {
  ...
  if (!isMulti) {
    // 单账号模式：合并到顶层
    const existing = cfg.channels?.[_const.CHANNEL_ID] ?? {};
    const merged = { ...existing, ...updates };
    return {
      ...cfg,
      channels: {
        ...cfg.channels,
        [_const.CHANNEL_ID]: merged
      }
    };
  }
