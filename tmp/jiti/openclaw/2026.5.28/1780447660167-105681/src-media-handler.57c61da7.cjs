"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.MediaOversizeError = void 0;exports.downloadAndSaveFiles = downloadAndSaveFiles;exports.downloadAndSaveImages = downloadAndSaveImages;
 ...
 */class MediaOversizeError extends Error {kind;filename;
  sizeBytes;
  maxBytes;
  constructor(params) {
...
async function isImageBuffer(data) {
  const type = await (0, _fileType.fileTypeFromBuffer)(data);
...
async function downloadAndSaveImages(params) {
  ...
  for (const imageUrl of imageUrls) {
    try {
      ...
      const mediaMaxMb = config.agents?.defaults?.mediaMaxMb ?? _const.DEFAULT_MEDIA_MAX_MB;
      ...
      try {
        ...
        const result = await (0, _timeout.withTimeout)(wsClient.downloadFile(imageUrl, imageAesKey), _const.IMAGE_DOWNLOAD_TIMEOUT_MS, `Image download timed out: ${imageUrl}`);
        ...
        const fetched = await (0, _timeout.withTimeout)(core.channel.media.fetchRemoteMedia({ url: imageUrl }), _const.IMAGE_DOWNLOAD_TIMEOUT_MS, `Manual image download timed out: ${imageUrl}`);
  ...
  for (const fileUrl of fileUrls) {
    try {
      ...
      try {
        ...
        const result = await (0, _timeout.withTimeout)(wsClient.downloadFile(fileUrl, fileAesKey), _const.FILE_DOWNLOAD_TIMEOUT_MS, `File download timed out: ${fileUrl}`);
        fileBuffer = result.buffer;
        originalFilename = result.filename;
        // 检测文件类型
        const type = await (0, _fileType.fileTypeFromBuffer)(fileBuffer);
        ...
        const fetched = await (0, _timeout.withTimeout)(core.channel.media.fetchRemoteMedia({ url: fileUrl }), _const.FILE_DOWNLOAD_TIMEOUT_MS, `Manual file download timed out: ${fileUrl}`);
