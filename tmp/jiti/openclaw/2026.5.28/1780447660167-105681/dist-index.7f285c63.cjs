"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _openclawCompat = require("./src/openclaw-compat.js");
var _channel = require("./src/channel.js");
var _index = require("./src/mcp/index.js");
var _stateManager = require("./src/state-manager.js");
var _runtime = require("./src/runtime.js");
var _const = require("./src/const.js");
var _webhook = require("./src/agent/webhook.js");
var _index2 = require("./src/webhook/index.js");
const plugin = {
  id: "wecom-openclaw-plugin",
  name: "企业微信",
  description: "企业微信 OpenClaw 插件",
  configSchema: (0, _openclawCompat.emptyPluginConfigSchema)(),
  register(api) {
    (0, _runtime.setWeComRuntime)(api.runtime);
    api.registerChannel({ plugin: _channel.wecomPlugin });
    // 注册 wecom_mcp：通过 HTTP 直接调用企业微信 MCP Server
    api.registerTool((ctx) => {
      const trustedRequesterUserId = ctx.messageChannel === _const.CHANNEL_ID ? ctx.requesterSenderId?.trim() ?? undefined : undefined;
      ...
      const sessionChat = (0, _stateManager.getSessionChatInfo)(ctx.sessionKey);
      ...
      return (0, _index.createWeComMcpTool)({
    ...
    const agentWebhookHandler = (0, _webhook.createWecomAgentWebhookHandler)(api.runtime);
    // 注册 Agent 模式 HTTP 路由（prefix match 覆盖带 accountId 的子路径）
    api.registerHttpRoute({
      path: _const.WEBHOOK_PATHS.AGENT_PLUGIN,
      handler: agentWebhookHandler,
      auth: "plugin",
      match: "prefix"
    });
    api.registerHttpRoute({
      path: _const.WEBHOOK_PATHS.AGENT,
      handler: agentWebhookHandler,
      auth: "plugin",
      match: "prefix"
    });
    ...
    const webhookRoutes = [_const.WEBHOOK_PATHS.BOT_PLUGIN, _const.WEBHOOK_PATHS.BOT_ALT, _const.WEBHOOK_PATHS.BOT];
    for (const routePath of webhookRoutes) {
      api.registerHttpRoute({
        path: routePath,
        handler: _index2.handleWecomWebhookRequest,
        auth: "plugin",
        match: "prefix"
      });
    }
    ...
    api.on("before_prompt_build", (_event, ctx) => {
      ...
      if (ctx?.channelId !== _const.CHANNEL_ID) {
        return;
      }
      return {
        appendSystemContext: [
