"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.readResponseBodyAsBuffer = readResponseBodyAsBuffer;exports.wecomFetch = wecomFetch;var _undici = require("undici");
...
function getProxyDispatcher(proxyUrl) {
  ...
  if (existing)
  return existing;
  const created = new _undici.ProxyAgent(proxyUrl);
 ...
 */
async function wecomFetch(input, init, opts) {
  ...
  try {
    return await (0, _undici.fetch)(input, nextInit);
  }
  catch (err) {
 ...
 * 适用于下载媒体文件等场景。
 */
async function readResponseBodyAsBuffer(res, maxBytes) {
  if (!res.body)
