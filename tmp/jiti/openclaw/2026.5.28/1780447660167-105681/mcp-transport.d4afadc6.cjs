"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.WECOM_USERID_HEADER = exports.McpRpcError = exports.McpHttpError = exports.MEDIA_DOWNLOAD_TIMEOUT_MS = void 0;exports.clearAccountCache = clearAccountCache;exports.clearCategoryCache = clearCategoryCache;exports.resolveCurrentAccountId = resolveCurrentAccountId;exports.sendJsonRpc = sendJsonRpc;
...
// 内部状态
// ============================================================================
/** HTTP 请求超时时间（毫秒） */const HTTP_REQUEST_TIMEOUT_MS = 30_000; /** 媒体下载请求超时时间（毫秒），base64 编码的媒体文件最大可达 ~27MB */const MEDIA_DOWNLOAD_TIMEOUT_MS = exports.MEDIA_DOWNLOAD_TIMEOUT_MS = 120_000; /** 日志前缀 */const LOG_TAG = "[mcp]"; /** 请求 MCP Server 时透传可信企业微信 userid 的 header 名 */const WECOM_USERID_HEADER = exports.WECOM_USERID_HEADER = "x-openclaw-wecom-userid"; /**
 * MCP JSON-RPC 错误
 *
...
function isWebSocketBot(cfg, accountId) {
  const resolved = (0, _accounts.resolveWeComAccountMulti)({ cfg, accountId });
...
function resolveMcpAccountId(cfg) {
  const defaultAccountId = (0, _accounts.resolveDefaultWeComAccountId)(cfg);
  // 优先使用默认账户
  if (isWebSocketBot(cfg, defaultAccountId)) {
    return defaultAccountId;
  }
  // 默认账户非长连接，查找第一个长连接账户
  const allIds = (0, _accounts.listWeComAccountIds)(cfg);
...
function resolveCurrentAccountId() {
  try {
    ...
    return (0, _accounts.resolveDefaultWeComAccountId)(cfg);
  }
  catch {
    // runtime 未初始化时回退到 "default"
    return _openclawCompat.DEFAULT_ACCOUNT_ID;
  }
}
/**
...
async function fetchMcpConfig(accountId, category) {
  const wsClient = (0, _stateManager.getWeComWebSocket)(accountId);
  if (!wsClient) {
    throw new Error(`WSClient 未连接 (accountId="${accountId}")，无法拉取 MCP 配置`);
  }
  const reqId = (0, _aibotNodeSdk.generateReqId)("mcp_config");
  const response = await (0, _timeout.withTimeout)(wsClient.reply({ headers: { req_id: reqId } }, { biz_type: category, plugin_version: _version.PLUGIN_VERSION }, _const.MCP_GET_CONFIG_CMD), _const.MCP_CONFIG_FETCH_TIMEOUT_MS, `MCP config fetch for "${category}" timed out after ${_const.MCP_CONFIG_FETCH_TIMEOUT_MS}ms`);
...
async function initializeSession(url, accountId, category, requesterUserId) {
  ...
  const initBody = {
    jsonrpc: "2.0",
    id: (0, _aibotNodeSdk.generateReqId)("mcp_init"),
    method: "initialize",
    params: {
...
async function sendJsonRpc(category, method, params, options) {
  ...
  const body = {
    jsonrpc: "2.0",
    id: (0, _aibotNodeSdk.generateReqId)("mcp_rpc"),
    method,
