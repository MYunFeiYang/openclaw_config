"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.checkDmPolicy = checkDmPolicy;
 ...
 */async function checkDmPolicy(params) {const { senderId, isGroup, account, wsClient, frame, runtime } = params;const core = (0, _runtime.getWeComRuntime)();
  // 群聊消息不检查 DM Policy
  if (isGroup) {
    return { allowed: true };
  }
  ...
  readAllowFromStore({ channel: _const.CHANNEL_ID, accountId: account.accountId }).
  catch(() => []);
  // 检查发送者是否在允许列表中
  const storeAllowFrom = [...oldStoreAllowFrom, ...newStoreAllowFrom];
  const effectiveAllowFrom = [...configAllowFrom, ...storeAllowFrom];
  const senderAllowedResult = (0, _groupPolicy.isSenderAllowed)(senderId, effectiveAllowFrom);
