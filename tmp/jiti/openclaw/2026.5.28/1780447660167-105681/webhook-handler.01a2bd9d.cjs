"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.handleWecomWebhookRequest = handleWecomWebhookRequest;
...
var _aibotNodeSdk = require("@wecom/aibot-node-sdk");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };} /**
 * Webhook HTTP 请求处理
 *
...
// 辅助函数
// ============================================================================
/** 解析 URL 查询参数 */function parseQuery(url) {const idx = url.indexOf("?");if (idx < 0) return {};const params = new URLSearchParams(url.slice(idx + 1));const result = {};for (const [key, value] of params) {result[key] = value;
  }
  return result;
}
/**
...
function shouldProcessBotInboundMessage(msg) {
  const senderUserId = (0, _helpers.resolveWecomSenderUserId)(msg)?.trim();
  if (!senderUserId) {
...
function encryptResponse(target, responseData, timestamp, nonce) {
  ...
  const wc = new _aibotNodeSdk.WecomCrypto(target.account.token, target.account.encodingAESKey, target.account.receiveId);
...
function findMatchingTarget(requestPath, signature, timestamp, nonce, encrypt, pathAccountId) {
  ...
  const targetsMap = (0, _target.getWebhookTargetsMap)();
  ...
  if (pathAccountId && pathTargets) {
    ...
    if (byAccountId?.account?.token) {
      const wc = new _aibotNodeSdk.WecomCrypto(byAccountId.account.token, byAccountId.account.encodingAESKey, byAccountId.account.receiveId);
  ...
  pathTargets :
  (0, _target.getRegisteredTargets)();
...
async function handleWecomWebhookRequest(req, res) {
  ...
  const reqId = _nodeCrypto.default.randomUUID().slice(0, 8);
  const url = req.url ?? "/";
  ...
  if (method === "POST") {
    ...
    const proxyUrl = (0, _utils.resolveWecomEgressProxyUrl)(target.config);
    // 按消息类型分发
    try {
...
async function dispatchMessage(target, message, timestamp, nonce, proxyUrl) {
  ...
  // stream_refresh 轮询
  if (msgtype === "stream") {
    return (0, _monitor.handleStreamRefresh)(target, message);
  }
  // 事件处理
  if (msgtype === "event") {
    const eventType = String(message.event?.eventtype ?? "").toLowerCase();
    if (eventType === "enter_chat") {
      return (0, _monitor.handleEnterChat)(target, message);
    }
    if (eventType === "template_card_event") {
      return (0, _monitor.handleTemplateCardEvent)(target, message, timestamp, nonce, proxyUrl);
    }
  ...
  if (["text", "image", "file", "voice", "video", "mixed"].includes(msgtype)) {
    ...
    if (!filterResult.shouldProcess) {
      target.runtime.log?.(`[webhook] 消息过滤: msgtype=${msgtype} reason=${filterResult.reason} from=${(0, _helpers.resolveWecomSenderUserId)(message) ?? "N/A"} chatType=${String(message.chattype ?? "N/A")}`);
      return null;
    }
    return (0, _monitor.handleInboundMessage)(target, message, timestamp, nonce, proxyUrl, filterResult);
  }
