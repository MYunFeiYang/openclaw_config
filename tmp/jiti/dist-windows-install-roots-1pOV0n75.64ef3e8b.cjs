"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = getWindowsProgramFilesRoots;exports.t = getWindowsInstallRoots;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
function getWindowsRegExeCandidates(env) {
  ...
  {
    if (!root) continue;
    const key = (0, _stringCoerceBje8XVt.a)(root);
    if (seen.has(key)) continue;
    seen.add(key);
    candidates.push(_nodePath.default.win32.join(root, "System32", "reg.exe"));
  }
  return candidates;
}
...
function getWindowsProgramFilesRoots(env = process.env) {
  ...
  {
    if (!value) continue;
    const key = (0, _stringCoerceBje8XVt.a)(value);
    if (seen.has(key)) continue;
    seen.add(key);
    result.push(value);
  }
