"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.getMonitorState = getMonitorState;exports.startWebhookGateway = startWebhookGateway;exports.stopWebhookGateway = stopWebhookGateway;
...
// 全局状态
// ============================================================================
/** 按 accountId 管理各账号的 Target 注销函数 */const accountUnregisters = new Map(); /** FlushHandler 是否已设置（只需设置一次，因为 monitorState 是单例） */let flushHandlerInstalled = false; // ============================================================================
// 路径解析
// ============================================================================
/**
 * 去除重复路径
 */function uniquePaths(paths) {return Array.from(new Set(paths.map((p) => p.trim()).filter(Boolean)));} /**
 * 解析 Bot Target 注册路径
 *
 ...
 */function resolveBotRegistrationPaths(params) {if (params.matrixMode) {
    return uniquePaths([
    `${_types.WEBHOOK_PATHS.BOT_PLUGIN}/${params.accountId}`,
    `${_types.WEBHOOK_PATHS.BOT_ALT}/${params.accountId}`,
    // 兼容老路径：不带 accountId 后缀，签名验证会自动匹配到正确账号
    _types.WEBHOOK_PATHS.BOT_PLUGIN,
    _types.WEBHOOK_PATHS.BOT,
    _types.WEBHOOK_PATHS.BOT_ALT]
    );
  }
  ...
  _types.WEBHOOK_PATHS.BOT,
  _types.WEBHOOK_PATHS.BOT_ALT,
  `${_types.WEBHOOK_PATHS.BOT_PLUGIN}/${_openclawCompat.DEFAULT_ACCOUNT_ID}`,
  `${_types.WEBHOOK_PATHS.BOT_ALT}/${_openclawCompat.DEFAULT_ACCOUNT_ID}`]
  );
}
// ============================================================================
// 公共 API
...
function startWebhookGateway(ctx) {
  ...
  _state.monitorState.startPruning(_types.PRUNE_INTERVAL_MS);
  // 3. 设置 FlushHandler（仅首次，因为 monitorState 是全局单例）
  if (!flushHandlerInstalled) {
    _state.monitorState.streamStore.setFlushHandler((pending) => void flushPending(pending));
  ...
  // 判断是否为多账号模式
  const matrixMode = (0, _accounts.hasMultiAccounts)(ctx.config);
  const target = {
    account,
    config,
    runtime: runtimeEnv,
    core: ctx.channelRuntime ?? runtime, // PluginRuntime 实例
    path: `${_types.WEBHOOK_PATHS.BOT_PLUGIN}/${ctx.accountId}`, // 主路径（用于日志和状态显示）
  ...
  const unregister = (0, _target.registerWecomWebhookTarget)(target, paths);
...
function stopWebhookGateway(ctx) {
  ...
  if (!(0, _target.hasActiveTargets)()) {
    _state.monitorState.stopPruning();
  }
  // 3. 更新状态
  ctx.setStatus?.({
    accountId: ctx.accountId,
...
async function flushPending(pending) {
  ...
  const { streamStore } = _state.monitorState;
  ...
  let core = null;
  try {
    core = (0, _runtime.getWeComRuntime)();
  }
  catch (err) {
  ...
  if (core) {
    ...
    (0, _monitor.startAgentForStream)({
