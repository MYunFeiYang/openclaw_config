...
function collectStoredSessionEntrySlotKeys(entry, pluginId) {
  ...
  for (const record of records) for (const slotKey of Object.values(record)) {
    const normalized = (0, _storeLoadDjaY7SAt.o)(slotKey);
...
async function clearPluginOwnedSessionStores(params) {
  ...
  const storePaths = new Set((0, _targetsDGxp4nvt.i)(params.cfg).map((target) => target.storePath).filter((storePath) => _nodeFs.default.existsSync(storePath)));
  let cleared = 0;
  for (const storePath of storePaths) cleared += await (0, _storeCmwzJ9N.s)(storePath, (store) => {
...
async function runPluginHostCleanup(params) {
  ...
  const sessionEntrySlotKeys = collectSessionEntrySlotKeys(registry ?? (0, _runtimeNSi6iKFR.a)(), params.pluginId);
  ...
  if (shouldCleanup()) try {
    persistentCleanupCount = params.reason === "restart" ? await clearPromotedSessionEntrySlotStores({
      cfg: params.cfg ?? (0, _ioDnGGugrj.i)(),
  ...
  if (registry) {
    for (const registration of registry.sessionExtensions ?? []) {
      ...
      try {
        await (0, _runtimeNSi6iKFR.M)(hookId, () => cleanup({
    ...
    const schedulerFailures = await (0, _runtimeNSi6iKFR.w)({
      pluginId: params.pluginId,
      reason: params.reason,
      sessionKey: params.sessionKey,
  ...
  if (params.reason !== "restart" && shouldCleanup()) {
    ...
    })).filter(({ jobId }) => jobId.length > 0).map(({ pluginId, jobId }) => (0, _runtimeNSi6iKFR.k)(pluginId, jobId)));
    const runtimeSchedulerFailures = await (0, _runtimeNSi6iKFR.w)({
      pluginId: params.pluginId,
      reason: params.reason,
      sessionKey: params.sessionKey,
      preserveJobIds: params.preserveSchedulerJobIds,
  ...
  if (shouldCleanup() && (params.pluginId || params.runId) && (params.reason !== "restart" || params.runId)) (0, _runtimeNSi6iKFR.T)({
    pluginId: params.pluginId,
    runId: params.runId
  });
  return {
