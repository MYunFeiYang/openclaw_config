"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = resolveCommandSecretRefsViaGateway;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
var _protocolBvHKcb_ = require("./protocol-BvHKcb_2.js");
var _callXuTtjcDL = require("./call-xuTtjcDL.js");
var _secretValueCavHlQ1h = require("./secret-value-CavHlQ1h.js");
var _commandConfigCTF7m9hu = require("./command-config-CTF7m9hu.js");
var _runtimeWebToolsCcpOsAbg = require("./runtime-web-tools-CcpOsAbg.js");
...
const commandSecretGatewayDeps = {
  analyzeCommandSecretAssignmentsFromSnapshot: _commandConfigCTF7m9hu.t,
  collectConfigAssignments: _runtimeWebToolsCcpOsAbg.n,
  discoverConfigSecretTargetsByIds: _targetRegistryDOBQeKjE.r,
  resolveManifestContractOwnerPluginId: _pluginRegistryBjn9rPwy.o,
  resolveRuntimeWebTools: _runtimeWebToolsCcpOsAbg.t
};
...
function classifyRuntimeWebTargetPathState(params) {
  ...
  if (pluginId) {
    if (params.path.endsWith(".config.webFetch.apiKey")) {
      ...
      const configuredProvider = (0, _stringCoerceBje8XVt.a)(fetch?.provider);
    ...
    const configuredProvider = (0, _stringCoerceBje8XVt.a)(search?.provider);
    if (!configuredProvider) return "active";
    return commandSecretGatewayDeps.resolveManifestContractOwnerPluginId({
...
function classifyConfiguredTargetRefs(params) {
  ...
  const context = (0, _runtimeSharedClUz2kEK.r)({
...
function parseGatewaySecretsResolveResult(payload) {
  if (!(0, _protocolBvHKcb_.wt)(payload)) throw new Error("gateway returned invalid secrets.resolve payload.");
...
function isUnsupportedSecretsResolveError(err) {
  const message = (0, _stringCoerceBje8XVt.a)((0, _errorsG8SYDTCe.i)(err));
...
async function resolveCommandSecretRefsLocally(params) {
  ...
  if (targetsRuntimeWebResolution({
    ...
    localResolutionDiagnostics.push(`${params.commandName}: failed to resolve web tool secrets locally (${(0, _errorsG8SYDTCe.i)(error)}).`);
  }
...
function scrubUnresolvedAssignments(config, unresolved) {
  for (const entry of unresolved) (0, _pathUtilsBF77VRFq.i)(config, entry.pathSegments, void 0);
}
...
async function resolveTargetSecretLocally(params) {
  ...
  try {
    const resolved = await (0, _resolveCKxwAAOr.a)(ref, {
    ...
    (0, _pathUtilsBF77VRFq.i)(params.resolvedConfig, params.target.pathSegments, resolved);
  } catch (error) {
    if (!enforcesResolvedSecrets(params.mode)) params.localResolutionDiagnostics.push(`${params.commandName}: failed to resolve ${params.target.path} locally (${(0, _errorsG8SYDTCe.i)(error)}).`);
  }
}
async function resolveCommandSecretRefsViaGateway(params) {
  ...
  let payload;
  try {
    payload = await (0, _callXuTtjcDL.r)({
      config: params.config,
      ...
      timeoutMs: 3e4,
      clientName: _clientInfoBhnDRj_s.i.CLI,
      mode: _clientInfoBhnDRj_s.r.CLI
    });
  } catch (err) {
    try {
      ...
      return {
        ...
        diagnostics: dedupeDiagnostics([...fallback.diagnostics, `${params.commandName}: gateway secrets.resolve unavailable (${(0, _errorsG8SYDTCe.i)(err)}); ${fallbackMessage}`]),
        targetStatesByPath: fallback.targetStatesByPath,
        hadUnresolvedTargets: fallback.hadUnresolvedTargets
      };
    } catch {}
    if (isUnsupportedSecretsResolveError(err)) throw new Error(`${params.commandName}: active gateway does not support secrets.resolve (${(0, _errorsG8SYDTCe.i)(err)}). Update the gateway or run without SecretRefs.`, { cause: err });
    throw new Error(`${params.commandName}: failed to resolve secrets from the active gateway snapshot (${(0, _errorsG8SYDTCe.i)(err)}). Start the gateway and retry.`, { cause: err });
  }
  ...
  for (const assignment of parsed.assignments) {
    ...
    try {
      (0, _pathUtilsBF77VRFq.i)(resolvedConfig, pathSegments, assignment.value);
    } catch (err) {
      const path = pathSegments.join(".");
      throw new Error(`${params.commandName}: failed to apply resolved secret assignment at ${path} (${(0, _errorsG8SYDTCe.i)(err)}).`, { cause: err });
    }
  }
  ...
  if (analyzed.unresolved.length > 0) try {
    ...
    for (const unresolved of analyzed.unresolved) {
      ...
      (0, _pathUtilsBF77VRFq.i)(resolvedConfig, unresolved.pathSegments, (0, _pathUtilsBF77VRFq.n)(localFallback.resolvedConfig, unresolved.pathSegments));
    ...
    diagnostics = dedupeDiagnostics([
    ...diagnostics,
    `${params.commandName}: local fallback after incomplete gateway snapshot failed (${(0, _errorsG8SYDTCe.i)(error)}).`,
