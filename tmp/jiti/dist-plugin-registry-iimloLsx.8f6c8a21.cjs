...
var _manifestRegistryInstalled7RUx9j9g = require("./manifest-registry-installed-7RUx9j9g.js");
...
function fingerprintPluginDiscoveryContext(context) {
  return (0, _installedPluginIndexStoreIOgCdyHd.b)(context);
}
function resolvePluginControlPlaneContext(params = {}) {
  const inventoryFingerprint = params.inventoryFingerprint ?? (params.index ? (0, _manifestRegistryInstalled7RUx9j9g.n)(params.index) : void 0);
  return {
    discovery: resolvePluginDiscoveryContext(params),
    policyFingerprint: params.policyHash ?? (0, _installedPluginIndexStoreIOgCdyHd._)(params.config),
...
function createPluginRegistryIdNormalizer(index, options = {}) {
  ...
  const registry = options.lookUpTable?.manifestRegistry ?? options.manifestRegistry ?? (0, _manifestRegistryInstalled7RUx9j9g.t)({
    index,
...
function setCurrentPluginMetadataSnapshot(snapshot, options = {}) {
  const compatiblePolicyHashes = snapshot ? options.compatibleConfigs?.map((config) => (0, _installedPluginIndexStoreIOgCdyHd._)(config)) : void 0;
  ...
  (0, _installedPluginIndexStoreIOgCdyHd.l)(snapshot, snapshot ? resolvePluginMetadataControlPlaneFingerprint(options.config, {
    env: options.env,
    index: snapshot.index,
...
function clearCurrentPluginMetadataSnapshot() {
  (0, _installedPluginIndexStoreIOgCdyHd.s)();
}
function captureCurrentPluginMetadataSnapshotState() {
  return (0, _installedPluginIndexStoreIOgCdyHd.c)();
}
function restoreCurrentPluginMetadataSnapshotState(state) {
  (0, _installedPluginIndexStoreIOgCdyHd.l)(state.snapshot, state.configFingerprint, state.compatiblePolicyHashes, state.compatibleConfigFingerprints);
}
function getCurrentPluginMetadataSnapshot(params = {}) {
  const { snapshot: rawSnapshot, configFingerprint, compatiblePolicyHashes, compatibleConfigFingerprints } = (0, _installedPluginIndexStoreIOgCdyHd.c)();
  const snapshot = rawSnapshot;
  if (!snapshot) return;
  const requestedPolicyHash = params.config ? (0, _installedPluginIndexStoreIOgCdyHd._)(params.config) : void 0;
...
function hasMissingPersistedPluginSource(index) {
  return index.plugins.some((plugin) => {
    ...
    return !_nodeFs.default.existsSync(plugin.rootDir) || !(0, _installedPluginIndexStoreIOgCdyHd.m)(plugin) && !_nodeFs.default.existsSync(plugin.manifestPath) || (plugin.source ? !_nodeFs.default.existsSync(plugin.source) : false) || (plugin.setupSource ? !_nodeFs.default.existsSync(plugin.setupSource) : false);
  });
}
...
function hasStalePersistedPluginMetadata(index) {
  return index.plugins.some((plugin) => {
    if (!(0, _installedPluginIndexStoreIOgCdyHd.m)(plugin)) {
      if ((0, _installedPluginIndexStoreIOgCdyHd.y)(plugin.manifestPath, plugin.manifestFile) !== true) {
    ...
    const packageJsonSignatureMatches = (0, _installedPluginIndexStoreIOgCdyHd.y)(packageJsonPath, plugin.packageJson.fileSignature);
...
function hasRecoveredInstallRecordsMissingFromPersistedIndex(index, installRecords, env) {
  const persistedRecords = (0, _installedPluginIndexStoreIOgCdyHd.h)(index);
...
function loadPluginRegistrySnapshotWithMetadata(params = {}) {
  ...
  if (persistedInstallRecordReadsEnabled) {
    persistedIndex = (0, _installedPluginIndexStoreIOgCdyHd.r)(params);
    if (persistedReadsEnabled && persistedIndex) {if (params.config && persistedIndex.policyHash !== (0, _installedPluginIndexStoreIOgCdyHd._)(params.config)) diagnostics.push({
        level: "warn",
  ...
  return {
    snapshot: (0, _installedPluginIndexStoreIOgCdyHd.f)({
      ...params,
...
function getPluginRecord(params) {
  return (0, _installedPluginIndexStoreIOgCdyHd.u)(resolveSnapshot(params), params.pluginId);
}
function isPluginEnabled(params) {
  return (0, _installedPluginIndexStoreIOgCdyHd.d)(resolveSnapshot(params), params.pluginId, params.config);
}
function inspectPluginRegistry(params = {}) {
  return (0, _installedPluginIndexStoreIOgCdyHd.t)(params);
}
function refreshPluginRegistry(params) {
  return (0, _installedPluginIndexStoreIOgCdyHd.i)(params);
}
//#endregion
...
function resolveContributionPluginIds(params) {
  ...
  return params.index.plugins.filter((plugin) => (0, _installedPluginIndexStoreIOgCdyHd.d)(params.index, plugin.pluginId, params.config)).map((plugin) => plugin.pluginId);
}
function loadContributionManifestRegistry(params) {
  return (0, _manifestRegistryInstalled7RUx9j9g.t)({
    index: params.index,
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env,
