...
var _manifestMetadataScanB2JJE1RN = require("./manifest-metadata-scan-B2JJE1RN.js");
...
function collectManifestProviderEndpoints() {
  const entries = [];
  for (const { manifest } of (0, _manifestMetadataScanB2JJE1RN.t)()) entries.push(...readManifestProviderEndpoints(manifest));
  return entries;
}
function collectManifestProviderRequests() {
  const entries = /* @__PURE__ */new Map();
  for (const { manifest } of (0, _manifestMetadataScanB2JJE1RN.t)()) for (const [provider, request] of readManifestProviderRequests(manifest)) entries.set(provider, request);
  return entries;
}
