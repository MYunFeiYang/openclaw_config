...
var _deliveryContextSharedBuB7h14N = require("./delivery-context.shared-BuB7h14N.js");
...
require("./reset-CX-zoWBz.js");
require("./session-key-B5xoqibm.js");
require("./transcript-BEv_N5f2.js");
...
function extractDeliveryInfo(sessionKey, options) {
  const { baseSessionKey, threadId } = (0, _storeLoadDjaY7SAt.S)(sessionKey);
  ...
  try {
    const lookup = loadDeliverySessionEntry({
      cfg: options?.cfg ?? (0, _ioDnGGugrj.i)(),
      sessionKey,
      baseSessionKey
    });
    let entry = lookup.entry;
    let storedDeliveryContext = (0, _deliveryContextSharedBuB7h14N.t)(entry);
    if (!hasRoutableDeliveryContext(storedDeliveryContext) && baseSessionKey !== sessionKey) {
      entry = lookup.baseEntry;
      storedDeliveryContext = (0, _deliveryContextSharedBuB7h14N.t)(entry);
    }
...
function resolveDeliveryStorePaths(cfg, agentId) {
  ...
  for (const target of (0, _targetsDGxp4nvt.i)(cfg)) if (target.agentId === agentId) paths.add(target.storePath);
  return [...paths];
}
function findSessionEntryInStore(store, keys) {
  ...
  const acceptCandidate = (candidate) => {
    if (!candidate) return;
    const candidateRoutable = hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(candidate));
  ...
  for (const key of keys) {
    ...
    const normalized = (0, _storeLoadDjaY7SAt.k)(key);
    ...
    if (Object.prototype.hasOwnProperty.call(store, normalized)) {
      foundRoutableCandidate ||= hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(store[normalized]));
      acceptCandidate(store[normalized]);
    }
    if (foldedLegacyKey !== normalized && Object.prototype.hasOwnProperty.call(store, foldedLegacyKey)) {
      foundRoutableCandidate ||= hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(store[foldedLegacyKey]));
      acceptCandidate(store[foldedLegacyKey]);
    }
    if (trimmed !== normalized && Object.prototype.hasOwnProperty.call(store, trimmed)) {
      foundRoutableCandidate ||= hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(store[trimmed]));
...
function buildFreshestSessionEntryIndex(store) {
  ...
  for (const [key, entry] of Object.entries(store)) {
    ...
    const entryRoutable = hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(entry));
    const existingRoutable = hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(existing));
    ...
    const foldedExistingRoutable = hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(foldedExisting));
...
function loadDeliverySessionEntry(params) {
  ...
  const canonicalBaseKey = (0, _combinedStoreGatewayDTxOYXKu.a)({
    cfg: params.cfg,
    sessionKey: params.baseSessionKey
  });
  const agentId = (0, _combinedStoreGatewayDTxOYXKu.i)(params.cfg, canonicalKey);
  ...
  for (const storePath of resolveDeliveryStorePaths(params.cfg, agentId)) {
    const store = (0, _storeLoadDjaY7SAt.t)(storePath);
    ...
    if (hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(entry)) || hasRoutableDeliveryContext((0, _deliveryContextSharedBuB7h14N.t)(baseEntry))) return {
      entry,
      baseEntry
    };
  }
...
function resolveMainSessionKeyFromConfig() {
  return (0, _mainSessionBIlePcj.i)((0, _ioDnGGugrj.i)());
}
//#endregion
...
function addEntryArtifactPathsToSet(params) {
  ...
  for (const key of params.keys) {
    ...
    if (!entry) continue;
    for (const artifactPath of (0, _storeCmwzJ9N.y)({
      sessionsDir,
      entry
...
async function previewStoreCleanup(params) {
  const beforeStore = (0, _storeLoadDjaY7SAt.t)(params.target.storePath, { skipCache: true });
  const previewStore = (0, _storeLoadDjaY7SAt.c)(beforeStore);
  ...
  const preserveSessionKeys = (0, _storeLoadDjaY7SAt.i)([params.activeKey]);
  const pruned = (0, _storeLoadDjaY7SAt.v)(previewStore, params.maintenance.pruneAfterMs, {
    log: false,
  ...
  const capped = (0, _storeLoadDjaY7SAt.h)(previewStore, params.maintenance.maxEntries, {
    log: false,
    preserveKeys: preserveSessionKeys,
    onCapped: ({ key }) => {
      cappedKeys.add(key);
  ...
  const beforeBudgetStore = (0, _storeLoadDjaY7SAt.c)(previewStore);
  const budgetRemovedFilePaths = /* @__PURE__ */new Set();
  const diskBudget = await (0, _storeCmwzJ9N._)({
    store: previewStore,
    storePath: params.target.storePath,
  ...
  const unreferencedArtifacts = await (0, _storeCmwzJ9N.v)({
    store: previewStore,
    storePath: params.target.storePath,
...
async function runSessionsCleanup(params) {
  ...
  const maintenance = (0, _storeLoadDjaY7SAt.r)();
  const mode = opts.enforce ? "enforce" : maintenance.mode;
  const targets = params.targets ?? (0, _targetsDGxp4nvt.a)(cfg, {
    store: opts.store,
    agent: opts.agent,
  ...
  if (!opts.dryRun) for (const target of targets) {
    ...
    await (0, _storeCmwzJ9N.s)(target.storePath, async (store) => {
      let removed = 0;
      if (opts.fixMissing) {
    ...
    if (dmScopeRemovedSessionFiles.size > 0) {
      const storeAfterDmScopeRetire = (0, _storeLoadDjaY7SAt.t)(target.storePath, { skipCache: true });
      await (0, _storeCmwzJ9N.t)({
    ...
    const afterStore = (0, _storeLoadDjaY7SAt.t)(target.storePath, { skipCache: true });
    const unreferencedArtifacts = mode === "warn" ? {
      scannedFiles: 0,
      removedFiles: 0,
      freedBytes: 0,
      olderThanMs: maintenance.pruneAfterMs
    } : await (0, _storeCmwzJ9N.v)({
      store: afterStore,
      storePath: target.storePath,
      olderThanMs: maintenance.pruneAfterMs,
      dryRun: false
...
async function purgeAgentSessionStoreEntries(cfg, agentId) {
  try {
    ...
    const storeAgentId = typeof storeConfig === "string" && storeConfig.includes("{agentId}") ? normalizedAgentId : (0, _sessionKeyCQewiu8n.c)((0, _agentScopeConfigDUwbsKjv.c)(cfg));
    await (0, _storeCmwzJ9N.s)((0, _pathsRbVDOqFp.u)(cfg.session?.store, { agentId: normalizedAgentId }), (store) => {
      for (const key of Object.keys(store)) if ((0, _combinedStoreGatewayDTxOYXKu.s)({
        cfg,
    ...
    (0, _loggerApTu_tEr.a)().debug("session store purge skipped during agent delete", err);
  }
}
//#endregion /* v9-66537cb240f01db7 */
