...
function toDiscoveryApiKey(value) {
  ...
  if (!trimmed || (0, _modelAuthMarkersCiLEGnXI.u)(trimmed)) return;
  return trimmed;
}
function resolveEnvApiKeyVarName(provider, env = process.env) {
  const resolved = (0, _modelAuthEnvCviUIkdl.t)(provider, env);
  if (!resolved) return;
...
function normalizeHeaderValues(params) {
  ...
  for (const [headerName, headerValue] of Object.entries(headers)) {
    ...
    mutated = true;
    nextHeaders[headerName] = resolvedRef.source === "env" ? (0, _modelAuthMarkersCiLEGnXI.m)(resolvedRef.id) : (0, _modelAuthMarkersCiLEGnXI.g)(resolvedRef.source);
  }
  if (!mutated) return {
    headers,
    mutated: false
...
function resolveApiKeyFromCredential(cred, env = process.env) {
  ...
  if (cred.type === "api_key") {
    ...
    if (keyRef && keyRef.id.trim()) {
      ...
      return {
        apiKey: (0, _modelAuthMarkersCiLEGnXI.h)(keyRef.source),
        source: "non-env-ref"
      };
    }
  ...
  if (cred.type === "token") {
    ...
    if (tokenRef && tokenRef.id.trim()) {
      ...
      return {
        apiKey: (0, _modelAuthMarkersCiLEGnXI.h)(tokenRef.source),
        source: "non-env-ref"
      };
    }
...
function normalizeConfiguredProviderApiKey(params) {
  ...
  if (configuredApiKeyRef && configuredApiKeyRef.id.trim()) {
    const marker = configuredApiKeyRef.source === "env" ? configuredApiKeyRef.id.trim() : (0, _modelAuthMarkersCiLEGnXI.h)(configuredApiKeyRef.source);
  ...
  if ((0, _modelAuthMarkersCiLEGnXI.u)(normalizedConfiguredApiKey)) params.secretRefManagedProviders?.add(params.providerKey);
...
function createProviderApiKeyResolver(env, authStoreInput, config) {
  return (provider) => {
    const authProvider = (0, _providerAuthAliasesCljpontN.r)(provider, {
      config,
      env
    });
...
function resolveConfigBackedProviderAuth(params) {
  const authProvider = (0, _providerAuthAliasesCljpontN.r)(params.provider, { config: params.config });
  const apiKey = (0, _providerRuntimeBzn5r2hY.N)({
    provider: authProvider,
  ...
  if (apiKey) return (0, _modelAuthMarkersCiLEGnXI.u)(apiKey) ? {
    apiKey,
    discoveryApiKey: toDiscoveryApiKey(apiKey),
    mode: "api_key",
    source: "config"
  } : {
    apiKey: (0, _modelAuthMarkersCiLEGnXI.h)("file"),
    discoveryApiKey: toDiscoveryApiKey(apiKey),
    mode: "api_key",
    source: "config"
  };
  ...
  if (configuredApiKeyRef) {
    ...
    return {
      apiKey: (0, _modelAuthMarkersCiLEGnXI.h)(configuredApiKeyRef.source),
      discoveryApiKey: void 0,
      mode: "api_key",
      source: "config"
    };
  ...
  if ((0, _modelAuthMarkersCiLEGnXI.l)(configuredApiKey)) {
  ...
  return (0, _modelAuthMarkersCiLEGnXI.u)(configuredApiKey) ? {
    apiKey: configuredApiKey,
