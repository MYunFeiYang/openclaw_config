...
function resolveConsoleSettings() {
  const envLevel = (0, _loggerApTu_tEr.h)();
  if (process.env.VITEST === "true" && process.env.OPENCLAW_TEST_CONSOLE !== "1" && !(0, _loggerApTu_tEr.b)() && !envLevel && !_stateCiZRzmxP.t.overrideSettings) return {
    level: "silent",
...
function formatConsoleTimestamp(style) {
  ...
  if (style === "pretty") return (0, _loggerApTu_tEr.p)(now, { style: "short" }).replace(/[+-]\d{2}:\d{2}$/, "");
  return (0, _loggerApTu_tEr.f)(now);
}
...
function enableConsoleCapture() {
  ...
  let logger = null;
  const getLoggerLazy = () => {
    if (!logger) logger = (0, _loggerApTu_tEr.a)();
    return logger;
  };
  const original = {
    log: console.log,
