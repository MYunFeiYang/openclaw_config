...
function buildManifestCatalogModel(providerId, model) {
  ...
  const id = (0, _modelRefSharedDTXcmZXB.r)(providerId, model.id, { allowManifestNormalization: false });
  return {
    id,
    name: model.name ?? id,
...
function readConfiguredProviderCatalogEntries(params) {
  ...
  for (const model of models) {
    ...
    if (!id) continue;
    const normalizedId = (0, _modelRefSharedDTXcmZXB.r)(provider, id);
...
function supportsNativeStreamingUsageCompat(params) {
  return (0, _providerAttributionDWAuHLb.r)({
