...
var _providerAuthAliasesCljpontN = require("./provider-auth-aliases-CljpontN.js");
var _fileLockCCOJxG = require("./file-lock-CCOJxG89.js");
require("./file-lock-n2tj_mjJ.js");
require("./config-CR6nsEzH.js");
var _runtimeSnapshotsCbKCG9ZW = require("./runtime-snapshots-CbKCG9ZW.js");
var _storeCacheCiDn8a = require("./store-cache-CiDn-8a7.js");
...
var _chutesOauthCircnLYE = require("./chutes-oauth-CircnLYE.js");
var _policyByTJx0He = require("./policy-ByTJx0He.js");
var _profilesDdWlNiEB = require("./profiles-DdWlNiEB.js");
var _repairDbkwHVtZ = require("./repair-DbkwHVtZ.js");
...
function loadStoredOAuthRefreshStore(agentDir) {
  return (0, _storeBYnnXRZ.l)(agentDir, {
...
async function loadFreshStoredOAuthCredential(params) {
  ...
  if (reloaded?.type !== "oauth" || reloaded.provider !== params.provider || !(0, _storeBYnnXRZ.D)(reloaded)) return null;
...
function resolveEffectiveOAuthCredential(params) {
  ...
  if ((0, _storeBYnnXRZ.D)(params.credential)) {
    _storeCacheCiDn8a.f.debug("resolved oauth credential from canonical local store", {
      profileId: params.profileId,
  ...
  if (!(0, _storeBYnnXRZ.O)(params.credential, imported)) {
    _storeCacheCiDn8a.f.warn("refused external oauth bootstrap credential: identity mismatch or missing binding", {
      profileId: params.profileId,
      provider: params.credential.provider
    });
    return params.credential;
  }
  if ((0, _storeBYnnXRZ.A)({
    existing: params.credential,
    imported
  })) {
    _storeCacheCiDn8a.f.debug("resolved oauth credential from external cli bootstrap", {
      profileId: params.profileId,
...
function createOAuthManager(adapter) {
  function adoptNewerMainOAuthCredential(params) {
    ...
    try {
      const mainCred = (0, _storeBYnnXRZ.i)(void 0, { allowKeychainPrompt: false }).profiles[params.profileId];
      if (mainCred?.type === "oauth" && mainCred.provider === params.credential.provider && (0, _storeBYnnXRZ.D)(mainCred) && Number.isFinite(mainCred.expires) && (!Number.isFinite(params.credential.expires) || mainCred.expires > params.credential.expires) && (0, _storeBYnnXRZ.k)(params.credential, mainCred)) {
        params.store.profiles[params.profileId] = { ...mainCred };
        _storeCacheCiDn8a.f.info("adopted newer OAuth credentials from main agent", {
          profileId: params.profileId,
      ...
      _storeCacheCiDn8a.f.debug("adoptNewerMainOAuthCredential failed", {
        profileId: params.profileId,
  ...
  async function mirrorRefreshedCredentialIntoMainStore(params) {
    try {
      (0, _storeBYnnXRZ.S)((0, _runtimeSnapshotsCbKCG9ZW.l)(void 0));
      await (0, _storeBYnnXRZ.p)({
        agentDir: void 0,
        updater: (store) => {
          ...
          if (!decision.shouldMirror) {
            if (decision.reason === "identity-mismatch-or-regression") _storeCacheCiDn8a.f.warn("refused to mirror OAuth credential: identity mismatch or regression", { profileId: params.profileId });
            return false;
          }
          store.profiles[params.profileId] = { ...params.refreshed };
          _storeCacheCiDn8a.f.debug("mirrored refreshed OAuth credential to main agent store", {
            profileId: params.profileId,
      ...
      _storeCacheCiDn8a.f.debug("mirrorRefreshedCredentialIntoMainStore failed", {
        profileId: params.profileId,
        error: (0, _errorsIxwfrboQ.i)(err)
      });
    }
  ...
  async function doRefreshOAuthTokenWithLock(params) {
    const ownerAgentDir = (0, _storeBYnnXRZ.d)(params);
    const authPath = (0, _runtimeSnapshotsCbKCG9ZW.l)(ownerAgentDir);
    (0, _storeBYnnXRZ.S)(authPath);
    const globalRefreshLockPath = (0, _runtimeSnapshotsCbKCG9ZW.f)(params.provider, params.profileId);
    try {
      return await (0, _fileLockCCOJxG.o)(globalRefreshLockPath, _storeCacheCiDn8a.u, async () => (0, _fileLockCCOJxG.o)(authPath, _storeCacheCiDn8a.i, async () => {
        const store = loadStoredOAuthRefreshStore(ownerAgentDir);
        const cred = store.profiles[params.profileId];
        if (!cred || cred.type !== "oauth") return null;
        let credentialToRefresh = cred;
        if (!params.forceRefresh && (0, _storeBYnnXRZ.D)(cred)) return {
        ...
        if (params.agentDir) try {
          ...
          if (mainCred?.type === "oauth" && mainCred.provider === cred.provider && (0, _storeBYnnXRZ.D)(mainCred) && !params.forceRefresh && (0, _storeBYnnXRZ.k)(cred, mainCred)) {
            store.profiles[params.profileId] = { ...mainCred };
            _storeCacheCiDn8a.f.info("adopted fresh OAuth credential from main store (under refresh lock)", {
              profileId: params.profileId,
              agentDir: params.agentDir,
              expires: new Date(mainCred.expires).toISOString()
            });
          ...
          } else if (mainCred?.type === "oauth" && mainCred.provider === cred.provider && (0, _storeBYnnXRZ.D)(mainCred) && !(0, _storeBYnnXRZ.k)(cred, mainCred)) _storeCacheCiDn8a.f.warn("refused to adopt fresh main-store OAuth credential: identity mismatch", {
            profileId: params.profileId,
            agentDir: params.agentDir
          });
        } catch (err) {
          _storeCacheCiDn8a.f.debug("inside-lock main-store adoption failed; proceeding to refresh", {
            profileId: params.profileId,
            error: (0, _errorsIxwfrboQ.i)(err)
          });
        }
        ...
        if (externallyManaged) if (externallyManaged.provider !== cred.provider) _storeCacheCiDn8a.f.warn("refused external oauth bootstrap credential: provider mismatch", {
          profileId: params.profileId,
          provider: cred.provider
        });else
        if (!(0, _storeBYnnXRZ.O)(cred, externallyManaged)) _storeCacheCiDn8a.f.warn("refused external oauth bootstrap credential: identity mismatch or missing binding", {
          profileId: params.profileId,
          provider: cred.provider
        });else
        {
          if ((0, _storeBYnnXRZ.j)(cred, externallyManaged) && !(0, _storeBYnnXRZ.T)(cred, externallyManaged)) {
            store.profiles[params.profileId] = { ...externallyManaged };
            (0, _storeBYnnXRZ.f)(store, ownerAgentDir);
          }
          credentialToRefresh = externallyManaged;
          if (!params.forceRefresh && (0, _storeBYnnXRZ.D)(externallyManaged)) return {
        ...
        const refreshedCredentials = await withRefreshCallTimeout(`refreshOAuthCredential(${cred.provider})`, _storeCacheCiDn8a.l, async () => {
        ...
        (0, _storeBYnnXRZ.f)(store, ownerAgentDir);
        if (ownerAgentDir) {
          if ((0, _runtimeSnapshotsCbKCG9ZW.l)(void 0) !== authPath) await mirrorRefreshedCredentialIntoMainStore({
            profileId: params.profileId,
  ...
  async function resolveOAuthAccess(params) {
    ...
    if (!params.forceRefresh && (0, _storeBYnnXRZ.D)(effectiveCredential)) return {
    ...
    try {
      ...
      if (refreshed?.type === "oauth" && (0, _storeBYnnXRZ.D)(refreshed) && canReuseOAuthCredentialAfterRefreshFailure({
        forceRefresh: params.forceRefresh,
      ...
      if (params.agentDir) try {
        const mainCred = (0, _storeBYnnXRZ.i)(void 0, { allowKeychainPrompt: false }).profiles[params.profileId];
        if (mainCred?.type === "oauth" && mainCred.provider === params.credential.provider && (0, _storeBYnnXRZ.D)(mainCred) && canReuseOAuthCredentialAfterRefreshFailure({
          forceRefresh: params.forceRefresh,
          attempted: effectiveCredential,
          candidate: mainCred
        }) && (0, _storeBYnnXRZ.k)(params.credential, mainCred)) {
          refreshedStore.profiles[params.profileId] = { ...mainCred };
          _storeCacheCiDn8a.f.info("inherited fresh OAuth credentials from main agent", {
            profileId: params.profileId,
            agentDir: params.agentDir,
            expires: new Date(mainCred.expires).toISOString()
          });
      ...
      if (fallback && fallback.provider === params.credential.provider && (0, _storeBYnnXRZ.D)(fallback) && (0, _storeBYnnXRZ.E)(params.credential, fallback) && canReuseOAuthCredentialAfterRefreshFailure({
        forceRefresh: params.forceRefresh,
        attempted: effectiveCredential,
        candidate: fallback
      })) {
        _storeCacheCiDn8a.f.info("using external OAuth credential after refresh failure", {
          profileId: params.profileId,
...
async function refreshOAuthCredential(credential) {
  ...
  if (credential.provider === "chutes") return await (0, _chutesOauthCircnLYE.a)({ credential });
...
const oauthManager = createOAuthManager({
  ...
  readBootstrapCredential: ({ profileId, credential }) => (0, _storeBYnnXRZ.w)({
    profileId,
    credential
  }),
  readFallbackCredential: ({ profileId, credential }) => credential.provider === "openai-codex" ? (0, _storeBYnnXRZ.C)({
    profileId,
    credential,
...
async function resolveProfileSecretString(params) {
  ...
  if (resolvedValue) {
    ...
    if (inlineRef) try {
      ...
      _storeCacheCiDn8a.f.debug(params.inlineFailureMessage, {
        profileId: params.profileId,
        provider: params.provider,
        error: (0, _errorsIxwfrboQ.i)(err)
      });
  ...
  if (!resolvedValue && explicitRef) try {
    ...
    _storeCacheCiDn8a.f.debug(params.refFailureMessage, {
      profileId: params.profileId,
      provider: params.provider,
      error: (0, _errorsIxwfrboQ.i)(err)
    });
...
async function resolveApiKeyForProfile(params) {
  ...
  const configForRefResolution = cfg ?? (0, _ioDnGGugrj.i)();
  ...
  if (cred.type === "token") {
    const expiryState = (0, _storeBYnnXRZ.F)(cred.expires);
  ...
  try {
    ...
    let refreshedStore = error instanceof OAuthManagerRefreshError ? error.getRefreshedStore() : (0, _storeBYnnXRZ.c)(params.agentDir);
    ...
    if (isRefreshTokenReusedError(surfacedCause)) {
      const ownerAgentDir = (0, _storeBYnnXRZ.d)({
        agentDir: params.agentDir,
        profileId
      });
      await (0, _profilesDdWlNiEB.t)({
        provider: cred.provider,
        profileId,
        agentDir: ownerAgentDir
      });
      if (params.agentDir !== ownerAgentDir && (0, _runtimeSnapshotsCbKCG9ZW.i)(params.agentDir)) {
        const snapshot = (0, _runtimeSnapshotsCbKCG9ZW.n)(params.agentDir);
        const providerKey = (0, _providerAuthAliasesCljpontN.r)(cred.provider);
        if (snapshot?.lastGood?.[providerKey] === profileId) {
          ...
          (0, _runtimeSnapshotsCbKCG9ZW.o)(snapshot, params.agentDir);
        }
      }
      refreshedStore = (0, _storeBYnnXRZ.c)(params.agentDir);
    }
    const fallbackProfileId = (0, _repairDbkwHVtZ.n)({
      cfg,
