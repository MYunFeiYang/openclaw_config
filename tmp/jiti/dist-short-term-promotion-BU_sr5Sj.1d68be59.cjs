"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = rankShortTermPromotionCandidates;exports.c = recordGroundedShortTermCandidates;exports.d = repairShortTermPromotionArtifacts;exports.f = resolveShortTermRecallLockPath;exports.i = filterLiveShortTermRecallEntries;exports.l = recordShortTermRecalls;exports.n = applyShortTermPromotions;exports.o = readShortTermRecallEntries;exports.p = resolveShortTermRecallStorePath;exports.r = auditShortTermPromotionArtifacts;exports.s = recordDreamingPhaseSignals;exports.t = void 0;exports.u = removeGroundedShortTermCandidates;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
}).flatMap((words) => words).map((word) => (0, _stringCoerceBje8XVt.a)(word)));
...
"게이트웨이",
"장애대응"].
map((word) => (0, _stringCoerceBje8XVt.a)(word.normalize("NFKC")));
...
function normalizeConceptToken(rawToken) {
  const normalized = (0, _stringCoerceBje8XVt.a)(rawToken.normalize("NFKC").replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu, "").replaceAll("_", "-"));
...
function collectGlossaryMatches(source) {
  const normalizedSource = (0, _stringCoerceBje8XVt.a)(source.normalize("NFKC"));
  const matches = [];
...
function deriveConceptTags(params) {
  const source = `${_nodePath.default.basename(params.path)} ${params.snippet}`;
...
function summarizeConceptTagScriptCoverage(conceptTagsByEntry) {
  ...
  for (const conceptTags of conceptTagsByEntry) {
    ...
    if (hasLatin && hasCjk) coverage.mixedEntryCount += 1;else
    if (hasCjk) coverage.cjkEntryCount += 1;else
    if (hasLatin) coverage.latinEntryCount += 1;else
    if (hasOther) coverage.otherEntryCount += 1;
  }
  return coverage;
}
//#endregion
...
const DEFAULT_PROMOTION_MIN_SCORE = exports.t = .75;
const PROMOTION_MARKER_PREFIX = "openclaw-memory-promotion:";
const MAX_QUERY_HASHES = 32;
const MAX_RECALL_DAYS = 16;
const SHORT_TERM_STORE_RELATIVE_PATH = _nodePath.default.join("memory", ".dreams", "short-term-recall.json");
const SHORT_TERM_PHASE_SIGNAL_RELATIVE_PATH = _nodePath.default.join("memory", ".dreams", "phase-signals.json");
const SHORT_TERM_LOCK_RELATIVE_PATH = _nodePath.default.join("memory", ".dreams", "short-term-promotion.lock");
...
function buildClaimHash(snippet) {
  return (0, _nodeCrypto.createHash)("sha1").update(normalizeSnippet(snippet)).digest("hex").slice(0, 12);
}
...
function hashQuery(query) {
  return (0, _nodeCrypto.createHash)("sha1").update((0, _stringCoerceBje8XVt.a)(query)).digest("hex").slice(0, 12);
}
...
function normalizeStore(raw, nowIso) {
  ...
  if (entriesRaw && typeof entriesRaw === "object") for (const [key, value] of Object.entries(entriesRaw)) {
    ...
    const conceptTags = Array.isArray(entry.conceptTags) ? normalizeDistinctStrings(entry.conceptTags.map((tag) => typeof tag === "string" ? (0, _stringCoerceBje8XVt.a)(tag) : tag), 8) : deriveConceptTags({
      path: entryPath,
      snippet
    });
...
function resolveStorePath(workspaceDir) {
  return _nodePath.default.join(workspaceDir, SHORT_TERM_STORE_RELATIVE_PATH);
}
function resolvePhaseSignalPath(workspaceDir) {
  return _nodePath.default.join(workspaceDir, SHORT_TERM_PHASE_SIGNAL_RELATIVE_PATH);
}
function resolveLockPath(workspaceDir) {
  return _nodePath.default.join(workspaceDir, SHORT_TERM_LOCK_RELATIVE_PATH);
}
function resolveShortTermArtifactsDir(workspaceDir) {
  return _nodePath.default.dirname(resolveLockPath(workspaceDir));
}
async function ensureShortTermArtifactsDir(workspaceDir) {
  ...
  const ensuring = _promises.default.mkdir(artifactsDir, { recursive: true }).then(() => void 0).catch((err) => {
...
async function canStealStaleLock(lockPath) {
  const ownerPid = await _promises.default.readFile(lockPath, "utf-8").then((raw) => parseLockOwnerPid(raw)).catch(() => null);
...
async function withShortTermLock(workspaceDir, task) {
  ...
  return withInProcessShortTermLock(lockPath, async () => {
    ...
    while (true) try {
      const lockHandle = await _promises.default.open(lockPath, "wx");
      await lockHandle.writeFile(`${process.pid}:${Date.now()}\n`, "utf-8").catch(() => void 0);
      try {
        return await task();
      } finally {
        await lockHandle.close().catch(() => void 0);
        await _promises.default.unlink(lockPath).catch(() => void 0);
      }
    } catch (err) {
      if (err?.code !== "EEXIST") throw err;
      if ((await _promises.default.stat(lockPath).then((stats) => Date.now() - stats.mtimeMs).catch(() => 0)) > SHORT_TERM_LOCK_STALE_MS) {
...
async function readStore(workspaceDir, nowIso) {
  ...
  try {
    const raw = await _promises.default.readFile(storePath, "utf-8");
...
function normalizePhaseSignalStore(raw, nowIso) {
  ...
  const entriesRaw = (0, _recordCoerceZvIvRO5e.n)(record?.entries);
  if (!entriesRaw) return emptyPhaseSignalStore(nowIso);
  const entries = {};
  for (const [mapKey, value] of Object.entries(entriesRaw)) {
    const entry = (0, _recordCoerceZvIvRO5e.n)(value);
    if (!entry) continue;
...
async function readPhaseSignalStore(workspaceDir, nowIso) {
  ...
  try {
    const raw = await _promises.default.readFile(phaseSignalPath, "utf-8");
...
async function writePhaseSignalStore(workspaceDir, store) {
  const phaseSignalPath = resolvePhaseSignalPath(workspaceDir);
  await ensureShortTermArtifactsDir(workspaceDir);
  const tmpPath = `${phaseSignalPath}.${process.pid}.${Date.now()}.${(0, _nodeCrypto.randomUUID)()}.tmp`;
  await _promises.default.writeFile(tmpPath, `${JSON.stringify(store, null, 2)}\n`, "utf-8");
  await _promises.default.rename(tmpPath, phaseSignalPath);
}
async function writeStore(workspaceDir, store) {
  const storePath = resolveStorePath(workspaceDir);
  await ensureShortTermArtifactsDir(workspaceDir);
  const tmpPath = `${storePath}.${process.pid}.${Date.now()}.${(0, _nodeCrypto.randomUUID)()}.tmp`;
  await _promises.default.writeFile(tmpPath, `${JSON.stringify(store, null, 2)}\n`, "utf-8");
  await _promises.default.rename(tmpPath, storePath);
}
...
async function shortTermRecallSourceExists(params) {
  ...
  for (const sourcePath of resolveShortTermSourcePathCandidates(workspaceDir, params.entry.path)) try {
    if ((await _promises.default.stat(sourcePath)).isFile()) return true;
  } catch (err) {
...
async function recordShortTermRecalls(params) {
  ...
  const todayBucket = normalizeIsoDay(params.dayBucket ?? "") ?? (0, _dreamingCUzWMDhQ.V)(nowMs, params.timezone);
  await withShortTermLock(workspaceDir, async () => {
    ...
    await (0, _eventsDLaYFGKU.n)(workspaceDir, {
...
async function recordGroundedShortTermCandidates(params) {
  ...
  const nowMs = Number.isFinite(params.nowMs) ? params.nowMs : Date.now();
  const nowIso = new Date(nowMs).toISOString();
  const fallbackDayBucket = (0, _dreamingCUzWMDhQ.V)(nowMs, params.timezone);
  await withShortTermLock(workspaceDir, async () => {
    const store = await readStore(workspaceDir, nowIso);
...
function resolveShortTermSourcePathCandidates(workspaceDir, candidatePath) {
  ...
  if (!normalizedPath.startsWith("memory/")) basenames.push(_nodePath.default.posix.join("memory", _nodePath.default.posix.basename(normalizedPath)));
  const seen = /* @__PURE__ */new Set();
  const resolved = [];
  for (const relativePath of basenames) {
    const absolutePath = _nodePath.default.resolve(workspaceDir, relativePath);
...
async function rehydratePromotionCandidate(workspaceDir, candidate) {
  ...
  for (const sourcePath of sourcePaths) {
    let rawSource;
    try {
      rawSource = await _promises.default.readFile(sourcePath, "utf-8");
    } catch (err) {
      if (err?.code === "ENOENT") continue;
      throw err;
    }
...
function buildPromotionSection(candidates, nowMs, timezone) {
  const lines = [
  "",
  `## Promoted From Short-Term Memory (${(0, _dreamingCUzWMDhQ.V)(nowMs, timezone)})`,
  ""];
...
async function applyShortTermPromotions(options) {
  ...
  const minUniqueQueries = toFiniteNonNegativeInt(options.minUniqueQueries, 2);
  const maxAgeDays = toFiniteNonNegativeInt(options.maxAgeDays, -1);
  const memoryPath = _nodePath.default.join(workspaceDir, "MEMORY.md");
  return await withShortTermLock(workspaceDir, async () => {
    ...
    const existingMemory = await _promises.default.readFile(memoryPath, "utf-8").catch((err) => {
    ...
    if (toAppend.length > 0) {
      ...
      await _promises.default.writeFile(memoryPath, `${header}${withTrailingNewline(existingMemory)}${section}`, "utf-8");
    }
...
async function auditShortTermPromotionArtifacts(params) {
  ...
  try {
    ...
    });else
    {
      ...
      invalidEntryCount = Object.keys((0, _recordCoerceZvIvRO5e.n)(parsed)?.entries ?? {}).length - entryCount;
  ...
  if (params.qmd) {
    ...
    if (dbPath) try {
      const stat = await _promises.default.stat(dbPath);
...
async function repairShortTermPromotionArtifacts(params) {
  ...
  await withShortTermLock(workspaceDir, async () => {
    ...
    try {
      ...
      const rawEntries = Object.keys((0, _recordCoerceZvIvRO5e.n)(parsed)?.entries ?? {}).length;
