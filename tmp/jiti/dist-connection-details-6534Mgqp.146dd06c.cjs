"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = buildGatewayConnectionDetailsWithResolvers;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _pathsB2cMKWd = require("./paths-B2cMK-wd.js");
var _netDDjdGYkE = require("./net-DDjdGYkE.js");
...
function buildGatewayConnectionDetailsWithResolvers(options = {}, resolvers = {}) {
  ...
  const configPath = options.configPath ?? resolvers.resolveConfigPath?.(process.env) ?? (0, _pathsB2cMKWd.o)(process.env);
  const isRemoteMode = config.gateway?.mode === "remote";
  const remote = isRemoteMode ? config.gateway?.remote : void 0;
  const tlsEnabled = config.gateway?.tls?.enabled === true;
  const localPort = resolvers.resolveGatewayPort?.(config, process.env) ?? (0, _pathsB2cMKWd.u)(config);
  const bindMode = config.gateway?.bind ?? "loopback";
  const localUrl = `${tlsEnabled ? "wss" : "ws"}://127.0.0.1:${localPort}`;
  const cliUrlOverride = (0, _stringCoerceBje8XVt.c)(options.url);
  const envUrlOverride = cliUrlOverride ? void 0 : (0, _stringCoerceBje8XVt.c)(process.env.OPENCLAW_GATEWAY_URL);
  const urlOverride = cliUrlOverride ?? envUrlOverride;
  const remoteUrl = (0, _stringCoerceBje8XVt.c)(remote?.url);
  ...
  if (!(0, _netDDjdGYkE.s)(url, { allowPrivateWs })) throw new Error([
