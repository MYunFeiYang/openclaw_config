...
var _providerAuthMode78iCs67F = require("./provider-auth-mode-78iCs67F.js");
var _providerAuthRefBAu07q9m = require("./provider-auth-ref-BAu07q9m.js");
...
async function ensureApiKeyFromEnvOrPrompt(params) {
  const selectedMode = await (0, _providerAuthMode78iCs67F.t)({
    prompter: params.prompter,
    explicitMode: params.secretInputMode
  });
  const env = params.env ?? process.env;
  const envKey = (0, _modelAuthEnvCviUIkdl.t)(params.provider, env);
  if (selectedMode === "ref") {
    if (typeof params.prompter.select !== "function") {
      const fallback = (0, _providerAuthRefBAu07q9m.r)({
        config: params.config,
        provider: params.provider,
        preferredEnvVar: envKey?.source ? (0, _providerAuthRefBAu07q9m.t)(envKey.source) : void 0,
        env
      });
      await params.setCredential(fallback.ref, selectedMode);
      return fallback.resolvedValue;
    }
    const resolved = await (0, _providerAuthRefBAu07q9m.n)({
      provider: params.provider,
      config: params.config,
      prompter: params.prompter,
      preferredEnvVar: envKey?.source ? (0, _providerAuthRefBAu07q9m.t)(envKey.source) : void 0,
