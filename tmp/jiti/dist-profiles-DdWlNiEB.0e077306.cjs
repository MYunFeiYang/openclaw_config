...
async function setAuthProfileOrder(params) {
  ...
  const deduped = (0, _profileListCdhQb6vz.t)(params.order && Array.isArray(params.order) ? (0, _stringNormalizationDgUPESoD.s)(params.order) : []);
  return await (0, _storeBYnnXRZ.p)({
...
async function promoteAuthProfileInOrder(params) {
  const providerKey = (0, _providerAuthAliasesCljpontN.r)(params.provider);
  return await (0, _storeBYnnXRZ.p)({
    agentDir: params.agentDir,
    updater: (store) => {
      const profile = store.profiles[params.profileId];
      if (!profile || (0, _providerAuthAliasesCljpontN.r)(profile.provider) !== providerKey) return false;
      ...
      const next = (0, _profileListCdhQb6vz.t)([params.profileId, ...existing.filter((profileId) => profileId !== params.profileId)]);
...
function upsertAuthProfile(params) {
  ...
  const store = (0, _storeBYnnXRZ.r)(params.agentDir);
  store.profiles[params.profileId] = credential;
  (0, _storeBYnnXRZ.f)(store, params.agentDir, {
...
async function removeProviderAuthProfilesWithLock(params) {
  ...
  return await (0, _storeBYnnXRZ.p)({
    agentDir: params.agentDir,
    updater: (store) => {
      const profileIds = (0, _profileListCdhQb6vz.n)(store, params.provider);
      let changed = false;
...
async function markAuthProfileSuccess(params) {
  ...
  const updated = await (0, _storeBYnnXRZ.p)({
    agentDir,
    updater: (freshStore) => {
  ...
  if (!profile || (0, _providerAuthAliasesCljpontN.r)(profile.provider) !== providerKey) return;
  store.lastGood = {
    ...store.lastGood,
    [providerKey]: profileId
  };
  updateSuccessfulUsageStatsEntry(store, profileId, lastUsed);
  (0, _storeBYnnXRZ.f)(store, agentDir);
}
//#endregion /* v9-6a5630756071459f */
