...
var _hostnameDS2VIhf = require("./hostname-DS-2VIhf.js");
...
function normalizeSsrFPolicyHostnames(values) {
  ...
  return Array.from(new Set(values.map((value) => (0, _hostnameDS2VIhf.t)(value)).filter(Boolean))).toSorted();
}
...
function normalizeHostnameSet(values) {
  ...
  return new Set(values.map((value) => (0, _hostnameDS2VIhf.t)(value)).filter(Boolean));
}
function normalizeHostnameAllowlist(values) {
  if (!values || values.length === 0) return [];
  return Array.from(new Set(values.map((value) => (0, _hostnameDS2VIhf.t)(value)).filter((value) => value !== "*" && value !== "*." && value.length > 0)));
}
...
function resolveSsrFPolicyForUrl(url, policy) {
  ...
  return {
    ...policy,
    allowedHostnames: Array.from(new Set([...(policy.allowedHostnames ?? []), (0, _hostnameDS2VIhf.t)(url.hostname)]))
  };
}
...
function isPrivateIpAddress(address, policy) {
  const normalized = (0, _hostnameDS2VIhf.t)(address);
...
function isBlockedHostname(hostname) {
  const normalized = (0, _hostnameDS2VIhf.t)(hostname);
  if (!normalized) return false;
...
function createPinnedLookup(params) {
  const normalizedHost = (0, _hostnameDS2VIhf.t)(params.hostname);
  ...
  return (host, options, callback) => {
    ...
    if (!cb) return;
    const normalized = (0, _hostnameDS2VIhf.t)(host);
...
function resolvePinnedDispatcherLookup(pinned, override, policy) {
  ...
  const normalizedOverrideHost = (0, _hostnameDS2VIhf.t)(override.hostname);
