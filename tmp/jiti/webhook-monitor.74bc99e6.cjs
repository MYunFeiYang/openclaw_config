"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.handleEnterChat = handleEnterChat;exports.handleInboundMessage = handleInboundMessage;exports.handleStreamRefresh = handleStreamRefresh;exports.handleTemplateCardEvent = handleTemplateCardEvent;exports.startAgentForStream = startAgentForStream;
...
var _apiClient = require("../agent/api-client.js");
var _http = require("./http.js");
var _helpers = require("./helpers.js");
var _dynamicRouting = require("../dynamic-routing.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);} /**
 * Webhook 核心消息处理
 *
 ...
 */async function handleInboundMessage(target, message, timestamp, nonce, proxyUrl, msgFilterData) {const state = (0, _gateway.getMonitorState)();const { streamStore, activeReplyStore } = state;const msgid = message.msgid;
  // msgid 去重检查
  if (msgid) {
    ...
    if (existingStreamId) {
      ...
      if (existingStream) {
        ...
        return (0, _helpers.buildStreamResponse)(existingStream);
      }
    }
  }
  // 解析消息内容（对齐原版 buildInboundBody）
  const msgContent = (0, _helpers.buildInboundBody)(message);
  if (!msgContent && !(0, _helpers.hasMedia)(message)) {
  ...
  if (status === "active_new") {
    // 第一条消息，返回默认占位符
    return (0, _helpers.buildStreamPlaceholderReply)(streamId, defaultPlaceholder);
  }
  if (status === "queued_new") {
    // 进入排队批次，返回排队提示
    target.runtime.log?.(`[webhook] queue: 已进入下一批次 streamId=${streamId} msgid=${String(message.msgid ?? "")}`);
    return (0, _helpers.buildStreamPlaceholderReply)(streamId, queuedPlaceholder);
  }
  ...
  return (0, _helpers.buildStreamTextPlaceholderReply)(ackStreamId, mergedQueuedPlaceholder);
}
// ============================================================================
// stream_refresh 处理
// ============================================================================
 ...
 */
async function handleStreamRefresh(target, message) {
  const state = (0, _gateway.getMonitorState)();
  ...
  return (0, _helpers.buildStreamResponse)(stream);
}
// ============================================================================
// enter_chat 处理
// ============================================================================
 ...
 * 返回可配置的欢迎消息。
 */
async function handleEnterChat(target, message) {
 ...
 */
async function handleTemplateCardEvent(target, message, timestamp, nonce, proxyUrl) {
  const state = (0, _gateway.getMonitorState)();
...
async function startAgentForStream(params) {
  ...
  const userid = (0, _helpers.resolveWecomSenderUserId)(msg) || "unknown";
  const chatType = msg.chattype === "group" ? "group" : "direct";
  const chatId = msg.chattype === "group" ? msg.chatid?.trim() || "unknown" : userid;
  const taskKey = (0, _helpers.computeTaskKey)(target, msg);
  ...
  // ──────────────────────────────────────────────────────────────────
  let { body: rawBody, media } = await (0, _helpers.processInboundMessage)(target, msg);
  ...
  // ──────────────────────────────────────────────────────────────────
  const directLocalPaths = (0, _helpers.extractLocalFilePathsFromText)(rawBody);
  if (directLocalPaths.length) {
    target.runtime.log?.(`local-path: 检测到用户消息包含本机路径 count=${directLocalPaths.length} intent=${(0, _helpers.looksLikeSendLocalFileIntent)(rawBody)}`);
  }
  if (directLocalPaths.length && (0, _helpers.looksLikeSendLocalFileIntent)(rawBody)) {
    const fs = await Promise.resolve().then(() => jitiImport("node:fs/promises").then((m) => _interopRequireWildcard(m)));
    const pathModule = await Promise.resolve().then(() => jitiImport("node:path").then((m) => _interopRequireWildcard(m)));
    ...
    if (imagePaths.length > 0 && otherPaths.length === 0) {
      ...
      for (const p of imagePaths) {
        try {
          ...
          const md5 = (0, _helpers.computeMd5)(buf);
      ...
      if (loaded.length > 0) {
        ...
        if (responseUrl) {
          try {
            const finalReply = (0, _helpers.buildStreamReplyFromState)(streamStore.getStream(streamId), _types.STREAM_MAX_BYTES);
            await useActiveReplyOnce(streamId, async ({ responseUrl, proxyUrl }) => {
              const res = await (0, _http.wecomFetch)(responseUrl, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(finalReply)
              }, { proxyUrl, timeoutMs: _types.REQUEST_TIMEOUT_MS });
              if (!res.ok)
      ...
      const agentOk = (0, _helpers.isAgentConfigured)(target);
      const fallbackName = imagePaths.length === 1 ?
      imagePaths[0].split("/").pop() || "image" :
      `${imagePaths.length} 张图片`;
      const prompt = (0, _helpers.buildFallbackPrompt)({
        kind: "media",
      ...
      if (agentOk && userid && userid !== "unknown") {
        for (const p of imagePaths) {
          const guessedType = (0, _helpers.guessContentTypeFromPath)(p);
          try {
            await agentDmMedia({
              target,
              userId: userid,
  ...
  if (media) {
    try {
      const maxBytes = (0, _helpers.resolveWecomMediaMaxBytes)(config);
  ...
  if (mediaPath && mediaType?.startsWith("video/")) {
    try {
      const { extractVideoFirstFrame } = await Promise.resolve().then(() => jitiImport("./video-frame.js").then((m) => _interopRequireWildcard(m)));
  ...
  // ──────────────────────────────────────────────────────────────────
  const authz = await (0, _commandAuth.resolveWecomCommandAuthorization)({
    core,
    cfg: config,
  ...
  if (authz.shouldComputeAuth && authz.commandAuthorized !== true) {
    const prompt = (0, _commandAuth.buildWecomUnauthorizedCommandPrompt)({
      senderUserId: userid,
  ...
  const attachments = mediaPath ? [{
    ...
    mimeType: mediaType,
    url: (0, _nodeUrl.pathToFileURL)(mediaPath).href
  }] : undefined;
  ...
  if (videoFirstFramePath && attachments) {
    ...
    attachments.push({
      ...
      mimeType: "image/jpeg",
      url: (0, _nodeUrl.pathToFileURL)(videoFirstFramePath).href
    });
  }
  // ──────────────────────────────────────────────────────────────────
  ...
  const cfgForDispatch = (0, _helpers.buildCfgForDispatch)(config);
  // ──────────────────────────────────────────────────────────────────
    ...
    dispatcherOptions: {
      deliver: async (payload, info) => {
        ...
        if (!payload.mediaUrl && !(payload.mediaUrls?.length ?? 0) && text.includes("/")) {
          const candidates = (0, _helpers.extractLocalImagePathsFromText)({ text, mustAlsoAppearIn: rawBody });
        ...
        if (text.trim()) {
          streamStore.updateStream(streamId, (s) => {
            (0, _helpers.appendDmContent)(s, text);
          });
        }
        // ── 超时渐进式检测（接近 6 分钟窗口，对齐 lh 版） ──
        const now = Date.now();
        const deadline = current.createdAt + _types.BOT_WINDOW_MS;
        const switchAt = deadline - _types.BOT_SWITCH_MARGIN_MS;
        ...
        while ((_mdMatch = mediaDirectiveRe.exec(text)) !== null) {
          ...
          continue;
          if (p.startsWith("~/") || p === "~") {
            const home = _nodeOs.default.homedir() || "/root";
            p = p.replace(/^~/, home);
          }
        ...
        for (const mPath of mediaUrls) {
          ...
          try {
            ...
            {
              const fs = await Promise.resolve().then(() => jitiImport("node:fs/promises").then((m) => _interopRequireWildcard(m)));
              const pathMod = await Promise.resolve().then(() => jitiImport("node:path").then((m) => _interopRequireWildcard(m)));
              buf = await fs.readFile(mPath);
              filename = pathMod.basename(mPath);
              const ext = pathMod.extname(mPath).slice(1).toLowerCase();
              contentType = _helpers.MIME_BY_EXT[ext] ?? "application/octet-stream";
            }
            if (contentType?.startsWith("image/")) {
        ...
        text.trim();
        streamStore.updateStream(streamId, (s) => {
          s.content = (0, _helpers.truncateUtf8Bytes)(nextText, _types.STREAM_MAX_BYTES);
  ...
  if (finishedState?.fallbackMode === "timeout" && !finishedState.finalDeliveredAt) {
    if (!(0, _helpers.isAgentConfigured)(target)) {
...
function getActiveReplyUrl(streamId) {
  return (0, _gateway.getMonitorState)().activeReplyStore.getUrl(streamId);
}
/**
 * 使用 response_url 发送一次性请求
 */
async function useActiveReplyOnce(streamId, fn) {
  return (0, _gateway.getMonitorState)().activeReplyStore.use(streamId, fn);
}
/**
  ...
  await useActiveReplyOnce(params.streamId, async ({ responseUrl, proxyUrl }) => {
    const payload = {
      ...
      stream: {
        id: params.streamId,
        finish: true,
        content: (0, _helpers.truncateUtf8Bytes)(params.text, _types.STREAM_MAX_BYTES) || "1"
      }
    };
    const res = await (0, _http.wecomFetch)(responseUrl, {
      method: "POST",
...
async function pushFinalStreamReplyNow(streamId) {
  const state = (0, _gateway.getMonitorState)().streamStore.getStream(streamId);
  const responseUrl = getActiveReplyUrl(streamId);
  if (!state || !responseUrl)
  return;
  const finalReply = (0, _helpers.buildStreamReplyFromState)(state, _types.STREAM_MAX_BYTES);
  await useActiveReplyOnce(streamId, async ({ responseUrl, proxyUrl }) => {
    const res = await (0, _http.wecomFetch)(responseUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function agentDmMedia(params) {
  ...
  mediaType = "voice";else
  if (ct.startsWith("video/"))
  mediaType = "video";
  const mediaId = await (0, _apiClient.uploadMedia)({ agent, type: mediaType, buffer, filename });
  await (0, _apiClient.sendMedia)({
    agent,
    toUser: userId,
    mediaId,
    mediaType,
