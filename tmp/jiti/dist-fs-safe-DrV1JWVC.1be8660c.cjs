...
var _fileIdentityOkAKtUrG = require("./file-identity-OkAKtUrG.js");
var _loggerDzNKIQlP = require("./logger-DzNKIQlP.js");
...
async function openVerifiedLocalFile(filePath, options) {
  ...
  try {
    ...
    if ((0, _boundaryPathBphsbLa.o)(err)) throw new SafeOpenError("not-found", "file not found");
    if ((0, _boundaryPathBphsbLa.c)(err)) throw new SafeOpenError("symlink", "symlink open blocked", { cause: err });
    if ((0, _boundaryPathBphsbLa.a)(err, "EISDIR")) throw new SafeOpenError("not-file", "not a file");
    throw err;
  }
  try {
    ...
    if (options?.allowSymlinkTargetWithinRoot) {
      if (!(0, _fileIdentityOkAKtUrG.t)(stat, await _promises.default.stat(filePath))) throw new SafeOpenError("path-mismatch", "path changed during read");
    } else {
      ...
      if (!(0, _fileIdentityOkAKtUrG.t)(stat, pathStat)) throw new SafeOpenError("path-mismatch", "path changed during read");
    }
    ...
    if (!(0, _fileIdentityOkAKtUrG.t)(stat, realStat)) throw new SafeOpenError("path-mismatch", "path mismatch");
    return {
      handle,
      realPath,
      stat
...
async function resolvePathWithinRoot(params) {
  ...
  try {
    ...
    if ((0, _boundaryPathBphsbLa.o)(err)) throw new SafeOpenError("not-found", "root dir not found");
    throw err;
  }
  ...
  if (!(0, _boundaryPathBphsbLa.s)(rootWithSep, resolved)) throw new SafeOpenError("outside-workspace", "file is outside workspace root");
  return {
    rootReal,
    rootWithSep,
    resolved
...
async function openFileWithinRoot(params) {
  ...
  if (!(0, _boundaryPathBphsbLa.s)(rootWithSep, opened.realPath)) {
    await opened.handle.close().catch(() => {});
...
function emitWriteBoundaryWarning(reason) {
  (0, _loggerDzNKIQlP.a)(`security: fs-safe write boundary warning (${reason})`);
}
...
async function verifyAtomicWriteResult(params) {
  ...
  try {
    if (!(0, _fileIdentityOkAKtUrG.t)(opened.stat, params.expectedIdentity)) throw new SafeOpenError("path-mismatch", "path changed during write");
    if (!(0, _boundaryPathBphsbLa.s)(rootWithSep, opened.realPath)) throw new SafeOpenError("outside-workspace", "file is outside workspace root");
  } finally {
    await opened.handle.close().catch(() => {});
  }
}
async function resolveOpenedFileRealPathForHandle(handle, ioPath) {
  ...
  for (const fdPath of fdCandidates) try {
    ...
    if ((0, _fileIdentityOkAKtUrG.t)(handleStat, await _promises.default.stat(fdRealPath))) return fdRealPath;
  } catch {}
  try {
    const ioRealPath = await _promises.default.realpath(ioPath);
    if ((0, _fileIdentityOkAKtUrG.t)(handleStat, await _promises.default.stat(ioRealPath))) return ioRealPath;
  } catch (err) {
    if (!(0, _boundaryPathBphsbLa.o)(err)) throw err;
  }
...
async function resolveOpenedFileRealPathFromParent(handleStat, ioPath) {
  ...
  try {
    ...
    if ((0, _boundaryPathBphsbLa.o)(err)) return null;
    throw err;
  }
  let entries;
  try {
  ...
  for (const entry of entries.toSorted()) {
    ...
    try {
      ...
      if (candidateStat.isFile() && (0, _fileIdentityOkAKtUrG.t)(handleStat, candidateStat)) return await _promises.default.realpath(candidatePath);
    } catch (err) {
      if (!(0, _boundaryPathBphsbLa.o)(err)) throw err;
    }
  }
...
async function openWritableFileWithinRoot(params) {
  ...
  try {
    ...
    if (!(0, _boundaryPathBphsbLa.s)(rootWithSep, resolvedRealPath)) throw new SafeOpenError("outside-workspace", "file is outside workspace root");
  ...
  try {
    ...
    if ((0, _boundaryPathBphsbLa.o)(err)) throw new SafeOpenError("not-found", "file not found");
    if ((0, _boundaryPathBphsbLa.c)(err)) throw new SafeOpenError("invalid-path", "symlink open blocked", { cause: err });
    throw err;
  }
  ...
  try {
    ...
    try {
      ...
      if (!(0, _fileIdentityOkAKtUrG.t)(stat, lstat)) throw new SafeOpenError("path-mismatch", "path changed during write");
    } catch (err) {
      if (!(0, _boundaryPathBphsbLa.o)(err)) throw err;
    }
    ...
    if (!(0, _boundaryPathBphsbLa.s)(rootWithSep, realPath)) throw new SafeOpenError("outside-workspace", "file is outside workspace root");
...
async function resolvePinnedPathWithinRoot(params) {
  const resolved = await resolvePinnedBoundaryPathWithinRoot({
    rootDir: params.rootDir,
    relativePath: params.relativePath,
    policy: _pathAliasGuardsKw7vvQoW.t.strict
  });
  ...
  if (!(0, _boundaryPathBphsbLa.s)(resolved.rootWithSep, resolved.canonicalPath)) throw new SafeOpenError("outside-workspace", "file is outside workspace root");
  return {
    rootReal: resolved.rootReal,
    resolved: resolved.canonicalPath,
    relativePosix
...
async function resolvePinnedRemovePathWithinRoot(params) {
  const resolved = await resolvePinnedBoundaryPathWithinRoot({
    rootDir: params.rootDir,
    relativePath: params.relativePath,
    policy: _pathAliasGuardsKw7vvQoW.t.unlinkTarget
  });
  const relativeResolved = _nodePath.default.relative(resolved.rootReal, resolved.canonicalPath);
  if (relativeResolved === "" || relativeResolved === "." || relativeResolved.startsWith("..") || _nodePath.default.isAbsolute(relativeResolved)) throw new SafeOpenError("outside-workspace", "file is outside workspace root");
  const relativePosix = relativeResolved.split(_nodePath.default.sep).join(_nodePath.default.posix.sep);
...
async function resolvePinnedBoundaryPathWithinRoot(params) {
  ...
  let resolved;
  try {
    resolved = await (0, _boundaryPathBphsbLa.n)({
