...
var _execDxBD2Ybt = require("./exec-DxBD2Ybt.js");
...
var _fileNameBd7rWN1y = require("./file-name-Bd7rWN1y.js");
...
function normalizeOutputFileName(value) {
  const baseName = (0, _fileNameBd7rWN1y.t)(value?.trim() || DEFAULT_OUTPUT_FILE_NAME);
...
const mediaAttachmentImageOpsLoader = (0, _lazyPromiseSFT4i6yI.n)(async () => {
  const { loadBundledPluginPublicArtifactModuleSync } = await Promise.resolve().then(() => jitiImport("./public-surface-loader-CL6x27zB.js").then((m) => _interopRequireWildcard(m)));
...
async function runPowerShellImageScript(scriptName, script, args) {
  ...
  return await withImageTemp(async (workspace) => {
    ...
    return await (0, _execDxBD2Ybt.i)(tool.command, [
    "-NoProfile",
    "-NonInteractive",
    "-ExecutionPolicy",
    "Bypass",
...
async function runConvertTool(tool, args) {
  await (0, _execDxBD2Ybt.i)(tool.command, convertToolArgs(tool, args), {
    timeoutMs: IMAGE_PROCESS_TIMEOUT_MS,
    maxBuffer: IMAGE_TOOL_MAX_BUFFER
  });
}
...
async function externalResizeToJpeg(backend, params) {
  ...
  return await withImageTemp(async (workspace) => {
    ...
    if (tool.flavor === "ffmpeg") {
      ...
      await (0, _execDxBD2Ybt.i)(tool.command, [
      "-y",
      "-i",
      input,
      "-vf",
...
async function externalConvertToJpeg(backend, buffer) {
  ...
  return await withImageTemp(async (workspace) => {
    ...
    const output = workspace.path("out.jpg");
    if (tool.flavor === "ffmpeg") await (0, _execDxBD2Ybt.i)(tool.command, [
    "-y",
    "-i",
    input,
    "-frames:v",
...
async function sipsResizeToJpeg(params) {
  return await withImageTemp(async (workspace) => {
    const input = await workspace.write("in.img", params.buffer);
    const output = workspace.path("out.jpg");
    await (0, _execDxBD2Ybt.i)("/usr/bin/sips", [
    "-Z",
