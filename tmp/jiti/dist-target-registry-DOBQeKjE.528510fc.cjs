...
function loadBundledChannelPublicArtifact(channelId, artifactBasenames) {
  for (const artifactBasename of artifactBasenames) try {
    return (0, _publicSurfaceLoaderDy6S3uW_.t)({
      dirName: channelId,
      artifactBasename,
...
function listBundledWebProviderSecretTargetRegistryEntries() {
  const entries = [];
  for (const record of (0, _pluginRegistryBjn9rPwy.n)({ includeDisabled: true }).plugins) {
...
function listBundledPluginConfigSecretTargetRegistryEntries() {
  ...
  for (const record of (0, _bundledPluginMetadataVxOxTVqO.n)({
...
function parsePathPattern(pathPattern) {
  return (0, _sharedC9ga15VD.i)(pathPattern).map((segment) => {
...
function expandPathTokens(root, tokens) {
  ...
  const walk = (node, tokenIndex, segments, captures) => {
    ...
    if (token.kind === "literal") {
      if (!(0, _utilsDvkbxKCZ.c)(node)) return;
      if (isLeaf) {
        out.push({
...
function discoverSecretTargetsFromEntries(source, discoveryEntries) {
  ...
  for (const entry of discoveryEntries) {
    ...
    for (const match of expanded) {
      ...
      if (seen.has(key)) continue;
      seen.add(key);
      const refValue = resolved.refPathSegments ? (0, _pathUtilsBF77VRFq.n)(source, resolved.refPathSegments) : void 0;
      out.push({
        entry,
