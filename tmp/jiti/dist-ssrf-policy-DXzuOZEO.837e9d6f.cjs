...
var _recordCoerceCRZjEt1j = require("./record-coerce-CRZjEt1j.js");
...
function isPrivateNetworkOptInEnabled(input) {
  ...
  const record = (0, _recordCoerceCRZjEt1j.n)(input);
  if (!record) return false;
  const network = (0, _recordCoerceCRZjEt1j.n)(record.network);
...
function hasLegacyFlatAllowPrivateNetworkAlias(value) {
  const entry = (0, _recordCoerceCRZjEt1j.n)(value);
...
function migrateLegacyFlatAllowPrivateNetworkAlias(params) {
  ...
  const currentNetworkRecord = (0, _recordCoerceCRZjEt1j.n)(params.entry.network);
...
function hasLegacyAllowPrivateNetworkInAccounts(value) {
  const accounts = (0, _recordCoerceCRZjEt1j.n)(value);
  return Boolean(accounts && Object.values(accounts).some((account) => hasLegacyFlatAllowPrivateNetworkAlias((0, _recordCoerceCRZjEt1j.n)(account) ?? {})));
}
function createLegacyPrivateNetworkDoctorContract(params) {
  ...
  return {
    legacyConfigRules: [{
      ...
      match: (value) => hasLegacyFlatAllowPrivateNetworkAlias((0, _recordCoerceCRZjEt1j.n)(value) ?? {})
    }, {
      path: [
      "channels",
      params.channelKey,
    ...
    normalizeCompatibilityConfig: ({ cfg }) => {
      const channelEntry = (0, _recordCoerceCRZjEt1j.n)((0, _recordCoerceCRZjEt1j.n)(cfg.channels)?.[params.channelKey]);
      ...
      const accounts = (0, _recordCoerceCRZjEt1j.n)(updatedChannel.accounts);
      if (accounts) {
        let accountsChanged = false;
        const nextAccounts = { ...accounts };
        for (const [accountId, accountValue] of Object.entries(accounts)) {
          const account = (0, _recordCoerceCRZjEt1j.n)(accountValue);
          if (!account) continue;
...
async function assertHttpUrlTargetsPrivateNetwork(url, params = {}) {
  ...
  if ((0, _ssrfCUQ1WjrX.c)(hostname)) return;
  ...
  if (!(await (0, _ssrfCUQ1WjrX.g)(hostname, {
    lookupFn: params.lookupFn,
    policy: ssrfPolicyFromDangerouslyAllowPrivateNetwork(true)
  })).addresses.every((address) => (0, _ssrfCUQ1WjrX.u)(address))) throw new Error(errorMessage);
}
