...
var _agentScopeCf7T6Ju = require("./agent-scope-Cf7T6Ju7.js");
...
function resolveAgentModelEntryRuntimePolicy(params) {
  ...
  const { sessionAgentId } = (0, _agentScopeCf7T6Ju.v)({
    config: params.config,
    agentId: params.agentId,
    sessionKey: params.sessionKey
  });
  const modelMaps = [(0, _agentScopeConfigDUwbsKjv.t)(params.config).find((entry) => (0, _sessionKeyCQewiu8n.c)(entry.id) === sessionAgentId)?.models, params.config.agents?.defaults?.models];
