"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.STREAM_MAX_DM_BYTES = exports.MIME_BY_EXT = void 0;exports.appendDmContent = appendDmContent;exports.buildCfgForDispatch = buildCfgForDispatch;exports.buildFallbackPrompt = buildFallbackPrompt;exports.buildInboundBody = buildInboundBody;exports.buildStreamPlaceholderReply = buildStreamPlaceholderReply;exports.buildStreamReplyFromState = buildStreamReplyFromState;exports.buildStreamResponse = buildStreamResponse;exports.buildStreamTextPlaceholderReply = buildStreamTextPlaceholderReply;exports.computeMd5 = computeMd5;exports.computeTaskKey = computeTaskKey;exports.extractLocalFilePathsFromText = extractLocalFilePathsFromText;exports.extractLocalImagePathsFromText = extractLocalImagePathsFromText;exports.formatQuote = formatQuote;exports.guessContentTypeFromPath = guessContentTypeFromPath;exports.hasMedia = hasMedia;exports.isAgentConfigured = isAgentConfigured;exports.looksLikeSendLocalFileIntent = looksLikeSendLocalFileIntent;exports.processInboundMessage = processInboundMessage;exports.resolveWecomMediaMaxBytes = resolveWecomMediaMaxBytes;exports.resolveWecomSenderUserId = resolveWecomSenderUserId;exports.truncateUtf8Bytes = truncateUtf8Bytes;
...
// 常量
// ============================================================================
/** DM 文本最大字节数上限 */const STREAM_MAX_DM_BYTES = exports.STREAM_MAX_DM_BYTES = 200_000; /** MIME 扩展名映射表 */const MIME_BY_EXT = exports.MIME_BY_EXT = { png: "image/png",
  jpg: "image/jpeg",
  jpeg: "image/jpeg",
  gif: "image/gif",
  webp: "image/webp",
...
function computeMd5(data) {
  return _nodeCrypto.default.createHash("md5").update(data).digest("hex");
}
// ============================================================================
// 配置解析
// ============================================================================
 ...
 */
async function processInboundMessage(target, msg) {
  const { decryptWecomMediaWithMeta } = await Promise.resolve().then(() => jitiImport("./media.js").then((m) => _interopRequireWildcard(m)));
  const { resolveWecomEgressProxyUrl } = await Promise.resolve().then(() => jitiImport("../utils.js").then((m) => _interopRequireWildcard(m)));
...
function inferInboundMediaMeta(params) {
  ...
  if (params.kind === "image") {
    ...
    contentType = magicType;else
    if (headerType?.startsWith("image/"))
    contentType = headerType;else
    if (typeByName?.startsWith("image/"))
    contentType = typeByName;else
