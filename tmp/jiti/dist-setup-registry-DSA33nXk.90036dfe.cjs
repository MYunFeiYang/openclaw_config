"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolvePluginSetupRegistry;exports.i = resolvePluginSetupProvider;exports.n = resolvePluginSetupAutoEnableReasons;exports.o = runPluginSetupConfigMigrations;exports.r = resolvePluginSetupCliBackend;exports.t = clearPluginSetupRegistryCache;var _pluginRegistryCutMFnk = require("./plugin-registry-Cut-MFnk.js");
...
var _configContractsB8EUdyU_ = require("./config-contracts-B8EUdyU_.js");
...
const CURRENT_MODULE_PATH = (0, _nodeUrl.fileURLToPath)("file:///Users/thinkway/.nvm/versions/node/v24.14.0/lib/node_modules/openclaw/dist/setup-registry-DSA33nXk.js");
...
function getModuleLoader(modulePath) {
  return (0, _pluginModuleLoaderCacheB600Kx.n)({
    cache: moduleLoaders,
    modulePath,
    importerUrl: "file:///Users/thinkway/.nvm/versions/node/v24.14.0/lib/node_modules/openclaw/dist/setup-registry-DSA33nXk.js"
  });
}
...
function resolveRelevantSetupMigrationPluginIds(params) {
  ...
  for (const plugin of registry.plugins) {
    ...
    if (paths.some((pathPattern) => (0, _configContractsB8EUdyU_.t)({
      root: params.config,
      pathPattern
...
function findUniqueSetupManifestOwner(params) {
  const matches = params.registry.plugins.filter((entry) => params.listIds(entry).some((id) => (0, _providerIdDIRgKpoh.r)(id) === params.normalizedId));
...
function mapNormalizedIds(ids) {
  ...
  for (const id of ids) {
    const normalized = (0, _providerIdDIRgKpoh.r)(id);
    ...
    const api = buildSetupPluginApi({
      ...
      handlers: {
        registerProvider(provider) {
          const key = `${record.id}:${(0, _providerIdDIRgKpoh.r)(provider.id)}`;
        ...
        registerCliBackend(backend) {
          const key = `${record.id}:${(0, _providerIdDIRgKpoh.r)(backend.id)}`;
...
function resolvePluginSetupProvider(params) {
  ...
  const api = buildSetupPluginApi({
    ...
    handlers: {
      registerProvider(provider) {
        const key = (0, _providerIdDIRgKpoh.r)(provider.id);
...
function resolvePluginSetupCliBackend(params) {
  const normalized = (0, _providerIdDIRgKpoh.r)(params.backend);
  const env = params.env ?? process.env;
  const record = findUniqueSetupManifestOwner({
    registry: loadSetupManifestRegistry({
      config: params.config,
  ...
  const api = buildSetupPluginApi({
    ...
    handlers: {
      ...
      registerAutoEnableProbe() {},
      registerCliBackend(backend) {
        const key = (0, _providerIdDIRgKpoh.r)(backend.id);
