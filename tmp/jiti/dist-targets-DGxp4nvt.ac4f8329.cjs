...
function listConfiguredSessionStoreAgentIds(cfg) {
  const ids = new Set((0, _agentScopeConfigDUwbsKjv.n)(cfg).map((agentId) => (0, _sessionKeyCQewiu8n.c)(agentId)));
...
function resolveSessionStoreTargets(cfg, opts, params = {}) {
  const env = params.env ?? process.env;
  const defaultAgentId = (0, _agentScopeConfigDUwbsKjv.c)(cfg);
  ...
  if (hasAgent) {
    const knownAgents = (0, _agentScopeConfigDUwbsKjv.n)(cfg);
