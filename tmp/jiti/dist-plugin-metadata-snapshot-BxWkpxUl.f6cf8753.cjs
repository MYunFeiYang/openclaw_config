...
function isDiagnosticsTimelineEnabled(options = {}) {
  ...
  return ((0, _diagnosticFlagsBAYJHjtr.t)("timeline", config, env) || (0, _diagnosticFlagsBAYJHjtr.t)("diagnostics.timeline", config, env) || (0, _envMqKSAeAo.t)(env.OPENCLAW_DIAGNOSTICS)) && typeof env.OPENCLAW_DIAGNOSTICS_TIMELINE_PATH === "string" && env.OPENCLAW_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0;
}
...
function resolvePersistedRegistryMemoContextHash(params) {
  return (0, _installedPluginIndexStoreIOgCdyHd.b)({
...
function hashWatchedFiles(watchedFiles) {
  return (0, _installedPluginIndexStoreIOgCdyHd.b)(watchedFiles.map((filePath) => fileFingerprint(filePath)));
}
function resolvePersistedRegistryMemoState(params) {
  ...
  const fastHash = (0, _installedPluginIndexStoreIOgCdyHd.b)(fastFingerprint);
  ...
  if (isRecord(fastFingerprint) && fastFingerprint.disabled === true) return {
    ...
    watchedFiles: [],
    watchedFilesHash: (0, _installedPluginIndexStoreIOgCdyHd.b)([])
  };
  const indexPath = (0, _installedPluginIndexRecordReaderDR3h__QQ.a)({
    env: params.env,
    ...(params.stateDir ? { stateDir: params.stateDir } : {})
  ...
  return {
    ...
    fingerprint: {
      ...fastFingerprint,
      indexHash: (0, _installedPluginIndexStoreIOgCdyHd.b)(stableMemoValue(index) ?? null),
      installRecords: (0, _installedPluginIndexStoreIOgCdyHd.b)(stableMemoValue(installRecords)),
      installRecordFiles,
...
function computePluginMetadataSnapshotMemoKey(params) {
  ...
  const indexFingerprint = snapshotParams.index ? (0, _manifestRegistryInstalled7RUx9j9g.n)(snapshotParams.index) : void 0;
  return (0, _installedPluginIndexStoreIOgCdyHd.b)({
    controlPlane: (0, _pluginRegistryIimloLsx.C)({
      config: snapshotParams.config,
      env,
      workspaceDir: snapshotParams.workspaceDir,
      policyHash: (0, _installedPluginIndexStoreIOgCdyHd._)(snapshotParams.config),
    ...
    pathPolicy: {
      compatibilityHostVersion: (0, _versionB2G3zXnp.o)(env),
...
function resolvePluginMetadataControlPlaneFingerprint(params) {
  return (0, _pluginRegistryIimloLsx.C)(params);
}
function indexesMatch(left, right) {
  if (!left || !right) return true;
  return (0, _manifestRegistryInstalled7RUx9j9g.n)(left) === (0, _manifestRegistryInstalled7RUx9j9g.n)(right);
}
...
function isPluginMetadataSnapshotCompatible(params) {
  ...
  return params.snapshot.policyHash === (0, _installedPluginIndexStoreIOgCdyHd._)(params.config) && (!params.snapshot.configFingerprint || params.snapshot.configFingerprint === resolvePluginMetadataControlPlaneFingerprint({
...
function loadPluginMetadataSnapshotImpl(params) {
  ...
  const registryResult = (0, _pluginRegistryIimloLsx.m)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    ...(params.stateDir ? { stateDir: params.stateDir } : {}),
    env: params.env,
  ...
  const manifestRegistry = index.plugins.length === 0 ? (0, _manifestRegistryCFqIGq5y.t)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env,
    diagnostics: [...index.diagnostics],
    installRecords: index.installRecords
  }) : (0, _manifestRegistryInstalled7RUx9j9g.t)({
    index,
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env,
  ...
  const normalizePluginId = (0, _pluginRegistryIimloLsx.x)(index, { manifestRegistry });
