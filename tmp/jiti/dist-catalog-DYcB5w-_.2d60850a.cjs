"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = getChannelPluginCatalogEntry;exports.r = listChannelPluginCatalogEntries;exports.t = buildChannelUiCatalog;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
...
var _installSourceInfoD4ewXUGX = require("./install-source-info-D4ewXUGX.js");
var _officialExternalPluginCatalog64MlR6o = require("./official-external-plugin-catalog--64MlR6o.js");
...
function parseCatalogEntries(raw) {
  if (Array.isArray(raw)) return raw.filter((entry) => (0, _utilsD5swhEXt.c)(entry));
  if (!(0, _utilsD5swhEXt.c)(raw)) return [];
  const list = raw.entries ?? raw.packages ?? raw.plugins;
  if (!Array.isArray(list)) return [];
  return list.filter((entry) => (0, _utilsD5swhEXt.c)(entry));
}
function splitEnvPaths(value) {
  const trimmed = value.trim();
  if (!trimmed) return [];
  return trimmed.split(/[;,]/g).flatMap((chunk) => chunk.split(_nodePath.default.delimiter)).map((entry) => entry.trim()).filter(Boolean);
}
...
function loadExternalCatalogEntries(options) {
  return loadCatalogEntriesFromPaths(resolveExternalCatalogPaths(options).map((rawPath) => (0, _utilsD5swhEXt.p)(rawPath, options.env ?? process.env)));
}
function loadCatalogEntriesFromPaths(paths) {
  const entries = [];
  for (const resolvedPath of paths) {
    if (!_nodeFs.default.existsSync(resolvedPath)) continue;
    try {
      const payload = JSON.parse(_nodeFs.default.readFileSync(resolvedPath, "utf-8"));
...
function loadOfficialCatalogEntriesFromPaths(paths) {
  ...
  for (const resolvedPath of paths) {
    ...
    try {
      const parsed = parseCatalogEntries(JSON.parse(_nodeFs.default.readFileSync(resolvedPath, "utf-8")));
...
function resolveOfficialCatalogPaths(options) {
  ...
  const candidates = [(0, _openclawRootCRSCIPqz.n)({ cwd: process.cwd() }), (0, _openclawRootCRSCIPqz.n)({ moduleUrl: "file:///Users/thinkway/.nvm/versions/node/v24.14.0/lib/node_modules/openclaw/dist/catalog-DYcB5w-_.js" })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index).map((packageRoot) => _nodePath.default.join(packageRoot, OFFICIAL_CHANNEL_CATALOG_RELATIVE_PATH));
  if (process.execPath) {
    const execDir = _nodePath.default.dirname(process.execPath);
    candidates.push(_nodePath.default.join(execDir, OFFICIAL_CHANNEL_CATALOG_RELATIVE_PATH));
    candidates.push(_nodePath.default.join(execDir, "channel-catalog.json"));
  }
  return candidates.filter((entry, index, all) => entry && all.indexOf(entry) === index);
}
function loadOfficialCatalogEntries(options) {
  const builtInEntries = (0, _officialExternalPluginCatalog64MlR6o.i)();
...
function resolveInstallInfo(params) {
  const clawhubSpec = (0, _stringCoerceBje8XVt.c)(params.install?.clawhubSpec);
  let npmSpec = (0, _stringCoerceBje8XVt.c)(params.install?.npmSpec) ?? (0, _stringCoerceBje8XVt.c)(params.packageName);
  const packageVersion = (0, _stringCoerceBje8XVt.c)(params.packageVersion);
  const parsedNpmSpec = npmSpec ? (0, _discoveryCVL9KJt.F)(npmSpec) : null;
  const expectedPackageName = (0, _stringCoerceBje8XVt.c)(params.packageName);
  const parsedPackageName = expectedPackageName ? (0, _discoveryCVL9KJt.F)(expectedPackageName) : null;
  if (npmSpec && packageVersion && (0, _discoveryCVL9KJt.P)(packageVersion) && parsedNpmSpec?.selectorKind === "none" && (!parsedPackageName || parsedNpmSpec.name === parsedPackageName.name)) npmSpec = `${parsedNpmSpec.name}@${packageVersion}`;
  if (!clawhubSpec && !npmSpec) return null;
  let localPath = (0, _stringCoerceBje8XVt.c)(params.install?.localPath);
  if (!localPath && params.workspaceDir && params.packageDir) localPath = _nodePath.default.relative(params.workspaceDir, params.packageDir) || void 0;
...
function buildCatalogEntryFromManifest(params) {
  ...
  const pluginId = (0, _stringCoerceBje8XVt.c)(params.pluginId);
  return {
    id,
    ...(pluginId ? { pluginId } : {}),
    ...
    meta,
    install,
    installSource: (0, _installSourceInfoD4ewXUGX.t)(install, { expectedPackageName: params.packageName })
  };
}
function buildExternalCatalogEntry(entry, options) {
  const manifest = entry[_legacyNamesB8PgFyB.n];
...
function listChannelPluginCatalogEntries(options = {}) {
  const manifestEntries = (0, _channelCatalogRegistryCNXtcf4Q.t)({
