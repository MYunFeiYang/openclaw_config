"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = isTaskDue;exports.c = stripHeartbeatToken;exports.i = isHeartbeatContentEffectivelyEmpty;exports.n = void 0;exports.o = parseHeartbeatTasks;exports.r = void 0;exports.s = resolveHeartbeatPrompt;exports.t = void 0;var _stringCoerceBje8XVt = require("./string-coerce-Bje8XVt9.js");
var _utilsDvkbxKCZ = require("./utils-DvkbxKCZ.js");
var _parseDurationSSTfcZq = require("./parse-duration-sSTfcZq4.js");
var _tokensBIJuzXU = require("./tokens-BIJuzXU-.js");
//#region src/auto-reply/heartbeat.ts
const HEARTBEAT_PROMPT = exports.n = "Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.";
const HEARTBEAT_TRANSCRIPT_PROMPT = exports.r = "[OpenClaw heartbeat poll]";
const DEFAULT_HEARTBEAT_ACK_MAX_CHARS = exports.t = 300;
/**
...
function resolveHeartbeatPrompt(raw) {
  return ((0, _stringCoerceBje8XVt.c)(raw) ?? "") || "Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply HEARTBEAT_OK.";
}
function stripTokenAtEdges(raw) {
  ...
  const token = _tokensBIJuzXU.t;
  const tokenAtEndWithOptionalTrailingPunctuation = new RegExp(`${(0, _utilsDvkbxKCZ.b)(token)}[^\\w]{0,4}$`);
  ...
  while (changed) {
    ...
    if (tokenAtEndWithOptionalTrailingPunctuation.test(next)) {
      ...
      if (!before) text = "";else
      text = `${before}${next.slice(idx + token.length).trimStart()}`.trimEnd();
      didStrip = true;
      changed = true;
    }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith("- name:")) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        ...
        if (nextTrimmed.startsWith("interval:") && (nextLine.startsWith(" ") || nextLine.startsWith("	"))) interval = nextTrimmed.replace("interval:", "").trim().replace(/^["']|["']$/g, "");else
        if (nextTrimmed.startsWith("prompt:") && (nextLine.startsWith(" ") || nextLine.startsWith("	"))) prompt = nextTrimmed.replace("prompt:", "").trim().replace(/^["']|["']$/g, "");else
...
function isTaskDue(lastRunMs, interval, nowMs) {
  ...
  try {
    const intervalMs = (0, _parseDurationSSTfcZq.t)(interval, { defaultUnit: "m" });
    return nowMs - lastRunMs >= intervalMs;
  } catch {
    return false;
  }
}
//#endregion /* v9-d0da5acb2e7edb26 */
