...
function applyPluginCompatibilityOverrides(params) {
  const allowlistCompat = params.compat?.allowlistPluginIds?.length ? (0, _bundledCompatC5xrsnA.t)({
    config: params.config,
    pluginIds: params.compat.allowlistPluginIds
  }) : params.config;
  const enablementCompat = params.compat?.enablementPluginIds?.length ? (0, _bundledCompatC5xrsnA.n)({
    config: allowlistCompat,
    pluginIds: params.compat.enablementPluginIds
  }) : allowlistCompat;
  return params.compat?.vitestPluginIds?.length ? (0, _bundledCompatC5xrsnA.r)({
...
function applyPluginAutoEnableForActivation(params) {
  const currentSnapshot = (0, _pluginRegistryIimloLsx.v)({
    config: params.config,
    env: params.env,
    workspaceDir: params.workspaceDir,
    allowWorkspaceScopedSnapshot: true
  });
  const defaultDiscoverySnapshot = (0, _configStateCOhlckC.s)(params.config.plugins).loadPaths.length === 0 ? (0, _pluginRegistryIimloLsx.v)({
    env: params.env,
    workspaceDir: params.workspaceDir,
    allowWorkspaceScopedSnapshot: true,
  ...
  return (0, _pluginAutoEnableQ8Kl1Dsn.t)({
    config: params.config,
    env: params.env,
