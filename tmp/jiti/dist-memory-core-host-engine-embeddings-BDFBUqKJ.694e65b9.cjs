"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.C = withRemoteHttpResponse;exports.D = sanitizeAndNormalizeEmbedding;exports.E = createLocalEmbeddingProvider;exports.S = exports.O = void 0;exports.T = formatUnavailableBatchError;exports._ = buildBatchHeaders;exports.a = normalizeEmbeddingModelWithPrefixes;exports.b = applyEmbeddingBatchOutputLine;exports.c = mapBatchEmbeddingsByIndex;exports.d = uploadBatchJsonlFile;exports.f = resolveBatchCompletionFromStatus;exports.g = runEmbeddingBatchGroups;exports.h = buildEmbeddingBatchGroupOptions;exports.i = resolveRemoteEmbeddingBearerClient;exports.l = sanitizeEmbeddingCacheHeaders;exports.m = throwIfBatchTerminalFailure;exports.n = resolveRemoteEmbeddingClient;exports.o = debugEmbeddingsLog;exports.p = resolveCompletedBatchResult;exports.r = fetchRemoteEmbeddingVectors;exports.s = isMissingEmbeddingApiKeyError;exports.t = createRemoteEmbeddingProvider;exports.u = enforceEmbeddingMaxInputTokens;exports.v = normalizeBatchBaseUrl;exports.w = extractBatchErrorMessage;exports.x = postJsonWithRetry;exports.y = void 0;var _proxyEnvDJkETjnr = require("./proxy-env-DJkETjnr.js");
var _modelAuthRuntimeSharedBX_oC9YK = require("./model-auth-runtime-shared-BX_oC9YK.js");
var _ssrfWmwvy = require("./ssrf-Wmwvy157.js");
var _fetchGuardVkqdiYWX = require("./fetch-guard-VkqdiYWX.js");
var _modelAuthCsyLGY9m = require("./model-auth-CsyLGY9m.js");
var _stringUtilsDoQjWCc = require("./string-utils-DoQjWCc3.js");
require("./openclaw-runtime-qAsu6jGz.js");
...
var _secretInputBetbU_3S = require("./secret-input-BetbU_3S.js");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
//#region packages/memory-host-sdk/src/host/embedding-defaults.ts
const DEFAULT_LOCAL_MODEL = exports.O = "hf:ggml-org/embeddinggemma-300m-qat-q8_0-GGUF/embeddinggemma-300m-qat-Q8_0.gguf";
//#endregion
...
async function importNodeLlamaCpp() {
  return ((specifier) => new Promise((r) => r(`${specifier}`)).then((s) => jitiImport(s).then((m) => _interopRequireWildcard(m))))(NODE_LLAMA_CPP_MODULE);
}
//#endregion
//#region packages/memory-host-sdk/src/host/embeddings.ts
async function createLocalEmbeddingProvider(options) {
  const modelPath = (0, _stringUtilsDoQjWCc.r)(options.local?.modelPath) || "hf:ggml-org/embeddinggemma-300m-qat-q8_0-GGUF/embeddinggemma-300m-qat-Q8_0.gguf";
  const modelCacheDir = (0, _stringUtilsDoQjWCc.r)(options.local?.modelCacheDir);
...
const buildRemoteBaseUrlPolicy = exports.S = _ssrfWmwvy._;
async function withRemoteHttpResponse(params) {
  const guardedFetch = params.fetchWithSsrFGuardImpl ?? _fetchGuardVkqdiYWX.n;
  const shouldUseEnvProxy = params.shouldUseEnvHttpProxyForUrlImpl ?? _proxyEnvDJkETjnr.c;
...
const EMBEDDING_BATCH_ENDPOINT = exports.y = "/v1/embeddings";
//#endregion
...
async function runEmbeddingBatchGroups(params) {
  ...
  await (0, _internalBhQAXls.d)(tasks, params.concurrency);
  return byCustomId;
}
...
async function uploadBatchJsonlFile(params) {
  ...
  form.append("file", new Blob([jsonl], { type: "application/jsonl" }), `memory-embeddings.${(0, _hashB7y7CIO.t)(String(Date.now()))}.jsonl`);
...
function enforceEmbeddingMaxInputTokens(provider, chunks, hardMaxInputTokens) {
  ...
  for (const chunk of chunks) {
    if ((0, _internalBhQAXls.m)(chunk.embeddingInput)) {
      out.push(chunk);
      continue;
    }
    if ((0, _internalBhQAXls.g)(chunk.text) <= maxInputTokens) {
      out.push(chunk);
      continue;
    }
    for (const text of (0, _internalBhQAXls._)(chunk.text, maxInputTokens)) out.push({
...
function sanitizeEmbeddingCacheHeaders(headers, excludedHeaderNames) {
  const excluded = new Set(excludedHeaderNames.map((name) => (0, _stringUtilsDoQjWCc.t)(name)));
  return Object.entries(headers).filter(([key]) => !excluded.has((0, _stringUtilsDoQjWCc.t)(key))).toSorted(([a], [b]) => a.localeCompare(b)).map(([key, value]) => [key, value]);
}
...
function isTruthyEnvValue(value) {
  switch ((0, _stringUtilsDoQjWCc.t)(value)) {
    case "1":
    case "on":
    case "true":
...
async function resolveRemoteEmbeddingBearerClient(params) {
  ...
  const remoteApiKey = (0, _secretInputBetbU_3S.n)({
    value: remote?.apiKey,
    path: "agents.*.memorySearch.remote.apiKey"
  });
  const remoteBaseUrl = (0, _stringUtilsDoQjWCc.r)(remote?.baseUrl);
  const providerConfig = params.options.config.models?.providers?.[params.provider];
  const apiKey = remoteApiKey ? remoteApiKey : (0, _modelAuthRuntimeSharedBX_oC9YK.t)(await (0, _modelAuthCsyLGY9m.s)({
    provider: params.provider,
    cfg: params.options.config,
    agentDir: params.options.agentDir
  }), params.provider);
  const baseUrl = remoteBaseUrl || (0, _stringUtilsDoQjWCc.r)(providerConfig?.baseUrl) || params.defaultBaseUrl;
